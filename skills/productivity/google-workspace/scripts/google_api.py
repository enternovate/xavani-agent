...
"""Google Workspace API CLI for Xavani Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Xavani-facing JSON contract and falls back to the Python client
...
def _gws_binary() -> str | None:
    override = os.getenv("XAVANI_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Xavani Agent")
