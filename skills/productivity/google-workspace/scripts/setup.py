...
"""Google Workspace OAuth2 setup for Xavani Agent.
...
from _xavani_home import display_xavani_home, get_xavani_home

XAVANI_HOME = get_xavani_home()
TOKEN_PATH = XAVANI_HOME / "google_token.json"
CLIENT_SECRET_PATH = XAVANI_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = XAVANI_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Xavani profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'xavani-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Xavani home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_xavani_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Xavani")
