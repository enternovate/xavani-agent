...
Extracted verbatim from xavani_state.py so the SessionDB mixin modules
(xavani_state_search / xavani_state_schema / xavani_state_portability) can
reference them without importing xavani_state (which would be a cycle).
xavani_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``xavani sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-Xavani-Session-Key / build_session_key)
...
# `xavani sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `xavani db optimize`. This is
...
#
# Several independent Xavani processes routinely share one state.db (gateway
# service, the Desktop app's `xavani serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `xavani_state._cross_process_repair_lock` (the schema-
...
# xavani_state) because the search/schema mixins cannot import xavani_state
# (cycle).
#
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-Xavani)"
    pid = record.get("pid")
