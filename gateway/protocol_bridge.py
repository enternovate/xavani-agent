...
class OpenAPIAdapter:
    ...
    def _load_spec_url(self, url: str) -> Dict[str, Any]:
        ...
        URL validation is delegated to :func:`gateway.security.assert_safe_url`,
        the project's central SSRF guard. It blocks non-HTTP(S) schemes,
        URLs with embedded credentials, hosts that resolve to private /
        loopback / link-local space, **and** cloud-metadata endpoints
        (AWS/Azure/GCP/Oracle IMDS at ``169.254.169.254``, Alibaba at
        ``100.100.100.200``, AWS IMDSv6) - the latter were missing from
        this module's previous home-grown checker, which is why CodeQL
        flagged ``py/full-ssrf`` here.
        """
        import urllib.request

        from gateway.security import URLValidationError, assert_safe_url

        try:
            assert_safe_url(url)
        except URLValidationError as exc:
            raise ValueError(f"Blocked potentially unsafe URL: {exc}") from exc
