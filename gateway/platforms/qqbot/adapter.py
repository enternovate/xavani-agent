...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.xavani/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Xavani upgrades overwrite this
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.xavani/.update_response`` for the
          detached ``xavani update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``xavani update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `xavani setup gateway` / `xavani gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``xavani update --gateway`` watcher. Button
        ...
        ``~/.xavani/.update_response`` so the detached update process
        can read it.
        """
