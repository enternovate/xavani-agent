...
def _get_xavani_version() -> str:
    """Return the xavani-agent package version, or 'dev' if unavailable."""
    try:
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Xavani/<xavani_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; Xavani/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    xavani_version = _get_xavani_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; Xavani/{xavani_version})"
