...
There are two ways to add a platform to the Xavani gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.xavani/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Xavani code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `xavani gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `xavani_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Xavani core.
...
And add it to the `xavani-gateway` composite:

```python
"xavani-gateway": {
    "includes": [..., "xavani-your-platform"]
}
```
...
## 12. Status Display (`xavani_cli/status.py`)
...
## 13. Gateway Setup Wizard (`xavani_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ xavani_cli/ toolsets.py \
  --include="*.py" -l | sort -u
