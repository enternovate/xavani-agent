...
try:
    ...
    _XAVANI_VERSION = "0.0.0"

_APP_VERSION = _XAVANI_VERSION
_BOT_VERSION = _XAVANI_VERSION
_YUANBAO_INSTANCE_ID = str(XAVANI_INSTANCE_ID)  # single source: yuanbao_proto.XAVANI_INSTANCE_ID
...
class ImageFileHandler(MediaSendHandler):
    ...
    async def acquire_file(self, adapter, **kwargs):
        ...
        logger.info("[%s] ImageFileHandler: reading %s", adapter.name, safe_path)
...
class DocumentHandler(MediaSendHandler):
    ...
    async def acquire_file(self, adapter, **kwargs):
        ...
        logger.info("[%s] DocumentHandler: reading %s", adapter.name, safe_path)
        with open(safe_path, "rb") as f:
            file_bytes = f.read()
