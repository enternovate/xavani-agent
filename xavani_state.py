...
"""
SQLite State Store for Xavani Agent.
...
DEFAULT_DB_PATH = get_xavani_home() / "state.db"
...
# xavani_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``xavani_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see xavani_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple xavani processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``xavani_state._set_last_init_error(None)`` explicitly.
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Xavani) receive
    ...
    def _sanitize_fts5_query(query: str) -> str:
        ...
        sanitized = re.sub(r"\b(\w+(?:[._-]\w+){1,5})\b", r'"\1"', sanitized)
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Xavani, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Xavani session.

        A Xavani session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Xavani session is already bound to any Telegram DM topic.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``xavani doctor`` / automatic schema surgery).
