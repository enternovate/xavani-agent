...
"""ACP session manager — maps ACP sessions to Xavani AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.xavani/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Xavani itself is running in WSL.

    Windows ACP clients can launch ``xavani acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Xavani from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["xavani-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Xavani agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Xavani AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Xavani runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.xavani/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``XAVANI_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_xavani_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=xavani_home / "state.db")
            return self._db_instance
        except Exception:
