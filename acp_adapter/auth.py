...
"""ACP auth helpers — detect and advertise Xavani authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "xavani-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Xavani runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Xavani can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Xavani.
    ...
    not have Xavani provider credentials configured yet, so Xavani always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Xavani using the currently configured "
