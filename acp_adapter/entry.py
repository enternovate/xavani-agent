...
"""CLI entry point for the xavani-agent ACP adapter.

Loads environment variables from ``~/.xavani/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from XAVANI_HOME (default ``~/.xavani``)."""
    from xavani_cli.env_loader import load_xavani_dotenv

    xavani_home = get_xavani_home()
    loaded = load_xavani_dotenv(xavani_home=xavani_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", xavani_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="xavani-acp",
        description="Run Xavani Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Xavani version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(xavani_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import XavaniACPAgent  # noqa: F401

    print("Xavani ACP check OK")


def _run_setup() -> None:
    from xavani_cli.main import main as xavani_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "xavani", "model"]
        xavani_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``xavani postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    import acp
    from .server import XavaniACPAgent
