...
an unlinked SQLite DB/WAL/SHM generation.  ``xavani_state`` supplies only the
...
_XAVANI_EXECUTABLES = frozenset({"xavani", "xavani-agent", "xavani-acp"})
_XAVANI_PYTHON_MODULES = frozenset({"acp_adapter", "xavani_cli.main"})
_XAVANI_PYTHON_SCRIPTS = frozenset({"xavani_cli/main.py", "run_agent.py"})
...
def _looks_like_xavani(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Xavani execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _XAVANI_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _XAVANI_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _XAVANI_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_xavani(argv):
                        cmdline = " ".join(argv)
