...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/enternovate/xavani-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    xavani_meta = (fm.get("metadata") or {}).get("xavani") or {}
    tags = xavani_meta.get("tags") or []
    related = xavani_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `xavani skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Xavani Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Xavani ships with a large built-in skill library copied into `~/.xavani/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Xavani also syncs bundled skills on `xavani update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.xavani/skills/` tree, it is still shipped with Xavani; restore it with `xavani skills reset <name> --restore`.",
        "",
...
def write_sidebar(entries):
    ...
    # NOTE: The original regex approach used (?:.*?\n)*? which is vulnerable to
    # polynomial backtracking. Instead, we use the text.find + brace-depth
    # walk below (lines 716+) which is both safer and more reliable.
