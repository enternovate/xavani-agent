---
title: "Xavani Tool Gateway（工具网关）"
description: "通过 Xavani Portal 订阅统一使用网页搜索、文生图、语音合成与浏览器自动化，无需单独申请 Firecrawl、FAL、OpenAI、Browser Use 等 API Key"
sidebar_label: "Tool Gateway"
sidebar_position: 2
---

# Xavani Tool Gateway（工具网关）

> Note: this integration is not available in current Xavani builds; portal services are disabled.

:::tip 快速开始
Tool Gateway 包含在付费 Xavani Portal 订阅中。**管理订阅 →**
:::

**Tool Gateway** 让已付费的 Xavani Portal 用户通过同一份订阅，直接使用网页搜索、文生图、语音合成（TTS）与浏览器自动化，而**不必**再分别注册 Firecrawl、FAL、OpenAI、Browser Use 等服务的 API Key。
...
上述四类能力均计入 Xavani Portal 订阅计费。你可以按需组合——例如网页与文生图走网关，TTS 仍使用自己的 ElevenLabs Key。

## 资格与账号

Tool Gateway 仅对 **付费** Xavani Portal 订阅开放；免费档不可用——请升级订阅后解锁。
...
在输出中找到 **Xavani Tool Gateway** 小节：会标明哪些工具经订阅网关启用、哪些使用直连 Key、哪些尚未配置。

## 如何启用 Tool Gateway

### 在模型配置流程中

运行 `xavani model` 并选择 Xavani Portal 作为提供商时，Xavani 会主动询问是否启用 Tool Gateway：

```
Your Xavani Portal subscription includes the Tool Gateway.

  The Tool Gateway gives you access to web search, image generation,
  text-to-speech, and browser automation through your Xavani Portal subscription.
...
选择工具类别（Web、Browser、Image Generation、TTS），再将提供商选为 **Xavani Portal subscription**。这会在配置里把对应工具的 `use_gateway` 设为 `true`。

### 手动编辑配置

在 `~/.xavani/config.yaml` 中直接设置 `use_gateway`：
...
当某工具的 `use_gateway: true` 时，运行时会把 API 调用路由到 Xavani Tool Gateway，而不是使用直连 Key：
...
网关使用 Xavani Portal 凭据认证（在 `xavani model` 完成后写入 `~/.xavani/auth.json`）。
...
在 `xavani tools` 中选择非网关提供商时，`use_gateway` 会自动设为 `false`，避免配置自相矛盾。
...
标记为 “active via Xavani Portal subscription” 的即经网关路由；带自有 Key 的会显示当前激活的提供商。

## 进阶：自建网关

若使用自建或自定义网关，可在 `~/.xavani/.env` 中用环境变量覆盖端点：
...
经网关路由的工具会停止工作，直到你续订或通过 `xavani tools` 改回直连 Key。
...
Modal（无服务器终端后端）可作为 Xavani Portal 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `xavani setup terminal` 或在 `config.yaml` 中配置。
