...
description: "Run Xavani Agent directly on an Android phone with Termux"
---

# Xavani on Android with Termux

This is the tested path for running Xavani Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Xavani from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Xavani now ships a Termux-aware installer path:
...
- links `xavani` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `xavani` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/xavani" "$PREFIX/bin/xavani"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `xavani` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.xavani/.env`.
...
### `xavani doctor` says ripgrep or Node is missing
