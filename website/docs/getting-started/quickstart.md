...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Xavani Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Xavani setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Xavani Agent Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Xavani Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Xavani for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Xavani working on my machine | `xavani setup` | Run a real chat and verify it responds |
| I already know my provider | `xavani model` | Save the config, then start chatting |
| I want a bot or always-on setup | `xavani gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `xavani model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `xavani model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Xavani cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Xavani Agent

**Option A — pip (simplest):**

```bash
pip install xavani-agent
xavani postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
The single most important setup step. Use `xavani model` to walk through the choice interactively:
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `xavani model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `xavani model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `xavani model` (requires `copilot` CLI + `copilot login`) |
...
Xavani Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `xavani model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Xavani separates secrets from normal config:

- **Secrets and tokens** → `~/.xavani/.env`
- **Non-secret settings** → `~/.xavani/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
xavani config set model anthropic/claude-opus-4.6
xavani config set terminal.backend docker
xavani config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
xavani            # classic CLI
xavani --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Xavani ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `xavani` vs `xavani --tui`.
:::
...
```bash
xavani --continue    # Resume the most recent session
xavani -c            # Short form
```
...
- `xavani tools` — tune tool access per platform
- `xavani skills` — browse and install reusable workflows
...
```bash
xavani config set terminal.backend docker    # Docker isolation
xavani config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Xavani install directory (the curl installer placed it at
# ~/.xavani/xavani-agent on Linux/macOS or %LOCALAPPDATA%\xavani\xavani-agent on Windows):
...
```bash
xavani skills search kubernetes
...
(If you installed without `[all]`, run `cd ~/.xavani/xavani-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Xavani opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `xavani model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `xavani gateway setup` and check `xavani gateway status` |
| `xavani --continue` can't find old session | Switched profiles or session never saved | Check `xavani sessions list` and confirm you're in the right profile |
...
| `xavani doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `xavani model`
3. `xavani setup`
4. `xavani sessions list`
5. `xavani --continue`
6. `xavani gateway status`
...
| `xavani tools` | Configure which tools are enabled per platform |
| `xavani setup` | Full setup wizard (configures everything at once) |
| `xavani doctor` | Diagnose issues |
| `xavani update` | Update to latest version |
| `xavani gateway` | Start the messaging gateway |
| `xavani --continue` | Resume last session |
