...
title: "Git Worktrees"
description: "Run multiple Xavani agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Xavani Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Xavani so each session has a clean, isolated working directory.

## Why Use Worktrees with Xavani?

Xavani treats the **current working directory** as the project root:

- CLI: the directory where you run `xavani` or `xavani chat`
...
git worktree add ../repo-feature feature/xavani-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/xavani-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Xavani there:

```bash
cd ../repo-feature

# Start Xavani in the worktree
xavani
```
...
git worktree add ../repo-experiment-a feature/xavani-a
git worktree add ../repo-experiment-b feature/xavani-b
```
...
- Works on its own branch (`feature/xavani-a` vs `feature/xavani-b`).
...
- Xavani checkpoint data under `~/.xavani/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Xavani experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/xavani-checkpoints-docs`, `feature/xavani-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Xavani from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `xavani -w` (Automatic Worktree Mode)

Xavani has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `xavani/xavani-<hash>`).
...
```bash
xavani -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `xavani -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Xavani session its own clean checkout.
