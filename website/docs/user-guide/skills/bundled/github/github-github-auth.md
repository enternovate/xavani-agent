...
- Give it a name like "xavani-agent"
- Select scopes:
...
- Give it a title like "xavani-agent-&lt;machine-name>"
...
elif [ -f ~/.xavani/.env ] && grep -q "^GITHUB_TOKEN=" ~/.xavani/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.xavani/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
