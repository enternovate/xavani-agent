...
| | |
|---|---|
| Source | Optional — install with `xavani skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Xavani Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Xavani terminal calls (desktop app flow only)
...
### Service Account (recommended for Xavani)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.xavani/.env` (the skill will prompt for this on first load).
...
## Xavani Execution Pattern (desktop app flow)

Xavani terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/xavani-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/xavani-op.sock"
