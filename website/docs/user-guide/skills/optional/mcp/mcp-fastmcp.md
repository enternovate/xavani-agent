...
| | |
|---|---|
| Source | Optional — install with `xavani skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Xavani or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Xavani. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.xavani/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.xavani/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Xavani integration, either:

- configure the server in `~/.xavani/config.yaml` using the `native-mcp` skill, or
...
### Xavani cannot see the deployed server

The server-building part may be correct while the Xavani config is not. Load the `native-mcp` skill and configure the server in `~/.xavani/config.yaml`, then restart Xavani.
