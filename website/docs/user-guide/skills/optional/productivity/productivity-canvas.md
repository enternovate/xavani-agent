...
| | |
|---|---|
| Source | Optional — install with `xavani skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Xavani Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.xavani/.env`:
...
```bash
CANVAS="python $XAVANI_HOME/skills/productivity/canvas/scripts/canvas_api.py"
