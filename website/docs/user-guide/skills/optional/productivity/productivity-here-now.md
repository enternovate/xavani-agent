...
| | |
|---|---|
| Source | Optional — install with `xavani skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${XAVANI_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${XAVANI_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${XAVANI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client xavani
```
...
```bash
PUBLISH="${XAVANI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client xavani
```
...
```bash
DRIVE="${XAVANI_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `xavani`)    |
