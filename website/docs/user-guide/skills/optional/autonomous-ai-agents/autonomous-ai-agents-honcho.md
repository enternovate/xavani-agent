...
description: "Configure and use Honcho memory with Xavani -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Xavani -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `xavani skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Xavani

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Xavani profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/xavani#running-honcho-locally-with-xavani

### Verify

```bash
xavani honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Xavani profile's AI peer
...
Honcho models conversations as interactions between **peers**. Xavani creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Xavani instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Xavani run |
| `global` | Single session across all directories |

Manual override: `xavani honcho map my-project-name`
...
Each Xavani profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block xavani.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `xavani.coder` host block in `honcho.json`
...
```bash
xavani honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Xavani when Honcho memory is active.
...
Config file: `$XAVANI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `xavani honcho setup`. Ensure `memory.provider: honcho` is in `~/.xavani/config.yaml`.

### Memory not persisting across sessions
Check `xavani honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `xavani profile create <name> --clone`. For existing profiles: `xavani honcho sync`.
...
| Command | Description |
|---------|-------------|
| `xavani honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `xavani honcho status` | Show resolved config, connection test, peer info for active profile |
| `xavani honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `xavani honcho disable` | Disable Honcho for the active profile |
| `xavani honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `xavani honcho peers` | Show peer identities across all profiles |
| `xavani honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `xavani honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `xavani honcho sessions` | List known directory-to-session-name mappings |
| `xavani honcho map <name>` | Map current working directory to a Honcho session name |
| `xavani honcho identity` | Seed AI peer identity or show both peer representations |
| `xavani honcho sync` | Create host blocks for all Xavani profiles that don't have one yet |
| `xavani honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Xavani + Honcho |
| `xavani memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `xavani memory status` | Show active memory provider and config |
| `xavani memory off` | Disable external memory provider |
