...
| | |
|---|---|
| Source | Optional — install with `xavani skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Xavani Agent |
| License | MIT |
...
import upstream classes into Xavani itself.
...
```bash
mkdir -p ~/.xavani/cache/darwinian-evolver && cd ~/.xavani/cache/darwinian-evolver
...
```bash
cd ~/.xavani/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.xavani/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.xavani/skills/research/darwinian-evolver
DE_DIR=~/.xavani/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Xavani core.
   Custom driver scripts under `~/.xavani/skills/...` are user-side and fine.
