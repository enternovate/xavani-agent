...
Xavani Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.xavani/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.xavani/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
Xavani stores session history so it can resume conversations, but it does not
...
Xavani explicitly injects for that turn.
...
For example, if a user sends an image and asks Xavani to make a meme from it,
Xavani may inspect that image once with vision and run an image-processing
...
`xavani sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`xavani` or `xavani chat`) |
...
xavani -c "my project"   # → resumes "my project #3"
```
...
xavani --resume 20250305_091523_a1b2c3d4
xavani -r 20250305_091523_a1b2c3d4

# Resume by title
xavani --resume "refactoring auth"

# Or with the chat subcommand
xavani chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `xavani sessions list`.

### Conversation Recap on Resume

When you resume a session, Xavani displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Xavani session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.xavani/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `xavani -r "<title>"` from the shell) and pick up where the platform left off.
...
Xavani automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `xavani sessions list` or `xavani sessions browse`.
...
```bash
xavani sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Xavani creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`xavani -c "my project"`), it automatically picks the most recent session in the lineage.
...
Xavani provides a full set of session management commands via `xavani sessions`:
...
# Filter by platform
xavani sessions list --source telegram

# Show more sessions
xavani sessions list --limit 50
```
...
xavani sessions export backup.jsonl

# Export sessions from a specific platform
xavani sessions export telegram-history.jsonl --source telegram

# Export a single session
xavani sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
xavani sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
xavani sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
xavani sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
xavani sessions prune

# Custom age threshold
xavani sessions prune --older-than 30

# Only prune sessions from a specific platform
xavani sessions prune --source telegram --older-than 60

# Skip confirmation
xavani sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`xavani insights`](/docs/reference/cli-commands#xavani-insights).
...
When Xavani cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Xavani uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Xavani in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.xavani/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.xavani/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.xavani/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Xavani process in the same `XAVANI_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.xavani/config.yaml`:
...
xavani sessions delete <session_id>
