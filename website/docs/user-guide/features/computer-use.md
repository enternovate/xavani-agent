...
Xavani Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
```
xavani computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `xavani computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `xavani tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
- **`xavani computer-use install --upgrade`** — manual force-refresh.
...
`xavani computer-use status` shows the installed version next to the
binary path.
...
Xavani applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.xavani/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Xavani applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `XAVANI_CUA_DRIVER_VERSION`
...
```
XAVANI_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
XAVANI_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
XAVANI_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`xavani computer-use install` to fetch the cua-driver binary, or run
`xavani tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/enternovate/xavani-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
