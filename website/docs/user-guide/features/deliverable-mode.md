...
When Xavani Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.xavani/config.yaml` under `agent.custom_instructions`.
...
If you use Xavani' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.xavani/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Xavani Agent's deliverable
