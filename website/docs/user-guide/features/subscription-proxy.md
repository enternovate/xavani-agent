...
chat completions — use your Xavani-managed provider subscription as their
...
| Use case | "Use Xavani as a chat backend" | "Use my Portal sub from another app" |
...
Model:      Xavani-4-70B    # or Xavani-4.3-36B, Xavani-4-405B
```
...
INFERENCE_TEXT_MODEL=Xavani-4-70B
```
...
```bash
xavani proxy start --host 0.0.0.0 --port 8645
```
...
`xavani_cli/proxy/adapters/<provider>.py` and registering it in
