...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Xavani Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Xavani Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Xavani](/docs/guides/use-voice-mode-with-xavani).

## Prerequisites

Before using voice features, make sure you have:

1. **Xavani Agent installed** — `pip install xavani-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `xavani model` or set your preferred provider credentials in `~/.xavani/.env`
3. **A working base setup** — run `xavani` to verify the agent responds to text before enabling voice

:::tip
The `~/.xavani/` directory and default `config.yaml` are created automatically the first time you run `xavani`. You only need to create `~/.xavani/.env` manually for API keys.
:::
...
pip install "xavani-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "xavani-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "xavani-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "xavani-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`xavani chat`) and the **TUI** (`xavani --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.xavani/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
xavani                # Start the interactive CLI
```
...
1. Start the CLI with `xavani` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.xavani/config.yaml` (default: `ctrl+b`).
:::
...
```bash
xavani gateway        # Start the gateway (connects to configured platforms)
xavani gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@xavanibyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.xavani/.env`:
```bash
...
```bash
xavani gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.xavani/.env`
...
- Review gateway logs: `tail -f ~/.xavani/logs/gateway.log`
