...
A reference plugin (`example-dashboard` in [`xavani-example-plugins`](https://github.com/enternovate/xavani-agent-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
The [`strike-freedom-cockpit`](https://github.com/enternovate/xavani-agent-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `xavani-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
