...
Type `@` followed by a reference to inject content directly into your message. Xavani expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Xavani env: `$XAVANI_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$XAVANI_HOME/skills/.hub/`
