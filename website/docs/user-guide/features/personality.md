...
If the file is empty, whitespace-only, or cannot be read, Xavani falls back to a built-in default identity ("You are Xavani Agent, an intelligent AI assistant created by Enternovate..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
