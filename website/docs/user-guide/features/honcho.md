...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Xavani's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Xavani instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
xavani memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.xavani/.env
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$XAVANI_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `xavani` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Xavani syncs them back at session init.
...
The `xavani honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `xavani memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
xavani honcho status          # Connection status, config, and key settings
xavani honcho setup           # Redirects to `xavani memory setup`
xavani honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
xavani honcho peer            # Show or update peer names + dialectic reasoning level
xavani honcho mode            # Show or set recall mode (hybrid/context/tools)
xavani honcho tokens          # Show or set token budget for context and dialectic
xavani honcho identity        # Seed or show the AI peer's Honcho identity
xavani honcho sync            # Sync Honcho config to all existing profiles
xavani honcho peers           # Show peer identities across all profiles
xavani honcho sessions        # List known Honcho session mappings
xavani honcho map             # Map current directory to a Honcho session name
xavani honcho enable          # Enable Honcho for the active profile
xavani honcho disable         # Disable Honcho for the active profile
xavani honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `xavani honcho`

If you previously used the standalone `xavani honcho setup`:
...
No re-login or re-setup needed. Run `xavani memory setup` and select "honcho" — the wizard detects your existing config.
