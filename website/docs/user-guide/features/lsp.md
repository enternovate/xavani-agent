...
Xavani runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Xavani
...
1. Xavani captures a baseline of current diagnostics for the file.
...
…). Xavani auto-detects the binary on PATH or in
`<XAVANI_HOME>/lsp/bin/`.
...
`node_modules` tree — Xavani installs both packages together when you
run `xavani lsp install typescript` or auto-install fires on first
use.

## CLI

```
xavani lsp status          # service state + per-server install status
xavani lsp list            # registry, optionally --installed-only
xavani lsp install <id>    # eagerly install one server
xavani lsp install-all     # try every server with a known recipe
xavani lsp restart         # tear down running clients
xavani lsp which <id>      # print resolved binary path
```

`xavani lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <XAVANI_HOME>/lsp/bin
...
When `install_strategy: auto`, Xavani installs binaries into
`<XAVANI_HOME>/lsp/bin/`. NPM packages land in
`<XAVANI_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Xavani-owned and is
...
Servers are kept alive for the life of the Xavani process. There's
...
**`xavani lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<XAVANI_HOME>/lsp/bin/`. Run
`xavani lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `xavani lsp status`**
...
When `xavani lsp status` shows a `Backend warnings` section, install
...
`~/.xavani/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.xavani/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `xavani lsp restart` to clear the set;
