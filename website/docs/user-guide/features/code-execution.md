...
The `execute_code` tool lets the agent write Python scripts that call Xavani tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Xavani over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from xavani_tools import ...`
2. Xavani generates a `xavani_tools.py` stub module with RPC functions
3. Xavani opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Xavani
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from xavani_tools import web_search, web_extract
...
```python
from xavani_tools import search_files, read_file
import json
...
```python
from xavani_tools import search_files, read_file, patch
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.xavani/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Xavani's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Xavani's own python) |
...
**Rule of thumb:** Use `execute_code` when you need to call Xavani tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.
