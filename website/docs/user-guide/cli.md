...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Xavani Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Xavani Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
Xavani also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `xavani --tui` — see the [TUI](tui.md) guide.
:::
...
xavani -s xavani-agent-dev,github-auth
xavani chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
xavani --continue             # Resume the most recent CLI session (-c)
xavani --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
xavani chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
xavani -w                         # Interactive mode in worktree
xavani -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Xavani CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Xavani CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `XAVANI_YOLO_MODE` is on (either `xavani --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`xavani -c` or `xavani --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Xavani to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Xavani supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Xavani replies |
...
```bash
xavani -s xavani-agent-dev,github-auth
xavani chat -s github-pr-workflow -s github-auth
```

Xavani loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.xavani/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.xavani/config.yaml`:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Xavani recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Xavani — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Xavani is working, Xavani prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Xavani to the background — just like any terminal process. The shell prints a confirmation:

```
Xavani Agent has been suspended. Run `fg` to bring Xavani Agent back.
```
...
Resume this session with:
  xavani --resume 20260225_143052_a1b2c3
...
```bash
xavani --continue                          # Resume the most recent CLI session
xavani -c                                  # Short form
xavani -c "my project"                     # Resume a named session (latest in lineage)
xavani --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
xavani --resume "refactoring auth"         # Resume by title
xavani -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `xavani sessions rename <id> <title>` from the command line. Use `xavani sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Xavani's SQLite state database under `~/.xavani/state.db`. The database keeps:
...
Xavani immediately confirms the task and gives you back the prompt:
