...
sidebar_position: 4
title: "Slack"
description: "Set up Xavani Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Xavani Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Xavani instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Xavani uses the modern
...
The fastest path is to paste a manifest Xavani generates for you. It
...
### Option A: From a Xavani-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   xavani slack manifest --write
   ```
   This writes `~/.xavani/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Xavani Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Xavani can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `xavani-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Xavani configuration issue.
:::
...
Xavani uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
xavani gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
xavani gateway              # Foreground
xavani gateway install      # Install as a user service
sudo xavani gateway install --system   # Linux only: boot-time system service
```
...
Every Xavani command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Xavani command with its description.

Under the hood: Xavani ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/enternovate/xavani-agent/blob/main/xavani_cli/commands.py)
...
When Xavani adds new commands (e.g. after `xavani update`), regenerate
...
3. Paste the new contents of `~/.xavani/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/xavani <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/xavani btw run the tests` — Xavani routes it the same way as `/btw
run the tests`. Free-form questions also work: `/xavani what's the
...
Slack never delivers them to Xavani.

As a workaround, Xavani recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Xavani treats it identically to the slash form and replies in the same
thread.
...
```bash
xavani slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Xavani behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Xavani Agent what time is it?`). In channels, Xavani replies in a thread attached to that message. |
| **Threads** | If you @mention Xavani inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.xavani/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Xavani in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Xavani will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Xavani will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Xavani Agent`).

---

## Multi-Workspace Support

Xavani can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Xavani also loads tokens from an **OAuth token file** at:

```
~/.xavani/slack_tokens.json
```
...
- When a message arrives, Xavani uses the correct workspace-specific client to respond.
...
Xavani supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Xavani Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Xavani Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Xavani now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Xavani Agent`)
...
- Tokens should be stored in `~/.xavani/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Xavani config directory
