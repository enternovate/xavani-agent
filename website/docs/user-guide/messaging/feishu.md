...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Xavani Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Xavani Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Xavani opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Xavani responds to every message. |
| Group chats | Xavani responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Xavani will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `xavani gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Xavani runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Xavani behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Xavani starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Xavani responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Xavani responds to @mentions from any user in any group. |
| `allowlist` | Xavani only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Xavani ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Xavani read all group traffic without requiring an @mention:
...
Xavani auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Xavani ignores messages sent by other bots. Enable bot-to-bot messaging when you want Xavani to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Xavani. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `xavani update --gateway` needs confirmation, the adapter records the selected answer in Xavani's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Xavani reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.xavani/feishu_comment_rules.json` (pairing grants in `~/.xavani/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
|---------|---------|---------|
| Quiet period | `XAVANI_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `XAVANI_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `XAVANI_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `XAVANI_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.xavani/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `XAVANI_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `XAVANI_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `XAVANI_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `XAVANI_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `XAVANI_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `XAVANI_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `xavani gateway setup` |
| `Another local Xavani gateway is already using this Feishu app_id` | Only one Xavani instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Xavani can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
...
Feishu / Lark uses the `xavani-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
