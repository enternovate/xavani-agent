...
description: "Chat with Xavani from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Xavani from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Xavani](/docs/guides/use-voice-mode-with-xavani).
...
```mermaid
flowchart TB
    subgraph Gateway["Xavani Gateway"]
...
```bash
xavani gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
xavani gateway              # Run in foreground
xavani gateway setup        # Configure messaging platforms interactively
xavani gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo xavani gateway install --system   # Linux only: install a boot-time system service
xavani gateway start        # Start the default service
xavani gateway stop         # Stop the default service
xavani gateway status       # Check default service status
xavani gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Xavani Agent to the latest version |
...
Configure per-platform overrides in `~/.xavani/gateway.json`:
...
xavani pairing approve telegram XKGH5N7P

# Other pairing commands:
xavani pairing list          # View pending + approved users
xavani pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Xavani appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.xavani/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.xavani/config.yaml`:
...
```bash
XAVANI_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
xavani gateway install               # Install as user service
xavani gateway start                 # Start the service
xavani gateway stop                  # Stop the service
xavani gateway status                # Check status
journalctl --user -u xavani-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo xavani gateway install --system
sudo xavani gateway start --system
sudo xavani gateway status --system
journalctl -u xavani-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Xavani will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Xavani installations on the same machine (with different `XAVANI_HOME` directories), each gets its own systemd service name. The default `~/.xavani` uses `xavani-gateway`; other installations use `xavani-gateway-<hash>`. The `xavani gateway` commands automatically target the correct service for your current `XAVANI_HOME`.
:::

### macOS (launchd)

```bash
xavani gateway install               # Install as launchd agent
xavani gateway start                 # Start the service
xavani gateway stop                  # Stop the service
xavani gateway status                # Check status
tail -f ~/.xavani/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.xavani.gateway.plist`. It includes three environment variables:
...
- **XAVANI_HOME** — scopes the gateway to your Xavani installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `xavani gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `XAVANI_HOME` directory gets its own launchd label. The default `~/.xavani` uses `ai.xavani.gateway`; other installations use `ai.xavani.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `xavani-cli` | Full access |
| Telegram | `xavani-telegram` | Full tools including terminal |
| Discord | `xavani-discord` | Full tools including terminal |
| WhatsApp | `xavani-whatsapp` | Full tools including terminal |
| Slack | `xavani-slack` | Full tools including terminal |
| Google Chat | `xavani-google_chat` | Full tools including terminal |
| Signal | `xavani-signal` | Full tools including terminal |
| SMS | `xavani-sms` | Full tools including terminal |
| Email | `xavani-email` | Full tools including terminal |
| Home Assistant | `xavani-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `xavani-mattermost` | Full tools including terminal |
| Matrix | `xavani-matrix` | Full tools including terminal |
| DingTalk | `xavani-dingtalk` | Full tools including terminal |
| Feishu/Lark | `xavani-feishu` | Full tools including terminal |
| WeCom | `xavani-wecom` | Full tools including terminal |
| WeCom Callback | `xavani-wecom-callback` | Full tools including terminal |
| Weixin | `xavani-weixin` | Full tools including terminal |
| BlueBubbles | `xavani-bluebubbles` | Full tools including terminal |
| QQBot | `xavani-qqbot` | Full tools including terminal |
| Yuanbao | `xavani-yuanbao` | Full tools including terminal |
| Microsoft Teams | `xavani-teams` | Full tools including terminal |
| API Server | `xavani-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `xavani-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.xavani/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
