...
Connect Xavani to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Xavani supports two WeCom integration modes:
...
3. Xavani decrypts the message, queues it for the agent
...
(Use `xavani gateway start` only after `xavani gateway install` has registered the systemd/launchd service.)
