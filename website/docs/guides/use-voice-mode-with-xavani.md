---
sidebar_position: 8
title: "Use Voice Mode with Xavani"
description: "A practical guide to setting up and using Xavani voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Xavani
...
- you want Xavani sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Xavani.
...
## Step 1: make sure normal Xavani works first
...
Xavani supports both local and cloud speech stacks.
...
### If you use `xavani setup`

If you choose NeuTTS in the setup wizard, Xavani checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Xavani transcribes and responds
...
- asking Xavani to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Xavani loop.

## Tuning CLI behavior

### Silence threshold

If Xavani starts/stops too aggressively, tune:
...
Xavani stays a normal chat bot, but can speak replies.
...
- you want Xavani to function like a portable research or ops assistant
...
Xavani joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Xavani detects speech boundaries
- transcripts are posted in the associated text channel
- Xavani responds in text and audio
...
1. get text Xavani working
2. install `xavani-agent[voice]`
