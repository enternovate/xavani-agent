...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `xavani send`."
---

# Pipe Script Output to Messaging Platforms

`xavani send` is a small, scriptable CLI that pushes a message to any
messaging platform Xavani is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | xavani send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `xavani
...
xavani send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | xavani send --to telegram:-1001234567890

# Send a file
xavani send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
xavani send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
xavani send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
xavani send --list

# Filter by platform
xavani send --list telegram
```
...
Any platform Xavani ships adapters for works as a target:
...
`xavani send` resolves the message body in this order:

1. **Positional argument** — `xavani send --to telegram "hi"`
2. **`--file PATH`** — `xavani send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | xavani send --to telegram`

When stdin is a TTY (no pipe), Xavani does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  xavani send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `xavani send` reuses your Xavani config, the same script works on
any host where Xavani is installed — no need to export bot tokens into
...
`xavani send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  xavani send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | xavani send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.xavani/bin/xavani send \
...
```bash
./train.py --epochs 200 && \
  xavani send --to telegram "training done" || \
  xavani send --to telegram "training failed (exit $?)"
```
...
xavani send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(xavani send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `xavani send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `xavani send` calls
the platform's REST endpoint directly using credentials from
`~/.xavani/.env` and `~/.xavani/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `xavani gateway start` and retry.
...
# Machine-readable
xavani send --list --json
```

The listing is built from `~/.xavani/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`xavani gateway
...
| Approach | Multi-platform | Reuses Xavani creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `xavani send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`xavani send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `xavani send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `xavani send` shares with cron delivery.
