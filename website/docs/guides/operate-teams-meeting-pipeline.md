...
```bash
xavani teams-pipeline validate
```
...
```bash
xavani teams-pipeline token-health
xavani teams-pipeline token-health --force-refresh
```
...
```bash
xavani teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
xavani teams-pipeline maintain-subscriptions
xavani teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Xavani cron (recommended if you already run the Xavani gateway)

Xavani ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.xavani/scripts/`. First create the script:

```bash
mkdir -p ~/.xavani/scripts
cat > ~/.xavani/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec xavani teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.xavani/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
xavani cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/xavani-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Xavani Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=xavani
EnvironmentFile=/etc/xavani/env
ExecStart=/usr/local/bin/xavani teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/xavani-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Xavani Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now xavani-teams-pipeline-maintain.timer
systemctl list-timers xavani-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/xavani teams-pipeline maintain-subscriptions >> /var/log/xavani/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.xavani/.env` at the top of a wrapper script that crontab calls.
...
```bash
xavani teams-pipeline subscriptions   # should show expirationDateTime advanced
xavani teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
xavani teams-pipeline list
xavani teams-pipeline list --status failed
xavani teams-pipeline show <job-id>
```

### Replay a stored job

```bash
xavani teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
xavani teams-pipeline fetch --meeting-id <meeting-id>
xavani teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `xavani teams-pipeline maintain-subscriptions --dry-run`
- inspect `xavani teams-pipeline list --status failed`
...
- run `xavani teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `xavani teams-pipeline run`
...
- [ ] `xavani teams-pipeline validate` returns an OK snapshot
- [ ] `xavani teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Xavani cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
