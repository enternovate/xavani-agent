---
sidebar_position: 7
title: "Use SOUL.md with Xavani"
description: "How to use SOUL.md to shape Xavani Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Xavani

`SOUL.md` is the **primary identity** for your Xavani instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Xavani to feel like the same assistant every time you talk to it — or if you want to replace the Xavani persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Xavani should be
- what Xavani should avoid stylistically
- how Xavani should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Xavani is and how Xavani speaks
...
Xavani now uses only the global SOUL file for the current instance:

```text
~/.xavani/SOUL.md
```

If you run Xavani with a custom home directory, it becomes:
...
Xavani automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Xavani does not overwrite it
- if the file exists but is empty, Xavani adds nothing from it to the prompt

## How Xavani uses it

When Xavani starts a session, it reads `SOUL.md` from `XAVANI_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Xavani feels.
...
Xavani already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Xavani should sound.

# Avoid
What Xavani should not do.

# Defaults
How Xavani should behave when ambiguity appears.
```
...
Then restart Xavani or start a new session.
...
### I edited SOUL.md but Xavani still sounds the same

Check:
- you edited `~/.xavani/SOUL.md` or `$XAVANI_HOME/SOUL.md`
...
### Xavani is ignoring parts of my SOUL.md
