---
sidebar_position: 6
title: "Use MCP with Xavani"
description: "A practical guide to connecting MCP servers to Xavani Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Xavani

This guide shows how to actually use MCP with Xavani Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Xavani tool
- you want Xavani to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Xavani to internal APIs, databases, or company systems without modifying Xavani core

Do not use MCP when:
- a built-in Xavani tool already solves the job well
...
- Xavani remains the agent
- MCP servers contribute tools
- Xavani discovers those tools at startup or reload time
...
If you installed Xavani with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Xavani banner/status should show MCP integration when configured
- ask Xavani what tools it has available
...
## WSL2: bridge Xavani in WSL to Windows Chrome

This is the practical setup when:

- Xavani runs inside WSL2
...
In this setup, Xavani does **not** connect to Chrome directly. Instead:

- Xavani runs in WSL
- Xavani starts a local stdio MCP server
...
```text
Xavani (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Xavani stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Xavani core browser transport
...
```bash
xavani mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
xavani mcp test chrome-devtools-win
```

Then start a fresh Xavani session or run:
...
Once loaded, Xavani can use the MCP-prefixed browser tools directly. For example:
...
If Xavani runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Xavani from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Xavani from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Xavani:
...
2. Xavani-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Xavani will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Xavani to reason over a bounded workspace.
...
Now Xavani can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Xavani core.
...
Because Xavani now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
