...
Copy-paste recipes for common automation patterns. Each template uses Xavani's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`xavani webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `xavani webhook subscribe`) |
...
```bash
xavani webhook subscribe github-pr-review \
  --events "pull_request" \
...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (xavani_cli/commands.py, xavani_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (xavani_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
xavani cron create "0 6 * * *" \
  "Run a dependency security audit on the xavani-agent project.

1. cd ~/.xavani/xavani-agent && source .venv/bin/activate
...
```bash
xavani webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
xavani webhook subscribe alert-triage \
...
```python title="~/.xavani/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Xavani-Monitor/1.0"})
...
xavani cron create "every 30m" \
  ...
  --script ~/.xavani/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
xavani cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Xavani Agent development." \
...
```bash
xavani webhook subscribe github-issues \
  --events "issues" \
...
```bash
xavani webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
xavani webhook subscribe stripe-payments \
...
```bash
xavani cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the xavani-agent codebase.
