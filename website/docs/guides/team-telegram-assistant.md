...
This tutorial walks you through setting up a Telegram bot powered by Xavani Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Xavani Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.xavani/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Xavani itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Xavani Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_xavani_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Xavani Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Xavani Gateway...
...
# View live logs
journalctl --user -u xavani-gateway -f
...
xavani gateway start
xavani gateway stop
tail -f ~/.xavani/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `xavani gateway install` to update the plist.
:::
...
```bash
xavani gateway stop && xavani gateway start
```
...
xavani pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.xavani/.env`:
...
Control how much detail the bot shows when using tools. In `~/.xavani/config.yaml`:
...
Customize how the bot communicates by editing `~/.xavani/SOUL.md`:

For a full guide, see [Use SOUL.md with Xavani](/docs/guides/use-soul-with-xavani).
...
```bash
# From the CLI
xavani cron list          # View all scheduled jobs
xavani cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u xavani-gateway` (Linux) or `~/.xavani/logs/gateway.log` (macOS) |
| Cron job output | `~/.xavani/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.xavani/cron/jobs.json` |
| Pairing data | `~/.xavani/pairing/` |
| Session history | `~/.xavani/sessions/` |
