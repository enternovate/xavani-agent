...
description: "Sign in with your SuperGrok subscription to use Grok models in Xavani Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok Subscription)

Xavani Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using your existing **SuperGrok subscription**. No `XAI_API_KEY` is required — log in once and Xavani automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Xavani — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Xavani opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.xavani/auth.json` and refreshed automatically before they expire.
...
```bash
xavani auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, Xavani detects the remote environment and prints the authorization URL instead of opening a browser.
...
xavani auth add xai-oauth --no-browser
...
If you don't have a regular SSH client (e.g. you're running Xavani inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — Xavani skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
xavani auth add xai-oauth --manual-paste
# Or via the model picker:
xavani model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/enternovate/xavani-agent/issues/26923).

## How the Login Works

1. Xavani opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Xavani and the tokens are saved to `~/.xavani/auth.json`.
4. From then on, Xavani refreshes the access token in the background — you stay signed in until you `xavani auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
xavani config set model.default grok-4.3
xavani config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.xavani/config.yaml` will contain:
...
```bash
xavani --provider xai-oauth        # canonical
xavani --provider grok-oauth       # alias
xavani --provider x-ai-oauth       # alias
xavani --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `xavani tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `xavani tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
| `XAVANI_INFERENCE_PROVIDER` | Force the active provider at runtime, e.g. `XAVANI_INFERENCE_PROVIDER=xai-oauth xavani`. |

## Troubleshooting

### Token expired — not re-logging in automatically

Xavani refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Xavani surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Xavani marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `xavani auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Xavani raises a timeout error.

**Fix:** re-run `xavani auth add xai-oauth` (or `xavani model`). The flow starts fresh.

### State mismatch (possible CSRF)

Xavani detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Xavani prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
xavani config set model.provider xai
```
...
**Fix:** run `xavani model` and pick the xAI Grok OAuth provider, or run `xavani auth add xai-oauth`.
...
```bash
xavani auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `xavani auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `xavani auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Xavani is on a different machine than your browser
