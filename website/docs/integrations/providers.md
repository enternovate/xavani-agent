...
This page covers setting up inference providers for Xavani Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Xavani.

## Inference Providers

You need at least one way to connect to an LLM. Use `xavani model` to switch providers and models interactively, or configure directly:
...
| **Anthropic** | `xavani model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.xavani/.env` |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.xavani/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.xavani/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.xavani/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.xavani/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.xavani/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.xavani/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.xavani/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.xavani/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.xavani/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.xavani/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `xavani model` → "xAI Grok OAuth (SuperGrok Subscription)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.xavani/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.xavani/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.xavani/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.xavani/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.xavani/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.xavani/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.xavani/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.xavani/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.xavani/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `xavani model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **LM Studio** | `xavani model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `xavani model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
# → done — Xavani auto-provisions your free tier on first request
```

Xavani ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.xavani/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `XAVANI_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Xavani works unchanged
...
| Workspace / Standard / Enterprise account | Set `XAVANI_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Xavani detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Xavani shows
...
```bash
XAVANI_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
XAVANI_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Xavani stores the resulting credentials in its own auth store under `~/.xavani/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Xavani marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `xavani auth add codex-oauth` (or `xavani model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::
...
Xavani has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`xavani model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Xavani chat session | Quick switch between **already-configured** providers and models |
...
Subscription-based access to Xavani-4 models (`Xavani-4-70B`, `Xavani-4.3-36B`, `Xavani-4-405B`) via Nous Research's portal. Run `xavani model`, pick **Nous Portal**, sign in through the browser — Xavani stores a long-lived refresh token at `~/.xavani/auth.json`.
...
When you authenticate via `xavani model` → Anthropic OAuth (or via `xavani auth add anthropic --type oauth`), Xavani routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Xavani — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
xavani chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `xavani model`
# Xavani will use Claude Code's credential store directly when available
xavani model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
xavani chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
xavani chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `xavani model`, Xavani prefers Claude Code's own credential store over copying the token into `~/.xavani/.env`. That keeps refreshable Claude credentials refreshable.
...
Xavani supports GitHub Copilot as a first-class provider with two modes:
...
```bash
xavani chat --provider copilot --model gpt-5.4
```
...
If no token is found, `xavani model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `xavani model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `xavani model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Xavani
Xavani sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Xavani now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Xavani therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
xavani chat --provider copilot-acp --model copilot-acp
...
| `XAVANI_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `XAVANI_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# NovitaAI Model API
xavani chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.xavani/.env

# z.ai / ZhipuAI GLM
xavani chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.xavani/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
xavani chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.xavani/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
xavani chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.xavani/.env

# MiniMax (global endpoint)
xavani chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.xavani/.env

# MiniMax (China endpoint)
xavani chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.xavani/.env

# Qwen Cloud / DashScope (Qwen models)
xavani chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.xavani/.env

# Xiaomi MiMo
xavani chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.xavani/.env

# Tencent TokenHub (Hy3 Preview)
xavani chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.xavani/.env

# Arcee AI (Trinity models)
xavani chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.xavani/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
xavani chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.xavani/.env
```
...
When using the Z.AI / GLM provider, Xavani automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.xavani/.env` and pick xAI in `xavani model`, or drop `grok` as a shortcut into `/model grok-4-1-fast-reasoning`.

SuperGrok subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok Subscription)** in `xavani model`, or run `xavani auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Xavani runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Xavani automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `xavani tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.
...
# Short alias
xavani chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `xavani model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Xavani auto-discovers the available models.
...
xavani chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 xavani chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `xavani model`, sign in through the browser, and Xavani persists the refresh token.

```bash
xavani model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.xavani/auth.json

xavani chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `XAVANI_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Xavani exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
xavani chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `xavani model`, sign in through the browser, and Xavani persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
xavani chat   # uses api.minimax.io/anthropic endpoint
```
...
xavani chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.xavani/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 xavani chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
Xavani automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
xavani chat --provider gmi --model deepseek-ai/DeepSeek-R1
# Requires: GMI_API_KEY in ~/.xavani/.env
```
...
```bash
# StepFun
xavani chat --provider stepfun --model step-3-mini
# Requires: STEPFUN_API_KEY in ~/.xavani/.env
```
...
```bash
# Use any available model
xavani chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.xavani/.env

# Short alias
xavani chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Xavani Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Xavani at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of Xavani — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `xavani setup` or config migration. Use `xavani model` or edit `config.yaml` directly.
:::
...
:::warning xavani model vs /model
**`xavani model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Xavani chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `xavani model`, set up the new provider, then start a new session.
:::
...
When switching providers, Xavani persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Xavani.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Xavani) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `xavani` (Qwen 2.5, Xavani 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure Xavani to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Xavani won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Xavani 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Xavani will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Xavani) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Xavani Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Xavani config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Xavani config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Xavani.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Xavani inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser xavani` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Xavani's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Xavani thinks the context is
...
Xavani auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Xavani uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Xavani with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/user-guide/configuration#context-compression) in Xavani.
...
Then configure Xavani with `xavani model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Xavani with `xavani model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `xavani model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Xavani uses this to decide when to compress history and to validate API requests.
...
Xavani uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`xavani model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Xavani uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `xavani model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `xavani model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
You can also select named custom providers from the interactive `xavani model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.xavani/config.yaml` and the matching API key goes in `~/.xavani/.env`.
...
Together's `/v1/models` endpoint works, so `xavani model` can auto-discover available models.
...
:::tip Troubleshooting
- `xavani doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `xavani model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `xavani model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Xavani at your instance (no API key needed):
   ```bash
   xavani config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.xavani/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.xavani/config.yaml`:
...
Configure a chain of backup providers Xavani tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `xavani fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/docs/user-guide/features/fallback-providers).
:::
