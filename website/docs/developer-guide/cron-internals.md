...
description: "How Xavani stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `xavani_cli/cron.py` | CLI `xavani cron` subcommands |
...
Jobs are stored in `~/.xavani/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `xavani cron` commands are run or during active CLI sessions.
...
```python
# ~/.xavani/scripts/check_competitors.py
...
2. **Environment variable** — `XAVANI_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.xavani/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `xavani cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `xavani cron` CLI provides direct job management:

```bash
xavani cron list                    # Show all jobs
xavani cron create                  # Interactive job creation (alias: add)
xavani cron edit <job_id>           # Edit job configuration
xavani cron pause <job_id>          # Pause a running job
xavani cron resume <job_id>         # Resume a paused job
xavani cron run <job_id>            # Trigger immediate execution
xavani cron remove <job_id>         # Delete a job
```
