...
description: "How to build a memory provider plugin for Xavani Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Xavani Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/docs/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `xavani plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          xavani_home (str): Active XAVANI_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `xavani memory setup` | **Yes** |
| `save_config(values, xavani_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `xavani memory setup`:
...
Every field in `get_config_schema()` is prompted during `xavani memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$XAVANI_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, xavani_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(xavani_home) / "my-provider.json"
...
All storage paths **must** use the `xavani_home` kwarg from `initialize()`, not hardcoded `~/.xavani`:

```python
# CORRECT — profile-scoped
from xavani_constants import get_xavani_home
data_dir = get_xavani_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.xavani/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `xavani my-provider status`, `xavani my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `xavani <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `xavani --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: xavani my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the xavani my-provider argparse tree.
