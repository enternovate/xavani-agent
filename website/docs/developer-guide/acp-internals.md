...
The ACP adapter wraps Xavani' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
xavani acp / xavani-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.xavani/.env
  -> configure stderr logging
  -> construct XavaniACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'xavani-agent[acp]==<version>' xavani-acp`, pointed at the `xavani-agent` PyPI release.
...
- `allow_once` -> Xavani `once`
- `allow_always` -> Xavani `always`
- reject options -> Xavani `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Xavani tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["xavani-acp"])
...
Instead it reuses Xavani' runtime resolver:

- `acp_adapter/auth.py`
- `xavani_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Xavani provider/credentials. It also always advertises a terminal setup auth method (`xavani-setup`, args `--setup`) so first-run registry clients can open Xavani' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.xavani/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `xavani-acp` toolset definition
- `xavani_cli/main.py` — `xavani acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `xavani-acp` script
