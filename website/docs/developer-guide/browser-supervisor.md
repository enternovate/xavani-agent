...
[PR #12550](https://github.com/enternovate/xavani-agent/pull/12550) proposed a
...
(`xavani-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Xavani `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `xavani-acp`, `xavani-api-server`, core toolsets (gated on CDP reachability)
...
- `xavani_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
