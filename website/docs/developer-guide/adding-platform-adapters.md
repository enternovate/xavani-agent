...
This guide covers adding a new messaging platform to the Xavani gateway. A platform adapter connects Xavani to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.xavani/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Xavani code. Your plugin is a directory with two files:

```
~/.xavani/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `xavani config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-xavani-config) below).
...
def register(ctx):
    """Plugin entry point — called by the Xavani plugin system."""
...
| `xavani config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `xavani status` | Shows plugin platforms with `(plugin)` tag |
| `xavani gateway setup` | Plugin platforms appear in setup menu |
| `xavani tools` / `xavani skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.xavani/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `xavani gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `xavani cron run` separate from `xavani gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `xavani config`

`xavani_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for Xavani Agent.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #xavani)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the Xavani core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`xavani_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`xavani_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`xavani_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`xavani_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`xavani_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`xavani_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"xavani-newplat"` toolset definition with `_XAVANI_CORE_TOOLS`
2. **`toolsets.py`** — Add `"xavani-newplat"` to the `"xavani-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | xavani-newplat toolset |
