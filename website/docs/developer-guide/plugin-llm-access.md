...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Xavani already has applies. The plugin never sees a
...
[`plugin-llm-async-example`](https://github.com/enternovate/xavani-agent-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Xavani provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Xavani provider
        # (must be one Xavani already knows about — same names as
        # `xavani model` and config.yaml model.provider).
        ...
        # plugin sends — Xavani does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.xavani/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Xavani' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Xavani subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/enternovate/xavani-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/enternovate/xavani-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/enternovate/xavani-agent-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/enternovate/xavani-agent-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
