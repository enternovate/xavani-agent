...
def _restore_tool_modules():
    original_xavani_home = os.environ.get("XAVANI_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_xavani_home is None:
            os.environ.pop("XAVANI_HOME", None)
        else:
            os.environ["XAVANI_HOME"] = original_xavani_home
        _reset_modules(("tools", "xavani_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    xavani_cli = types.ModuleType("xavani_cli")
    xavani_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["xavani_cli"] = xavani_cli
    xavani_home = tmp_path / "xavani-home"
    os.environ["XAVANI_HOME"] = str(xavani_home)
    sys.modules["xavani_cli.config"] = types.SimpleNamespace(
        get_xavani_home=lambda: xavani_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="xavani-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": xavani_home / "modal_snapshots.json",
