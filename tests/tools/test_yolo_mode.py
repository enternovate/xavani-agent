...
"""Tests for --yolo (XAVANI_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("XAVANI_INTERACTIVE", "1")
        monkeypatch.setenv("XAVANI_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With XAVANI_YOLO_MODE, dangerous (non-hardline) commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """XAVANI_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("XAVANI_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for XAVANI_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("XAVANI_YOLO_MODE", "")
        monkeypatch.setenv("XAVANI_INTERACTIVE", "1")
        monkeypatch.setenv("XAVANI_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("XAVANI_YOLO_MODE") returns ""
    ...
    def test_false_like_yolo_values_do_not_bypass_dangerous_command(self, monkeypatch, value):
        ...
        monkeypatch.setenv("XAVANI_YOLO_MODE", value)
        monkeypatch.setenv("XAVANI_INTERACTIVE", "1")
        monkeypatch.setenv("XAVANI_SESSION_KEY", "test-session")
