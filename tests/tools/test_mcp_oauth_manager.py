...
def test_xavani_provider_subclass_exists():
    """XavaniMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _XAVANI_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _XAVANI_PROVIDER_CLS is not None
    assert issubclass(_XAVANI_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_xavani_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns XavaniMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _XAVANI_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("XAVANI_HOME", str(tmp_path))
    ...
    assert _XAVANI_PROVIDER_CLS is not None
    assert isinstance(provider, _XAVANI_PROVIDER_CLS)
    assert provider._xavani_server_name == "srv"
