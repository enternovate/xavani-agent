...
def _clean_state():
    ...
    saved = {}
    for k in ("XAVANI_INTERACTIVE", "XAVANI_GATEWAY_SESSION", "XAVANI_EXEC_ASK", "XAVANI_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["XAVANI_INTERACTIVE"] = "1"
        session_key = os.getenv("XAVANI_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No XAVANI_INTERACTIVE or XAVANI_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
