...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._xavani_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_xavani_bin(self):
        """After _INSTALL_FAILED, manual install in XAVANI_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        xavani_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(xavani_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(xavani_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._xavani_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == xavani_bin
            assert _tirith_mod._resolved_path == xavani_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# XAVANI_HOME isolation
# ---------------------------------------------------------------------------

class TestXavaniHomeIsolation:
    def test_xavani_bin_dir_respects_xavani_home(self):
        """_xavani_bin_dir must use XAVANI_HOME, not hardcoded ~/.xavani."""
        from tools.tirith_security import _xavani_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"XAVANI_HOME": tmpdir}):
            result = _xavani_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_xavani_home(self):
        """_failure_marker_path must use XAVANI_HOME, not hardcoded ~/.xavani."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"XAVANI_HOME": "/custom/xavani"}):
            result = _failure_marker_path()
        assert result == "/custom/xavani/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets XAVANI_HOME; verify it's active."""
        xavani_home = os.getenv("XAVANI_HOME")
        assert xavani_home is not None, "XAVANI_HOME should be set by conftest"
        assert "xavani_test" in xavani_home, "Should point to test temp dir"

    def test_get_xavani_home_fallback(self):
        """Without XAVANI_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_xavani_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove XAVANI_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".xavani")
            result = _get_xavani_home()
