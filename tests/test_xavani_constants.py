...
"""Tests for xavani_constants module."""
...
from xavani_constants import (
    ...
    get_default_xavani_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultXavaniRoot:
    """Tests for get_default_xavani_root() — Docker/custom deployment awareness."""

    def test_no_xavani_home_returns_native(self, tmp_path, monkeypatch):
        """When XAVANI_HOME is not set, returns ~/.xavani."""
        monkeypatch.delenv("XAVANI_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_xavani_root() == tmp_path / ".xavani"

    def test_xavani_home_is_native(self, tmp_path, monkeypatch):
        """When XAVANI_HOME = ~/.xavani, returns ~/.xavani."""
        native = tmp_path / ".xavani"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(native))
        assert get_default_xavani_root() == native

    def test_xavani_home_is_profile(self, tmp_path, monkeypatch):
        """When XAVANI_HOME is a profile under ~/.xavani, returns ~/.xavani."""
        native = tmp_path / ".xavani"
    ...
    def test_xavani_home_is_docker(self, tmp_path, monkeypatch):
        """When XAVANI_HOME points outside ~/.xavani (Docker), returns XAVANI_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(docker_home))
        assert get_default_xavani_root() == docker_home

    def test_xavani_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any XAVANI_HOME outside ~/.xavani is treated as the root."""
        custom = tmp_path / "my-xavani-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(custom))
        assert get_default_xavani_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (XAVANI_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(profile))
        assert get_default_xavani_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(xavani_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(xavani_constants, "_container_detected", True)
        assert is_container() is True
