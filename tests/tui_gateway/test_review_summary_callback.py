...
def server():
    with patch.dict(
        "sys.modules",
        {
            "xavani_constants": MagicMock(
                get_xavani_home=MagicMock(return_value="/tmp/xavani_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'xavani-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'xavani-release' patched"
    }
