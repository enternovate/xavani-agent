...
def _isolate(tmp_path, monkeypatch):
    """Isolate XAVANI_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal XavaniCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = XavaniCLI.__new__(XavaniCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["xavani-core"]
        obj.compact = False
