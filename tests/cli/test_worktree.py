...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("xavani/xavani-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned xavani/* and pr-* branches."""

    def test_prunes_orphaned_xavani_branch(self, git_repo):
        """xavani/xavani-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "xavani/xavani-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "xavani/xavani-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "xavani/xavani-deadbeef" in result.stdout
        ...
        assert "xavani/xavani-deadbeef" in orphaned
        ...
        assert "xavani/xavani-deadbeef" not in result.stdout
