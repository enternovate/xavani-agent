...
Note: Tests that import ``xavani_cli.auth`` or ``xavani_cli.runtime_provider``
...
class TestModelCatalog:
    ...
    def test_bedrock_models_include_nova(self):
        from xavani_cli.models import _PROVIDER_MODELS
        models = _PROVIDER_MODELS.get("bedrock", [])
        nova_models = [m for m in models if "nova" in m.lower()]
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("xavani_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_resolution(self, monkeypatch):
        ...
        with patch("xavani_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("xavani_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}):
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("xavani_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("xavani_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("xavani_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
...
class TestProvidersModule:
    """Verify bedrock is wired into xavani_cli/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from xavani_cli.providers import ALIASES
    ...
    def test_bedrock_transport_mapping(self):
        from xavani_cli.providers import TRANSPORT_TO_API_MODE
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "xavani-agent[bedrock]" not in extras["all"]
