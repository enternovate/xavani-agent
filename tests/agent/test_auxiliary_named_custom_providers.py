...
def _isolate(tmp_path, monkeypatch):
    """Redirect XAVANI_HOME and clear module caches."""
...
def _write_config(tmp_path, config_dict):
    """Write a config.yaml to the test XAVANI_HOME."""
    import yaml
...
class TestResolveProviderClientMainAlias:
    ...
    def test_main_resolves_to_named_custom_provider(self, tmp_path):
        ...
        assert urllib.parse.urlparse(str(client.base_url)).hostname and urllib.parse.urlparse(str(client.base_url)).hostname.lower() == "beans.local"
    ...
    def test_main_resolves_github_copilot_alias(self, tmp_path):
        ...
        with (
            patch("xavani_cli.auth.resolve_api_key_provider_credentials", return_value={
...
class TestCustomProviderAliasCollision:
    ...
    *alias* (not a canonical provider) must win over the built-in.
    ...
    def test_custom_named_kimi_wins_over_builtin_alias(self, tmp_path):
        ...
        assert urllib.parse.urlparse(str(client.base_url)).hostname and urllib.parse.urlparse(str(client.base_url)).hostname.lower() == "my-custom-kimi.example.com"
    ...
    def test_explicit_overrides_applied_on_api_key_branch(self, tmp_path, monkeypatch):
        ...
        assert isinstance(client, OpenAI)
        assert urllib.parse.urlparse(str(client.base_url)).hostname and urllib.parse.urlparse(str(client.base_url)).hostname.lower() == "override.example.com"
