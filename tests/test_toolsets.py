...
class TestToolsetConsistency:
    ...
    def test_xavani_platforms_share_core_tools(self):
        """All xavani-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on xavani-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["xavani-cli", "xavani-telegram", "xavani-discord", "xavani-whatsapp", "xavani-slack", "xavani-signal", "xavani-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_xavani_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("xavani-whatsapp")

    def test_xavani_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("xavani-api-server")
