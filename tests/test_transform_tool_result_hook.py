...
def _run_handle_function_call(
    ...
    *,
    ...
    if invoke_hook is not _UNSET:
        ...
        monkeypatch.setattr("xavani_cli.plugins.invoke_hook", invoke_hook)
...
def test_result_unchanged_when_no_hook_registered(monkeypatch):
    ...
    monkeypatch.setenv("XAVANI_HOME", "/tmp/xavani_no_plugins")
...
def test_transform_tool_result_integration_with_real_plugin(monkeypatch, tmp_path):
    """End-to-end: load a real plugin from XAVANI_HOME and verify it rewrites results."""
    import yaml

    xavani_home = Path(os.environ["XAVANI_HOME"])
