...
"""Tests for xavani_cli configuration management."""
...
class TestGetXavaniHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert home == Path.home() / ".xavani"

    def test_env_override(self):
        with patch.dict(os.environ, {"XAVANI_HOME": "/custom/path"}):
            home = get_xavani_home()
            assert home == Path("/custom/path")


class TestEnsureXavaniHome:
    ...
    def test_refreshes_unmodified_autoseed_soul_md(self, tmp_path):
        ...
        default so identity corrections reach existing installs."""
        from xavani_cli.config import _STALE_SOUL_BASE_IDENTITIES, _ensure_default_soul_md
    ...
    def test_does_not_refresh_customized_soul_md(self, tmp_path):
        """A user-customized SOUL.md is never refreshed/clobbered."""
        from xavani_cli.config import _ensure_default_soul_md

        with patch.dict(os.environ, {"XAVANI_HOME": str(tmp_path)}):
            soul_path = tmp_path / "SOUL.md"
            custom = "You are Captain Xavani, a helpful pirate first mate."
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``xavani logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from xavani_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"XAVANI_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="xavani_cli.config"):
                config = load_config()
            ...
            # stands out at xavani startup before logging is configured)
            captured = capsys.readouterr()
            assert "xavani config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"XAVANI_HOME": str(tmp_path)}):
            ...
            assert "xavani config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"XAVANI_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"XAVANI_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"XAVANI_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"XAVANI_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"XAVANI_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from xavani_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
