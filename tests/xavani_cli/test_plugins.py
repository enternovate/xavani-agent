...
"""Tests for the Xavani plugin system (xavani_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<xavani_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<xavani_home>/plugins/``; we derive
    ``<xavani_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <XAVANI_HOME>/config.yaml.
        # Config is always read from XAVANI_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        xavani_home_str = os.environ.get("XAVANI_HOME")
        if xavani_home_str:
            xavani_home = Path(xavani_home_str)
        else:
            xavani_home = base.parent
        xavani_home.mkdir(parents=True, exist_ok=True)
        cfg_path = xavani_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.xavani/plugins/ are discovered."""
        plugins_dir = tmp_path / "xavani_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("XAVANI_HOME", str(tmp_path / "xavani_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XAVANI_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".xavani" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under xavani_plugins.<name>."""
        plugins_dir = tmp_path / "xavani_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("XAVANI_HOME", str(tmp_path / "xavani_test"))

        # Clean up any prior namespace module
        sys.modules.pop("xavani_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "xavani_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="xavani_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "xavani_cli.plugins.invoke_hook",
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("xavani_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "xavani_test" / "plugins"
        # Set XAVANI_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("xavani_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("xavani_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("xavani_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("xavani_cli.plugins.PluginContext.dispatch_tool.__module__", "xavani_cli.plugins"):
...
class TestPluginDebugLogging:
    """XAVANI_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("XAVANI_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With XAVANI_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("XAVANI_PLUGINS_DEBUG", "1")
        from xavani_cli import plugins as plugins_mod
