...
The terminal `xavani model` flow already exposes `custom_providers`, but the
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "XAVANI_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_named_custom_provider(monkeypatch):
    ...
    monkeypatch.setattr("xavani_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("xavani_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("xavani_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Xavani's own writer stores configured models as a dict
...
def test_list_authenticated_providers_groups_same_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "XAVANI_OVERLAYS", {})
    monkeypatch.setattr("xavani_cli.models.fetch_api_models", lambda *a, **kw: [])
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("xavani_cli.models.fetch_lmstudio_models", _fake_fetch)
