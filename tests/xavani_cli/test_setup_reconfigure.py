...
- Bare `xavani setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `xavani setup --quick` runs the narrower "fill in missing items" flow.
- `xavani setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("xavani_cli.setup.ensure_xavani_home", {}),
        ("xavani_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("xavani_cli.config.is_managed", {"return_value": False}),
        ("xavani_cli.setup.load_config", {"return_value": {}}),
        ("xavani_cli.setup.save_config", {}),
        ("xavani_cli.setup.get_env_value", {"return_value": None}),
        ("xavani_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("xavani_cli.setup._print_setup_summary", {}),
        ("xavani_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("xavani_cli.setup.load_config", {"return_value": {}}),
        ("xavani_cli.setup.save_config", {}),
        ("xavani_cli.auth.get_active_provider", {"return_value": None}),
        ("xavani_cli.setup.get_env_value", {"return_value": None}),
        ("xavani_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `xavani setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="xavani_cli.setup.prompt_choice",
                quick="xavani_cli.setup._run_quick_setup",
                model="xavani_cli.setup.setup_model_provider",
                terminal="xavani_cli.setup.setup_terminal_backend",
                agent="xavani_cli.setup.setup_agent_settings",
                gateway="xavani_cli.setup.setup_gateway",
                tools="xavani_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`xavani setup --reconfigure` behaves the same as bare `xavani setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("xavani_cli.setup.prompt_choice", {"return_value": 0}),
                first="xavani_cli.setup._run_first_time_quick_setup",
            )
            from xavani_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["xavani", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["xavani", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["xavani", "setup"])
        try:
            main()
        except SystemExit:
            pass
