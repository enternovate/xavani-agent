...
"""Tests for the Kanban DB layer (xavani_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated XAVANI_HOME with an empty kanban DB."""
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("XAVANI_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import xavani_cli.kanban_db as _kb
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Xavani profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="xavani-task", assignee="daily")
...
def test_scratch_workspace_created_under_xavani_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `xavani -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's XAVANI_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's XAVANI_HOME."""

    def _set_home(self, monkeypatch, tmp_path, xavani_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(xavani_home))
        monkeypatch.delenv("XAVANI_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_xavani(
        self, tmp_path, monkeypatch
    ):
        # Standard install: XAVANI_HOME == ~/.xavani, no profile active.
        default_home = tmp_path / ".xavani"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.xavani/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.xavani/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.xavani/kanban.db.
        default_home = tmp_path / ".xavani"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # XAVANI_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `xavani -p coder`).
    ...
    def test_docker_custom_xavani_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: XAVANI_HOME points outside ~/.xavani.
        # `get_default_xavani_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".xavani"`.
        custom_root = tmp_path / "opt" / "xavani"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: XAVANI_HOME=/opt/xavani/profiles/coder;
        # `get_default_xavani_root()` walks up to /opt/xavani because
    ...
    def test_explicit_override_via_xavani_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: XAVANI_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".xavani"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(profile_home))
        monkeypatch.setenv("XAVANI_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(default_home))
        monkeypatch.setenv("XAVANI_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # XAVANI_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile XAVANI_HOME and reads.
        monkeypatch.setenv("XAVANI_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_xavani_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # XAVANI_KANBAN_DB pins the file path directly and beats both
        # XAVANI_KANBAN_HOME and the `get_default_xavani_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(default_home))
        monkeypatch.setenv("XAVANI_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("XAVANI_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows XAVANI_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_xavani_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # XAVANI_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".xavani"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("XAVANI_HOME", str(default_home))
        monkeypatch.setenv("XAVANI_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("XAVANI_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows XAVANI_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(default_home))
        monkeypatch.setenv("XAVANI_KANBAN_DB", "   ")
        monkeypatch.setenv("XAVANI_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject XAVANI_KANBAN_DB
        # and XAVANI_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites XAVANI_HOME.
        default_home = tmp_path / ".xavani"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["XAVANI_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["XAVANI_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["XAVANI_KANBAN_TASK"] == "t_dispatch_env"
        assert env["XAVANI_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see xavani_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("xavani_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="xavani_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'xavani kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_xavani_argv()
#
# Workers spawned by the dispatcher must use a `xavani` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["xavani", ...]`
...
def test_resolve_xavani_argv_prefers_path_shim(monkeypatch):
    """When `xavani` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import xavani_cli.kanban_db as kb

    monkeypatch.delenv("XAVANI_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/xavani")
    argv = kb._resolve_xavani_argv()
    assert argv == ["/usr/local/bin/xavani"]


def test_resolve_xavani_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import xavani_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("XAVANI_BIN", ".\\xavani.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_xavani_argv() == [os.path.abspath(".\\xavani.exe")]


def test_resolve_xavani_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "xavani.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("XAVANI_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_xavani_argv() == [sys.executable, "-m", "xavani_cli.main"]


def test_resolve_xavani_argv_honors_xavani_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like XAVANI_BIN lets service managers pin the executable."""
    import shutil
    import xavani_cli.kanban_db as kb

    shim = tmp_path / "bin" / "xavani"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("XAVANI_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_xavani_argv() == [str(shim)]


def test_resolve_xavani_argv_xavani_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare XAVANI_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import xavani_cli.kanban_db as kb

    cwd_xavani = tmp_path / "xavani"
    cwd_xavani.write_text("wrong\n", encoding="utf-8")
    cwd_xavani.chmod(cwd_xavani.stat().st_mode | stat.S_IXUSR)
    path_xavani = tmp_path / "bin" / "xavani"
    path_xavani.parent.mkdir()
    path_xavani.write_text("right\n", encoding="utf-8")
    path_xavani.chmod(path_xavani.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_xavani.parent))
    monkeypatch.setenv("XAVANI_BIN", "xavani")

    assert kb._resolve_xavani_argv() == [str(path_xavani)]


def test_resolve_xavani_argv_xavani_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare XAVANI_BIN does not accept current-directory shadow executables."""
    import sys
    import xavani_cli.kanban_db as kb

    (tmp_path / "xavani.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_xavani_argv_xavani_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved XAVANI_BIN batch shim is not used as worker argv[0]."""
    import sys
    import xavani_cli.kanban_db as kb
...
def test_resolve_xavani_argv_xavani_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved XAVANI_BIN command names do not delegate cwd search to Popen."""
    import sys
    import xavani_cli.kanban_db as kb
...
def test_resolve_xavani_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m xavani_cli.main`.

    Pins the correct module name (NOT `xavani` — there is no top-level
    `xavani` package). Regression for #23198: the original PR shipped
    `python -m xavani` which fails with `No module named xavani` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_xavani_argv()
    assert argv == [sys.executable, "-m", "xavani_cli.main"]


def test_resolve_xavani_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `xavani_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m xavani_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("XAVANI_BIN", None)
    ...
    assert "Xavani Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
