...
Ensures ``xavani update`` correctly detects running gateways managed by
...
def _no_restart_verify_sleep(monkeypatch):
    """Neutralise the post-restart verification waits so these tests are fast
    AND deterministic.

    ``_wait_for_service_active`` (nested in ``cmd_update``) polls
    ``systemctl is-active`` until a ``time.monotonic()`` deadline
    (``timeout=10.0``), sleeping 0.5s between polls.  On the failure path it
    is called TWICE (initial restart + retry) = ~20s.

    Mocking ``time.sleep`` alone is NOT enough: with sleep neutered but
    ``monotonic`` real, the poll loop BUSY-SPINS for the full 10s of real
    wall-clock per call (~20s total).  That both wastes time and pushes the
    test toward the 30s per-test SIGALRM guard in the global conftest — under
    parallel xdist load it intermittently trips, producing flaky assertion
    failures and even xdist worker crashes.

    Driving a fake monotonic clock forward (advanced by ``sleep`` and nudged
    on every read) makes each deadline fire after a handful of fast
    iterations, so the verification loops terminate near-instantly while
    still exercising the real branch logic.  ``signal.alarm``-based timeouts
    use real time, so this does not interfere with the SIGALRM guard itself.
    """
    import time as _real_time

    _clock = {"t": _real_time.monotonic()}
    ...
    def _fake_monotonic():
        # Nudge forward on every read so even a sleep-free busy loop that
        # only checks monotonic() still reaches its deadline quickly.
        _clock["t"] += 0.05
        return _clock["t"]
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units xavani-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="xavani-gateway.service loaded active running Xavani Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>XAVANI_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: xavani gateway run'."""
        ...
        assert "Restart manually: xavani gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: xavani gateway run" in captured
    ...
    def test_update_restarts_profile_manual_gateways(
        ...
        monkeypatch.setattr(
            ...
            lambda: tmp_path / "ai.xavani.gateway.plist",
        )
        ...
        process = gateway_cli.ProfileGatewayProcess(
            profile="coder",
            path=tmp_path / ".xavani" / "profiles" / "coder",
            pid=12345,
        )
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted xavani-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="xavani-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "xavani_cli.gateway._graceful_restart_via_sigusr1",
    ...
    def test_update_bypasses_restartsec_after_graceful_drain(
        ...
        # Must have called ``reset-failed xavani-gateway`` AND ``start
        # xavani-gateway`` explicitly so systemd bypasses RestartSec.
        reset_calls = [c for c in calls if "reset-failed" in c and "xavani-gateway" in c]
        start_calls = [
            c for c in calls
            if "start" in c and "xavani-gateway" in c and "restart" not in c
        ]
        assert reset_calls, (
            f"Expected explicit `reset-failed xavani-gateway` after graceful drain; "
            f"systemctl calls were: {calls}"
        )
        assert start_calls, (
            f"Expected explicit `start xavani-gateway` after graceful drain to "
...
class TestServicePidExclusion:
    ...
    where ``xavani update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.xavani.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="xavani-gateway.service loaded inactive dead Xavani Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.xavani/xavani-agent/venv/bin/python -m xavani_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.xavani/xavani-agent/venv/bin/python -m xavani_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr("os.getpid", lambda: 999)
        monkeypatch.setattr(gateway_cli, "_get_service_pids", lambda: set())
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda xavani_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``xavani update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point XAVANI_HOME at a temp dir so the marker file lands there
        xavani_home = tmp_path / ".xavani"
        xavani_home.mkdir()
        monkeypatch.setenv("XAVANI_HOME", str(xavani_home))
        import xavani_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_xavani_home", lambda: xavani_home)
        # Also patch the module-level ref used by cmd_update
        import xavani_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_xavani_home", lambda: xavani_home)
        ...
        exit_code_path = xavani_home / ".update_exit_code"
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy xavani.service warning printed by `xavani update`.

    Users who installed Xavani before the service rename often have a
    dormant ``xavani.service`` that starts flap-fighting the current
    ``xavani-gateway.service`` after PR #5646. Every ``xavani update``
    should remind them to run ``xavani gateway migrate-legacy`` until
    they do.
    """
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "xavani.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy Xavani gateway unit(s) detected" in captured
        assert "xavani.service" in captured
        assert "xavani gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Xavani gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (xavani-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``xavani.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        (user_dir / "xavani-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "xavani-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy Xavani gateway" not in captured
        assert "xavani-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Xavani gateway" in captured
...
class TestCmdUpdateResetFailedBeforeRestart:
    """`xavani update` must call `systemctl reset-failed` before every
    ...
    Mirrors the recovery pattern `xavani gateway restart` (systemd_restart)
    ...
    def test_reset_failed_runs_before_fallback_restart(
        ...
        assert any(
            "xavani-gateway" in " ".join(str(c) for c in call)
            for call in reset_calls
        ), (
            "Expected `systemctl reset-failed xavani-gateway` before the "
        ...
        for idx, call in enumerate(mock_run.call_args_list):
            ...
            if "systemctl" in joined and "reset-failed" in joined and first_reset_idx is None:
                first_reset_idx = idx
            if "systemctl" in joined and "restart" in joined and "xavani-gateway" in joined:
    ...
    def test_reset_failed_also_runs_before_retry_restart(
        ...
        gateway_restarts = [
            ...
            if "xavani-gateway" in " ".join(str(a) for a in c)
        ]
        gateway_resets = [
    ...
    def test_final_failure_message_tells_user_to_reset_failed(
        ...
        assert "xavani-gateway" in captured
