...
When ``XAVANI_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.xavani/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.xavani            (has its own auth.json fixture)
    * Profile     -> tmp_path/.xavani/profiles/coder   (active, XAVANI_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (XAVANI_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.xavani as the "real home" — which is NOT equal
    # to the XAVANI_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    xavani_home = tmp_path / "classic"
    xavani_home.mkdir()
    monkeypatch.setenv("XAVANI_HOME", str(xavani_home))

    _write(xavani_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from xavani_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: XAVANI_HOME is set to a custom path that is NOT under
    # ~/.xavani/profiles/ — get_default_xavani_root() returns XAVANI_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from xavani_cli.auth import read_credential_pool, write_credential_pool
