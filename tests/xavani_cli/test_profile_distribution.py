...
"""Tests for xavani_cli.profile_distribution — git-based profile installs.
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_xavani_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Xavani"):
                check_xavani_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "Xavani distribution" in out
...
class TestInstall:
    ...
    def test_install_enforces_xavani_requires(self, profile_env, monkeypatch):
        # Pin current Xavani version to something well below the requirement
        import xavani_cli
        monkeypatch.setattr(xavani_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            xavani_requires=">=99.0.0",
        )
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        from xavani_cli.profiles import get_profile_dir
        ...
        monkeypatch.setattr(
            "xavani_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from xavani_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from xavani_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from xavani_cli.profiles import create_profile, list_profiles, get_profile_dir
