...
"""Tests for SIGHUP protection and stdout mirroring in ``xavani update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``xavani_cli/main.py``.  These exist so
that ``xavani update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_xavani_home if present
        import xavani_cli.config as _cfg
        if hasattr(_cfg, "_XAVANI_HOME_CACHE"):
            _cfg._XAVANI_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        # Resolve install/finalize/class ALL from the live module object so
        # they are mutually consistent.  Another xdist-worker test
        # (test_skills_subparser, test_env_loader) may have reloaded
        # xavani_cli.main, leaving sys.modules with a fresh module whose
        # _UpdateOutputStream class differs from this file's collection-time
        # import.  Using one live reference for everything avoids the
        # "wrapped with old class, checked against new class" mismatch.
        import xavani_cli.main as _live_main
        _LiveInstall = _live_main._install_hangup_protection
        _LiveFinalize = _live_main._finalize_update_output
        _LiveUpdateOutputStream = _live_main._UpdateOutputStream

        prev_out, prev_err = sys.stdout, sys.stderr
        state = _LiveInstall(gateway_mode=False)

        try:
            # On Windows (no SIGHUP) we still wrap stdio and create the log.
            assert state["installed"] is True
            assert isinstance(sys.stdout, _LiveUpdateOutputStream)
            assert isinstance(sys.stderr, _LiveUpdateOutputStream)
            ...
            assert "xavani update started" in contents
        finally:
            _LiveFinalize(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_xavani_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "xavani_cli.config.get_xavani_home", _boom, raising=True
        )
