...
"""Tests for IRC gateway configuration via `xavani setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Xavani install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Xavani install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#xavani")
            monkeypatch.setenv("IRC_NICKNAME", "xavani-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `xavani setup gateway` experience with IRC present."""
