...
"""Tests for Bug #12905 fix — stale OAuth token detection in xavani model flow.

Bug 3: `xavani model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.xavani/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        from xavani_cli.main import _model_flow_anthropic
        cfg = {}
