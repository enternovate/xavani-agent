...
class TestSlackNativeSlashes:
    ...
    def test_xavani_catchall_is_first(self):
        """``/xavani`` must be reserved as the first slot so the legacy
        ``/xavani <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "xavani"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /xavani <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `xavani slack manifest`)."""
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch xavani_cli.plugins.get_plugin_commands() to a fixed dict."""
        from xavani_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/xavani metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
