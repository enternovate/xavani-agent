...
subparser, placing the flag BEFORE the subcommand (e.g. 'xavani --yolo chat')
...
class TestYoloEnvVar:
    """Verify --yolo sets XAVANI_YOLO_MODE regardless of flag position.
    ...
    def test_no_yolo_no_env(self):
        ...
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("XAVANI_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `xavani gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `xavani <argv>` must exit 0 (help) rather than
