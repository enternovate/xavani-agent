...
"""Tests for the decomposer module + `xavani kanban decompose` CLI surface.
...
from xavani_cli import kanban as kanban_cli
...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("xavani_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("xavani_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch("xavani_cli.profiles.get_active_profile_name", return_value=names[0] if names else "default"),
    ]
...
def test_decompose_fanout_false_assigns_default_when_unassigned(kanban_home):
    ...
    try:
        with _patch_aux_client(llm_payload), _patch_extra_body(), patch(
            "xavani_cli.kanban_decompose._load_config",
