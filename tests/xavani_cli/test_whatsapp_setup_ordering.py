...
Before the fix, ``xavani whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``xavani gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    xavani = home / ".xavani"
    xavani.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("XAVANI_HOME", str(xavani))
...
def _env_value(xavani_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("xavani_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``xavani whatsapp`` with an existing paired session and
