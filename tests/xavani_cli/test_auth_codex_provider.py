...
"""Tests for Codex auth — tokens stored in Xavani auth store (~/.xavani/auth.json)."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_xavani_auth(xavani_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_xavani_auth(xavani_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("XAVANI_HOME", str(xavani_home))
    ...
    monkeypatch.setattr("xavani_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    xavani_home = tmp_path / "xavani"
    _setup_xavani_auth(xavani_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("XAVANI_HOME", str(xavani_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Xavani auth store, not ~/.codex/."""
    xavani_home = tmp_path / "xavani"
    codex_home = tmp_path / "codex-cli"
    xavani_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "xavani-at", "refresh_token": "xavani-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Xavani store
    assert not (codex_home / "auth.json").exists()

    # Xavani auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "xavani-at"


def test_resolve_returns_xavani_auth_store_source(tmp_path, monkeypatch):
    xavani_home = tmp_path / "xavani"
    _setup_xavani_auth(xavani_home)
    monkeypatch.setenv("XAVANI_HOME", str(xavani_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "xavani_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("xavani_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("xavani_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
