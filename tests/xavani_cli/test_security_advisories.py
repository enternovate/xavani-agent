...
"""Tests for xavani_cli.security_advisories.
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect XAVANI_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full xavani_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "xavani_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "xavani_cli.config.save_config",
...
class TestRendering:
    def test_short_banner_lines_includes_id_and_version(self, fake_advisory):
        ...
        assert "xavani doctor" in joined
