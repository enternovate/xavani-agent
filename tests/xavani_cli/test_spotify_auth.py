...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from xavani_cli.auth_commands import auth_status_command
...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes XAVANI_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("XAVANI_HOME", str(tmp_path))
    ...
    assert "XAVANI_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "XAVANI_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "XAVANI_SPOTIFY_CLIENT_ID" not in env_path.read_text()
