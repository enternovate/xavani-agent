...
def container_env(tmp_path, monkeypatch):
    """Set up a fake XAVANI_HOME with .container-mode file."""
    xavani_home = tmp_path / ".xavani"
    xavani_home.mkdir()
    monkeypatch.setenv("XAVANI_HOME", str(xavani_home))
    monkeypatch.delenv("XAVANI_DEV", raising=False)

    container_mode = xavani_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=xavani-agent\n"
        "exec_user=xavani\n"
        "xavani_bin=/data/current-package/bin/xavani\n"
    )
    return xavani_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("xavani_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "xavani-agent"
    assert info["exec_user"] == "xavani"
    assert info["xavani_bin"] == "/data/current-package/bin/xavani"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("xavani_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_xavani_dev(container_env, monkeypatch):
    """Returns None when XAVANI_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("XAVANI_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_xavani_dev_zero(container_env, monkeypatch):
    """XAVANI_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("XAVANI_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        xavani_home = Path(tmpdir) / ".xavani"
        xavani_home.mkdir()
        (xavani_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("xavani_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_xavani_home": lambda: xavani_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("XAVANI_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=xavani-custom\n"
        "exec_user=myuser\n"
        "xavani_bin=/opt/xavani/bin/xavani\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "xavani-custom"
    assert info["exec_user"] == "myuser"
    assert info["xavani_bin"] == "/opt/xavani/bin/xavani"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from xavani_cli.main import _exec_in_container
