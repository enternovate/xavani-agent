...
from xavani_cli.main, these tests mock the delegation point and verify
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("xavani_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("xavani_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_keep_current_custom_from_config_does_not_fall_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("xavani_cli.main.select_provider_and_model", fake_select)
...
def test_setup_same_provider_rotation_strategy_saved_for_multi_credential_pool(tmp_path, monkeypatch):
    ...
    import xavani_cli.setup as _setup_mod
...
def test_setup_same_provider_fallback_can_add_another_credential(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("xavani_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("xavani_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("agent.credential_pool.load_pool", fake_load_pool)
    monkeypatch.setattr("xavani_cli.auth_commands.auth_add_command", fake_auth_add_command)
...
def test_setup_pool_step_shows_manual_vs_auto_detected_counts(tmp_path, monkeypatch, capsys):
    ...
    _stub_tts(monkeypatch)
    monkeypatch.setattr("xavani_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("xavani_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("xavani_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("agent.credential_pool.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("xavani_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("xavani_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("xavani_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'xavani setup' to configure" not in output


def test_setup_summary_shows_browser_direct_hint_without_subscription(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("XAVANI_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
