...
import xavani_cli.doctor as doctor
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `xavani doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "XAVANI_HOME", xavani_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("XAVANI_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("XAVANI_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_xavani_home(self, tmp_path, provider=""):
        """Create a minimal XAVANI_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_xavani_home(tmp_path, provider)
...
def test_run_doctor_accepts_xavani_provider_ids_that_catalog_aliases(
...
def test_run_doctor_dashscope_retries_china_endpoint_after_intl_unauthorized(monkeypatch, tmp_path):
    ...
    # The DashScope provider itself must NOT report "invalid API key".
    # Note: "Alibaba Cloud (Coding Plan)" is a separate provider that may fail
    # independently (it uses ALIBABA_CODING_PLAN_API_KEY, not this test's focus).
    assert not any(
        "Alibaba/DashScope" in line and "invalid API key" in line
        for line in out.splitlines()
    ), "DashScope (not Coding Plan) reported 'invalid API key'"
    assert any(
...
class TestGitHubTokenCheck:
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from xavani_cli.doctor import run_doctor, _DHH
        import io, contextlib
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from xavani_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("xavani_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "xavani_cli.doctor", raising=False)
        from xavani_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "Xavani Portal auth" in out
        assert "logged in" in out
