...
"""Tests for the top-level `./xavani` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./xavani` should use `xavani_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "xavani"
    called = []

    fake_main_module = types.ModuleType("xavani_cli.main")

    def fake_main():
        called.append("xavani_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "xavani_cli.main", fake_main_module)
