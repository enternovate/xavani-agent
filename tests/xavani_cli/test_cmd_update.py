...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_refreshes_repo_and_tui_node_dependencies(
        ...
        from xavani_cli import main as hm
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "xavani_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "xavani_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("xavani_cli.config.check_config_version", return_value=(1, 2)), patch(
            "xavani_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("xavani_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.xavani"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.xavani/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.xavani/profiles/work"))
        ...
        with (
            patch("xavani_cli.profiles.list_profiles", return_value=all_profiles),
            patch("xavani_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("xavani_cli.profiles.list_profiles", return_value=[default_p]),
            patch("xavani_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
