...
"""Comprehensive tests for xavani_cli.profiles module.
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.xavani/profiles)
    * XAVANI_HOME  -> tmp_path/.xavani  (so get_xavani_home() agrees)
    * Creates the bare-minimum ~/.xavani directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["xavani", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Xavani install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_xavani_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".xavani" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".xavani" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.xavani must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".xavani"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes xavani-agent, .worktrees,
        ...
        (default_home / "xavani-agent" / ".git").mkdir(parents=True)
        (default_home / "xavani-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "xavani-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "xavani-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `xavani profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `xavani update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("xavani_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".xavani" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_xavani_home_returns_default(self, profile_env):
        # XAVANI_HOME points to tmp_path/.xavani which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom XAVANI_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("XAVANI_HOME", str(custom))
        # With Docker-aware roots, a custom XAVANI_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("xavani")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "xavani.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["xavani.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["xavani.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "xavani.ssi_health": {"workspace": "xavani", "enabled": True}
            }
        }))
        ...
        assert "xavani.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["xavani.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["xavani.heimdall"]["workspace"] == "xavani"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "xavani.ssi_health": {"aiPeer": "ssi_health"},
                "xavani.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["xavani.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["xavani.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("xavani-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".xavani_history",
        ...
        excluded_prefixes = [
            "default/xavani-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.xavani_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultXavaniHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_xavani_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".xavani" / "profiles"

    def test_default_xavani_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_xavani_home()
        assert home == tmp_path / ".xavani"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (XAVANI_HOME outside ~/.xavani), profiles go under XAVANI_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_xavani_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_xavani_home() returns XAVANI_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (XAVANI_HOME under ~/.xavani), profiles root is still ~/.xavani/profiles."""
        native = tmp_path / ".xavani"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under XAVANI_HOME."""
        from xavani_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under XAVANI_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("XAVANI_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".xavani" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from xavani_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".xavani"
