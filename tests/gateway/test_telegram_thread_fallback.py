...
Supergroup forum topics route with ``message_thread_id``. Xavani-created
...
def _inject_fake_telegram(monkeypatch):
    ...
    Also patches ``gateway.platforms.telegram.ChatType`` directly so that
    ``telegram_mod.ChatType.SUPERGROUP`` etc. are real strings regardless of
    which telegram mock was in place when the gateway module was first imported.

    Simply replacing sys.modules entries is not enough because Python caches
    the imported submodule as a package attribute on ``gateway.platforms``.
    ``from gateway.platforms import telegram`` returns that cached attribute
    (the already-imported module object) even if sys.modules is updated — so
    the test's ``telegram_mod.ChatType`` is the old ChatType global captured at
    first-import time, not the fake one in ``_fake_telegram_constants``.
    Patching the module attribute directly is the only reliable fix.
    """
    ...
    # Patch ChatType on the already-cached gateway telegram module so any test
    # accessing ``telegram_mod.ChatType.SUPERGROUP`` sees the real string value.
    import gateway.platforms.telegram as _gw_tg
    monkeypatch.setattr(_gw_tg, "ChatType", _fake_telegram_constants.ChatType, raising=False)
...
async def test_send_typing_attempts_api_call_for_dm_topic_reply_fallback():
    """Xavani-created DM topic lanes should still attempt scoped typing.
...
@pytest.mark.asyncio
async def test_send_uses_reply_fallback_for_xavani_dm_topics():
    """Xavani-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in Xavani-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []
