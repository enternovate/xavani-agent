...
class TestPlatformTokenPlaceholderGuard:
    ...
    def test_accepts_real_token(self, caplog):
        ...
        The fixture uses an obviously-synthetic string that satisfies the
        validator's length / shape heuristics without matching any
        provider's actual token format. GitHub's secret-scanner picked up
        an earlier value that happened to overlap the Telegram bot-token
        regex; this version keeps the test meaningful while staying
        outside that pattern.
        """
        config = _make_gateway_config(
            Platform.TELEGRAM,
            "synthetic-test-token-not-a-real-secret-0000000000000000",
        )
        with caplog.at_level(logging.ERROR):
            _validate_and_return(config)
