...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    def test_get_chat_info_uses_real_feishu_chat_api(self):
        ...
        class _ChatAPI:
            def get(self, request):
                ...
                return SimpleNamespace(
                    success=lambda: True,
                    data=SimpleNamespace(name="Xavani Group", chat_type="group"),
                )
        ...
        self.assertEqual(info["name"], "Xavani Group")
...
class TestAdapterBehavior(unittest.TestCase):
    # File-upload tests create temp files outside os.getcwd(), but feishu.py
    # calls validate_path(path, os.getcwd()) as a security guard. Patch it
    # to a simple pass-through so upload tests focus on Feishu API behaviour,
    # not path containment (which is covered by test_security.py).
    def setUp(self):
        self._vp_patch = patch(
            "gateway.platforms.feishu.validate_path",
            side_effect=lambda p, *_a, **_kw: Path(p),
        )
    ...
    @patch.dict(
        os.environ,
        {
            "XAVANI_FEISHU_TEXT_BATCH_MAX_MESSAGES": "2",
        },
        clear=True,
    )
    ...
    def test_download_remote_document_reads_response_before_httpx_client_closes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict(os.environ, {"XAVANI_HOME": tmp}, clear=False):
                adapter = FeishuAdapter(PlatformConfig())
    ...
    def test_dedup_state_persists_across_adapter_restart(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"XAVANI_HOME": temp_home}, clear=False):
    ...
    def test_send_splits_fenced_code_blocks_into_separate_post_rows(self):
        ...
        content = (
            "确认已入库 ✓\n"
            "文件路径：`/root/.xavani/profiles/agent_cto/cron/jobs.json`\n"
            "**解码后的内容：**\n"
            "```json\n"
            '{"cron": "list"}\n'
            "```\n"
...
class TestHydrateBotIdentity(unittest.TestCase):
    ...
    def test_hydration_populates_open_id_from_bot_info(self):
        ...
        self.assertEqual(adapter._bot_open_id, "ou_xavani_hydrated")
        self.assertEqual(adapter._bot_name, "Xavani Bot")

    @patch.dict(
        os.environ,
        {
            "FEISHU_BOT_OPEN_ID": "ou_env",
            "FEISHU_BOT_NAME": "Env Xavani",
        },
        clear=True,
    )
    def test_hydration_refreshes_env_values_when_bot_info_available(self):
        ...
        self.assertEqual(adapter._bot_name, "Hydrated Xavani")
    ...
    def test_hydration_overwrites_stale_env_open_id(self):
        ...
        self.assertEqual(adapter._bot_name, "Xavani Bot")  # filled in
    ...
    def test_hydration_preserves_env_values_when_bot_info_probe_fails(self):
        ...
        self.assertEqual(adapter._bot_name, "Env Xavani")
...
class TestDedupTTL(unittest.TestCase):
    ...
    def test_load_tolerates_malformed_timestamp_values(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"XAVANI_HOME": temp_home}, clear=True):
                adapter = FeishuAdapter(PlatformConfig())
...
class TestFeishuMentionMap(unittest.TestCase):
    def test_build_mentions_map_handles_at_all(self):
        ...
        result = _build_mentions_map(
            [mention],
            _FeishuBotIdentity(open_id="ou_bot", name="Xavani"),
        )
    ...
    def test_build_mentions_map_marks_self_by_open_id(self):
        ...
        self.assertEqual(ref.name, "Xavani")
    ...
    def test_build_mentions_map_marks_self_by_name_fallback(self):
        ...
        result = _build_mentions_map([mention], _FeishuBotIdentity(name="Xavani"))
    ...
    def test_build_mentions_map_name_match_does_not_override_mismatching_open_id(self):
        ...
        result = _build_mentions_map(
            ...
            _FeishuBotIdentity(open_id="ou_bot", name="Xavani Bot"),
        )
    ...
    def test_build_mentions_map_falls_back_to_name_when_bot_open_id_not_hydrated(self):
        ...
        result = _build_mentions_map(
            [bot_mention],
            _FeishuBotIdentity(open_id="", name="Xavani Bot"),
        )
        self.assertTrue(result["@_user_1"].is_self)
...
class TestFeishuMentionHint(unittest.TestCase):
    ...
    def test_hint_filters_self_mentions(self):
        ...
        refs = [
            FeishuMentionRef(name="Xavani", open_id="ou_bot", is_self=True),
            FeishuMentionRef(name="Alice", open_id="ou_alice"),
        ]
        self.assertEqual(
            _build_mention_hint(refs),
    ...
    def test_hint_returns_empty_when_only_self(self):
        ...
        refs = [FeishuMentionRef(name="Xavani", open_id="ou_bot", is_self=True)]
...
class TestFeishuStripLeadingSelf(unittest.TestCase):
    def _make_refs(self, *, self_name="Xavani", other_name=None):
    ...
    def test_strips_leading_self(self):
        ...
        result = _strip_edge_self_mentions("@Xavani /help", self._make_refs())
        self.assertEqual(result, "/help")

    def test_strips_consecutive_leading_self(self):
        from gateway.platforms.feishu import _strip_edge_self_mentions

        result = _strip_edge_self_mentions("@Xavani @Xavani hi", self._make_refs())
    ...
    def test_stops_at_first_non_self_token(self):
        ...
        result = _strip_edge_self_mentions(
            "@Xavani @Alice make a group", self._make_refs(other_name="Alice")
        )
    ...
    def test_preserves_mid_text_self(self):
        ...
        result = _strip_edge_self_mentions("check @Xavani said yesterday", self._make_refs())
        self.assertEqual(result, "check @Xavani said yesterday")

    def test_strips_trailing_self_at_end_of_text(self):
        from gateway.platforms.feishu import _strip_edge_self_mentions

        result = _strip_edge_self_mentions("look up docs @Xavani", self._make_refs())
    ...
    def test_preserves_trailing_self_before_non_terminal_char(self):
        ...
        result = _strip_edge_self_mentions(
            "please don't @Xavani anymore", self._make_refs()
        )
        self.assertEqual(result, "please don't @Xavani anymore")

    def test_returns_input_when_refs_empty(self):
        from gateway.platforms.feishu import _strip_edge_self_mentions

        self.assertEqual(_strip_edge_self_mentions("@Xavani /help", []), "@Xavani /help")
...
class TestFeishuNormalizeText(unittest.TestCase):
    ...
    def test_renders_self_mention_with_name(self):
        ...
        refs = {"@_user_1": FeishuMentionRef(name="Xavani", open_id="ou_bot", is_self=True)}
        self.assertEqual(
            _normalize_feishu_text("stop pinging @_user_1 please", refs),
            "stop pinging @Xavani please",
        )
...
class TestFeishuNormalizeWithMentions(unittest.TestCase):
    ...
    def test_text_message_marks_bot_self_mention(self):
        ...
        self.assertEqual(normalized.text_content, "@Xavani /help")
    ...
    def test_post_message_marks_self_via_mentions_map_lookup(self):
        ...
        raw = json.dumps({
            "en_us": {
                "content": [
                    [
                        {"tag": "at", "user_id": "@_user_1", "user_name": "Xavani"},
...
class TestFeishuProcessInboundMessage(unittest.TestCase):
    ...
    def test_mid_text_self_mention_preserved(self):
        ...
        event = adapter._dispatch_inbound_event.call_args.args[0]
        self.assertEqual(event.text, "stop pinging @Xavani please")

    def test_pure_self_mention_message_is_ignored(self):
        """A message containing only '@Bot' (no body, no media) must not dispatch.

        Regression guard: the rendered '@Xavani' slips past the pre-strip empty
...
class TestFeishuFetchMessageText(unittest.TestCase):
    ...
    def test_fetch_message_text_marks_is_self_via_string_id_shape(self):
        ...
        result = asyncio.run(adapter._fetch_message_text("m_parent"))
        self.assertEqual(result, "@Xavani hi")
    ...
    def test_build_mentions_map_string_id_shape(self):
        ...
        bot_oid = SimpleNamespace(key="@_user_3", id="ou_bot", id_type="open_id", name="Xavani")
        self.assertTrue(
...
class TestFeishuMentionEndToEnd(unittest.TestCase):
    ...
    def test_scenario_bot_plus_alice_plus_bob_build_group(self):
        ...
        event = self._run(
            ...
            [
                {"key": "@_user_1", "open_id": "ou_bot", "name": "Xavani"},
        ...
        self.assertNotIn("@Xavani", event.text)
    ...
    def test_scenario_mid_text_self_mention_preserved(self):
        ...
        self.assertEqual(event.text, "please don't @Xavani anymore")
    ...
    def test_scenario_post_bot_plus_alice_filters_self_from_hint(self):
        ...
        self.assertIn("[Mentioned: Alice (open_id=ou_alice)]", event.text)
        self.assertNotIn("Xavani (open_id=", event.text)
        # Body: leading @Xavani stripped, Alice preserved, trailing text intact.
        self.assertIn("@Alice review the spec with Alice", event.text)
        self.assertNotIn("@Xavani @Alice", event.text)
