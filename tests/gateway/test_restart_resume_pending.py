...
class TestFreshnessHelpers:
    ...
    def test_auto_continue_freshness_window_malformed_falls_back(self, monkeypatch):
        monkeypatch.setenv("XAVANI_AUTO_CONTINUE_FRESHNESS", "not-a-number")
        assert _auto_continue_freshness_window() == 3600.0

    def test_auto_continue_freshness_window_empty_falls_back(self, monkeypatch):
        monkeypatch.setenv("XAVANI_AUTO_CONTINUE_FRESHNESS", "")
        assert _auto_continue_freshness_window() == 3600.0
...
async def test_clean_drain_does_not_mark_resume_pending():
    ...
    # Sessions are pre-marked before drain then cleared on clean completion.
    # Verify net state: no session should remain flagged after a clean drain.
    mark_calls = session_store.mark_resume_pending.call_args_list
    clear_calls = session_store.clear_resume_pending.call_args_list
    marked = {args[0][0] for args in mark_calls}
    cleared = {args[0][0] for args in clear_calls}
    assert marked == cleared, (
        f"Clean drain left {marked - cleared} session(s) resume_pending"
    )
...
async def test_drain_timeout_only_marks_still_running_sessions():
    ...
    mark_calls = session_store.mark_resume_pending.call_args_list
    clear_calls = session_store.clear_resume_pending.call_args_list
    # Sessions are pre-marked before drain, then cleared if they finish during
    # drain even when overall drain times out.  Compute net effective set:
    # sessions that were marked but NOT subsequently cleared.
    marked = {args[0][0] for args in mark_calls}
    cleared = {args[0][0] for args in clear_calls}
    net_marked = marked - cleared
    # Only the session still running at timeout should be net-marked; the
    # finisher completed cleanly so its mark must have been cleared.
    assert net_marked == {session_key_stuck}
...
class TestStuckLoopEscalation:
    ...
    def test_escalation_via_stuck_loop_counter_overrides_resume_pending(
        ...
        monkeypatch.setattr("gateway.run._xavani_home", tmp_path)
