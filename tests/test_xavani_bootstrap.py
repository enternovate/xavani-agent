...
"""Tests for xavani_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Xavani entry point
(xavani, xavani-agent, xavani-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Xavani entry point imports xavani_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported xavani_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("xavani_bootstrap", None)
    import xavani_bootstrap  # noqa: WPS433
    return xavani_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Xavani CLI
...
class TestEntryPointsImportBootstrap:
    """Every Xavani entry point must import xavani_bootstrap as its
    ...
    # Entry points that invoke Xavani as a process.  Each one must
    # import xavani_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "xavani_cli/main.py",   # xavani CLI (console_script)
        "run_agent.py",          # xavani-agent (console_script)
        "acp_adapter/entry.py",  # xavani-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import xavani_bootstrap' and that
        ...
        half-finished ``xavani update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``xavani_bootstrap`` as a top-level module) leaves xavani
        ...
        """
        # Resolve relative to the xavani-agent repo root.  Tests live
        # at tests/test_xavani_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # xavani start even when xavani_bootstrap hasn't been
        ...
        assert first_import_name == "xavani_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'xavani_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import xavani_bootstrap' line to be the first import."
        )
