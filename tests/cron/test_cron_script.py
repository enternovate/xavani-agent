...
- Path resolution (absolute, relative to XAVANI_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp XAVANI_HOME."""
    xavani_home = tmp_path / ".xavani"
    xavani_home.mkdir()
    (xavani_home / "cron").mkdir()
    (xavani_home / "cron" / "output").mkdir()
    (xavani_home / "scripts").mkdir()
    monkeypatch.setenv("XAVANI_HOME", str(xavani_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "XAVANI_DIR", xavani_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", xavani_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", xavani_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", xavani_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.xavani/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("XAVANI_SESSION_PLATFORM") is None
        assert os.environ.get("XAVANI_SESSION_CHAT_ID") is None
        assert os.environ.get("XAVANI_SESSION_CHAT_NAME") is None
