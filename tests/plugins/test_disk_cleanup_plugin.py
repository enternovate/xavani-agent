...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate XAVANI_HOME for each test.

    The global hermetic fixture already redirects XAVANI_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    XAVANI_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "xavani_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "xavani_plugins" not in sys.modules:
        ns = types.ModuleType("xavani_plugins")
        ns.__path__ = []
        sys.modules["xavani_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "xavani_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["xavani_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_xavani_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_xavani_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_xavani_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/xavani-abc/x.log")) is True
...
class TestBundledDiscovery:
    def _write_enabled_config(self, xavani_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from xavani_cli import plugins as pmod
