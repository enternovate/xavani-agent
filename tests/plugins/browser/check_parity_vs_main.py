...
Run from the PR worktree:
    ...
    cd ~/.xavani/xavani-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.xavani/xavani-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.xavani/xavani-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a xavani-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a xavani-agent checkout"
)
...
# Isolated XAVANI_HOME for the config write.
