...
"""Shared fixtures for the xavani-agent test suite.
...
2. **Isolated XAVANI_HOME.** XAVANI_HOME points to a per-test tempdir so
   code reading ``~/.xavani/*`` via ``get_xavani_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".xavani"`` instead of the canonical
   ``get_xavani_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No XAVANI_SESSION_* inheritance** — the agent's current gateway
...
# XAVANI_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_XAVANI_BEHAVIORAL_VARS = frozenset({
    "XAVANI_YOLO_MODE",
    "XAVANI_INTERACTIVE",
    "XAVANI_QUIET",
    ...
    "XAVANI_EXEC_ASK",
    "XAVANI_HOME_MODE",
    "XAVANI_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.xavani/kanban.db instead of the per-test XAVANI_HOME.
    "XAVANI_KANBAN_DB",
    "XAVANI_KANBAN_BOARD",
    "XAVANI_KANBAN_HOME",
    "XAVANI_KANBAN_WORKSPACES_ROOT",
    "XAVANI_KANBAN_LOGS_ROOT",
    "XAVANI_KANBAN_TASK",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and XAVANI_HOME to per-test tempdirs so code that
    reads ``~/.xavani/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral XAVANI_* vars that could change test semantics.
    for name in _XAVANI_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect XAVANI_HOME to a per-test tempdir. Code that reads
    #    ``~/.xavani/*`` via ``get_xavani_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.xavani/*`` via
    #    ``Path.home() / ".xavani"`` instead of ``get_xavani_home()``
    #    is a bug to fix at the callsite.
    fake_xavani_home = tmp_path / "xavani_test"
    fake_xavani_home.mkdir()
    (fake_xavani_home / "sessions").mkdir()
    (fake_xavani_home / "cron").mkdir()
    (fake_xavani_home / "memories").mkdir()
    (fake_xavani_home / "skills").mkdir()
    monkeypatch.setenv("XAVANI_HOME", str(fake_xavani_home))
    ...
    #    ~/.xavani/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import xavani_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_xavani_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "xavani_cli"):
    ...
    # --- xavani_constants._XAVANI_HOME_OVERRIDE — context-local home override ---
    #     get_xavani_home() consults this ContextVar BEFORE the XAVANI_HOME env
    #     var.  If any test (or production path like cron/scheduler profile
    #     switching) sets the override and a later test on the same xdist worker
    #     runs without it being reset, that later test's
    #     ``monkeypatch.setenv("XAVANI_HOME", tmp_path)`` is silently ignored —
    #     all config reads/writes land in the leaked override dir.  This is the
    #     root cause of the intermittent setup-wizard / provider / config tests
    #     failing only under full parallel runs ("reloaded['model'] == ''").
    #     Reset to the unset sentinel so the per-test XAVANI_HOME env is always
    #     authoritative.
    try:
        import xavani_constants as _xc_mod
        _xc_mod._XAVANI_HOME_OVERRIDE.set(_xc_mod._UNSET)
    except Exception:
        pass
    ...
    # --- xavani_cli.config — path-keyed (mtime,size) config caches ---
    # Clear the lookup caches before each test so a prior test's entry can't
    # shadow a later test that writes a fresh config.yaml under the same
    # resolved path.  NOTE: we deliberately do NOT touch
    # ``_LAST_EXPANDED_CONFIG_BY_PATH`` — that is load-bearing (save_config
    # uses it to map expanded secret values back to their ``${ENV}`` templates
    # on write), not a mere perf cache, so clearing/replacing it breaks
    # template-preservation behaviour.
    try:
        from xavani_cli import config as _cfg_mod
        _lock = getattr(_cfg_mod, "_CONFIG_LOCK", None)
    ...
    # --- xavani_cli.model_catalog — process-global curated-model catalog ---
    # provider_model_ids() consults this cache before falling back to the
    # static _PROVIDER_MODELS list.  A test that populates the catalog (e.g.
    # any provider-setup flow) leaks it into later tests that expect the
    # empty-catalog fallback path (test_gmi_provider's
    # test_provider_model_ids_falls_back_to_static_models), which then see the
    # leaked catalog's models instead of the static list.  Reset to the unset
    # state before every test.
    try:
        from xavani_cli import model_catalog as _mc_mod
        _mc_mod._catalog_cache = None
        _mc_mod._catalog_cache_source_mtime = 0.0
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal xavani config dict suitable for unit tests."""
    return {
...
@pytest.fixture(autouse=True)
def _reset_tool_registry_caches(monkeypatch):
    """Clear tool-registry-level caches and bypass TTL for every test.
    ...
    one test's cache lands in another's process.

    We clear BOTH caches AND bypass the 30 s TTL so that monkeypatched
    ``_get_env_config`` / ``find_spec`` values are always consulted fresh.
    """
    try:
        import tools.registry as _tools_registry
        import model_tools as _mt
        _tools_registry.invalidate_check_fn_cache()
        ...
        monkeypatch.setattr(_tools_registry, "_check_fn_cached", _no_ttl_check_fn)
    except ImportError:
        pass
...
# environment and finds the developer's live ``xavani-gateway`` process
...
# correlated with a ``tests/xavani_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> xavani-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart xavani-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting xavani/python patterns are also blocked.
    """
    ...
    def _matches_xavani_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _XAVANI_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_xavani_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch xavani-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m xavani_cli.main".
                if (
                    "xavani" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live xavani-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting xavani/python could hit the live gateway. "
...
# ── Lazy-deps bypass for mocked packages (project-wide) ───────────────────
#
...
# a live pip-install attempt which fails with "install reported success but
# packages still not importable" because the running process can't reload its
# import path without a restart.
#
# This autouse fixture wraps ``ensure`` so that if all packages for a feature
# are already importable (mocked in sys.modules or genuinely installed) we
# skip the lazy-install entirely.  Real missing packages still flow through
# the original logic.


def _install_fal_client_stub_global() -> None:
    """Pre-populate sys.modules with a fal_client stub when not installed.

    image_generation_tool._load_fal_client does ``import fal_client`` after
    the lazy-ensure.  Tests that only exercise the managed-gateway path still
    trigger this import.  A lightweight stub in sys.modules lets both the
    lazy-ensure bypass AND the bare import succeed without a real package.
    """
    if "fal_client" not in sys.modules:
        from unittest.mock import MagicMock as _MM
        _stub = _MM()
        _stub.__name__ = "fal_client"
        sys.modules.setdefault("fal_client", _stub)


_install_fal_client_stub_global()


@pytest.fixture(autouse=True)
def _bypass_lazy_ensure_for_mocked_packages(monkeypatch):
    """Skip lazy-install when packages are already importable (mock or real)."""
    ...
    def _patched_ensure(feature: str, *, prompt: bool = True) -> None:
        try:
            ...
            if specs and all(
                _try_import(_lazy_deps._pkg_name_from_spec(s)) for s in specs
            ):
                return
        except Exception:
            pass
