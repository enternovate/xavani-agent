...
Older versions of ``install.sh`` created ``$command_link_dir/xavani`` as a
symlink to the pip-generated entry point at ``$XAVANI_BIN`` (i.e.
``venv/bin/xavani``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/xavani" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$XAVANI_BIN" "$@"`` then self-recursed and ``xavani`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/xavani``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/xavani\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/xavani"')
    cat_idx = block.find('cat > "$command_link_dir/xavani" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/xavani before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/xavani        <- pip entry point (the one we must preserve)
          local_bin/xavani       <- symlink → ../venv/bin/xavani  (old install)

    Then we run the exact shim-write block from setup_path() with
    ``XAVANI_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/xavani`` still contains its original pip-script body
      * ``local_bin/xavani`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "xavani"
    ...
    shim_path = command_link_dir / "xavani"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/xavani was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/xavani must be replaced with a regular file, not "
