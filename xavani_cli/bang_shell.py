...
def bang_shell_enabled() -> bool:
    ...
    if env_var_enabled("XAVANI_CRON_SESSION"):
        return False
    if (os.getenv("XAVANI_SESSION_PLATFORM") or "").strip():
        return False
    return True
...
def _bang_env() -> dict:
    """Environment for a bang command, with Xavani-managed secrets filtered.
...
def run_bang_command(
    ...
    *,
    ...
    try:
        from xavani_cli._subprocess_compat import windows_hide_flags
    ...
    try:
        ...
        # Ctrl+C interrupts the command, not the Xavani session.
        proc.kill()
        emit("!: interrupted")
        return 130
    finally:
