...
- **gateway profiles**, relaunched through the existing per-profile
  ``xavani_cli.main -p <profile> gateway restart`` command; and
- **``xavani-serve*`` systemd units**, restarted directly through
  ``systemctl``.  ``xavani serve`` is not a gateway profile and has no
  ...
  inventory, so a manually launched or Desktop-owned ``xavani serve`` — which
  has no relaunch authority — can never enter this path.

Serve-unit identity is always ``<scope>/<unit>`` (``user/xavani-serve``,
``system/xavani-serve``).  The two managers can each own a unit of the same
...
_RECOVERY_ENV = "XAVANI_UPDATE_RESTART_RECOVERY"
_GATEWAY_MARKERS = ("_XAVANI_GATEWAY", "XAVANI_GATEWAY", "XAVANI_GATEWAY_MODE")
_PROFILE_RESTART_TIMEOUT = 90
_VERIFY_TIMEOUT = 15
_PROFILE_ID_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")
_SUPERVISOR_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,31}$")
_UNIT_RE = re.compile(r"^xavani-serve(-[a-z0-9][a-z0-9_-]{0,63})?\.service$")
_SERVE_UNIT_PATTERN = "xavani-serve*"
...
def _systemd_unit_candidates(profile: str) -> tuple[str, ...]:
    ...
    if profile == "default":
        return (
            "xavani-gateway.service",
            "gateway.service",
            "gateway-default.service",
        )
    return (
        f"xavani-gateway-{profile}.service",
...
def _systemctl_scopes() -> list[tuple[str, list[str]]]:
    ...
    any Xavani platform helper — importing the freshly pulled tree is exactly
    what aborted the phase that called us.

    Each scope is returned with its label because ``xavani-serve.service`` in
    the user manager and ``xavani-serve.service`` in the system manager are
...
def _listed_serve_units(scope: list[str], *, run: Callable[..., Any]) -> list[str]:
    ...
    for line in (getattr(result, "stdout", "") or "").splitlines():
        ...
        if not parts:
            continue
        # The glob is a systemd pattern, not a name gate: `xavani-serve*` also
        # matches the unrelated `xavani-server.service`. Require the exact
...
def _normalized_skips(
    ...
    Qualified entries (``{"scope": "user", "unit": "xavani-serve"}`` or the
    equivalent ``"user/xavani-serve"`` string) suppress exactly one process.

    A bare ``"xavani-serve"`` carries no scope and therefore cannot say WHICH
...
def restart_serve_units(
    *,
    ...
    """Restart every active ``xavani-serve*`` systemd unit from this process.

    ``xavani serve`` hosts ``tui_gateway.server`` and is restarted by the
    ...
    part-way — systemd lists ``xavani-gateway.service`` before
    ``xavani-serve.service``, so the gateway is typically already done — the
    ...
    or Desktop-owned ``xavani serve`` owns no unit and therefore cannot be
    touched here.

    Every identity in and out of this function is scope-qualified. The user
    manager and the system manager can each own a ``xavani-serve.service``,
...
def _parse_payload(stream) -> tuple[list[str], dict[str, str], bool, list[str]]:
    ...
    if raw_serve is not None:
        ...
        for entry in raw_skip:
            ...
            # shape carries the systemd scope, because `xavani-serve.service`
