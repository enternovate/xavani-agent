...
Pure logic: walks a Xavani config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `xavani doctor` and a future `xavani migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Xavani config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Xavani config and return retirement issues.
...
def apply_migration(
    ...
    from xavani_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `xavani migrate xai` reports nothing to migrate
