...
Backs ``xavani kanban export|import``, the matching ``/boards/{slug}/export``
...
from xavani_cli.archive_safe import (
    archive_root_dirs,
    copy_regular_files,
    make_targz,
    safe_extract_targz,
...
ARCHIVE_FORMAT = "xavani-kanban-board"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = {
            ...
            "xavani_version": str(xavani_version),
            "includes": {
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError(
            "archive is not a Xavani kanban board export (no manifest.json)"
        )
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a Xavani kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this Xavani "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update Xavani and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "xavani_version": manifest.get("xavani_version"),
        },
        "counts": counts,
