"""Offline, non-destructive recovery for a damaged Xavani session database.
...
from xavani_state import SessionDB, _db_opens_cleanly
from xavani_state_common import FTS_STORAGE_VERSION, SCHEMA_VERSION
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``xavani_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``xavani_state._backup_db_file``.
    """
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="xavani-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Xavani process using this profile and retry. "
                "This includes the interactive `xavani` CLI session this "
                ...
                "Xavani process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `xavani` session open, "
...
def _lost_and_found_plausibility_errors(
    ...
    """
    from xavani_cli.session_lost_and_found import _EPOCH_LOW, STUB_TITLE_PREFIX
...
def _recover_via_lost_and_found(
    *,
    ...
    from xavani_cli.session_lost_and_found import (
