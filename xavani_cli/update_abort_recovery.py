...
``xavani update`` performs its fleet restart in the interpreter that started
...
itself lives in :mod:`xavani_cli.update_restart_recovery`.
"""
...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``xavani-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``xavani-serve.service`` can exist in BOTH the user and the system
...
def _recover_gateway_restart_after_abort(
    ...
    *,
    ...
    ``xavani update`` normally performs the fleet restart in the interpreter
    ...
    The same child additionally restarts active ``xavani-serve*`` systemd
    units (#92145).  ``xavani serve`` hosts ``tui_gateway.server`` and is
    ...
    """
    from xavani_cli.update_cmd import _gateway_recovery_partition
    ...
    command = [
        sys.executable,
        "-m",
        "xavani_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["XAVANI_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_XAVANI_GATEWAY", "XAVANI_GATEWAY", "XAVANI_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_stale_serve_runtimes(rows) -> None:
    ...
    terminal naming the responsible process. ``xavani serve`` hosts
    ...
    print(
        "    Restart them before using Xavani again, e.g."
        " `systemctl --user restart xavani-serve.service`"
        " or by relaunching `xavani serve` / the Desktop app."
    )
...
def _abort_recovery_is_complete(
    *,
    ...
    ``gateway_state.json`` / control socket — so a ``xavani serve`` still
    ...
    * no ``xavani-serve*`` unit failed to produce a verified replacement; and
