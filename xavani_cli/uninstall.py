...
"""
Xavani Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.xavani/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Xavani PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing xavani-agent or xavani PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Xavani Agent" comment and following line
                if '# Xavani Agent' in line or '# xavani-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('xavani' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing xavani
                if 'xavani' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the xavani wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "xavani",
        Path("/usr/local/bin/xavani"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains xavani_cli reference)
                content = wrapper.read_text()
                if 'xavani_cli' in content or 'xavani-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``xavani gateway run`` processes
    ...
    try:
        from xavani_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from xavani_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``XAVANI_HOME`` and ``XAVANI_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\xavani\git\cmd``, ``%LOCALAPPDATA%\xavani\git\bin``,
#      ``%LOCALAPPDATA%\xavani\git\usr\bin``, ``%LOCALAPPDATA%\xavani\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\xavani\git\`` and Node to
#      ``%LOCALAPPDATA%\xavani\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``xavani dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\xavani\gateway-service\`` and sometimes
...
def _xavani_path_markers(xavani_home: Path) -> list[str]:
    """Path-entry substrings that identify Xavani-owned User-PATH entries."""
    root = str(xavani_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare xavani-agent install dir.
    markers = [root + "\\xavani-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if XAVANI_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "xavani".  We don't want to catch
    # unrelated entries like "cxavani-foo" or "ephermeral", so we look for
    # backslash-xavani as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(xavani_home: Path) -> list[str]:
    """Strip Xavani-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _xavani_path_markers(xavani_home)
            kept: list[str] = []
...
def remove_xavani_env_vars_windows() -> list[str]:
    """Delete XAVANI_HOME and XAVANI_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("XAVANI_HOME", "XAVANI_GIT_BASH_PATH"):
                try:
...
def _is_default_xavani_home(xavani_home: Path) -> bool:
    """Return True when ``xavani_home`` points at the default (non-profile) root."""
    try:
        from xavani_constants import get_default_xavani_root
        return xavani_home.resolve() == get_default_xavani_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its XAVANI_HOME directory.

    We shell out to ``xavani -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current XAVANI_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m xavani_cli.main` so we don't depend on a `xavani`
    #    wrapper that may be half-removed mid-uninstall.
    xavani_invocation = [_sys.executable, "-m", "xavani_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                xavani_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's XAVANI_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.xavani/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.xavani/ for future reinstall
    """
    project_root = get_project_root()
    xavani_home = get_xavani_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie XAVANI_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_xavani_home(xavani_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Xavani Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {xavani_home / 'config.yaml'}")
    print(f"  Secrets: {xavani_home / '.env'}")
    print(f"  Data:    {xavani_home / 'cron/'}, {xavani_home / 'sessions/'}, {xavani_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their XAVANI_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Xavani data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Xavani code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in xavani_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\xavani\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(xavani_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Xavani-owned PATH entries in User environment")

        log_info("Removing XAVANI_HOME / XAVANI_GIT_BASH_PATH User env vars...")
        removed_env = remove_xavani_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Xavani-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing xavani command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.xavani/, just remove the xavani-agent subdir
            if xavani_home in project_root.parents or project_root.parent == xavani_home:
    ...
    #     under XAVANI_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(xavani_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(xavani_home)
    ...
    # 5. Optionally remove ~/.xavani/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile XAVANI_HOME dirs live under
        ...
        try:
            if xavani_home.exists():
                shutil.rmtree(xavani_home)
                log_success(f"Removed {xavani_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {xavani_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {xavani_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/xavani-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/xavani-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Xavani Agent! ⚕")
    print()
