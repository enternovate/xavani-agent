"""Per-terminal session breadcrumbs for ``xavani -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$XAVANI_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``xavani -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_xavani_home() / "terminal-sessions"
