"""xavani import-agent — import Claude Code / Codex CLI setups into Xavani.

Usage:
    xavani import-agent                       # auto-detect ~/.claude or ~/.codex
    xavani import-agent claude-code           # import from ~/.claude
    xavani import-agent codex                 # import from ~/.codex
    xavani import-agent claude-code --dry-run # preview only, no changes
    xavani import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``xavani claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_xavani.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in XAVANI_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → XAVANI_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in XAVANI_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in XAVANI_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → XAVANI_HOME/skills/codex-imports/<name>/
...
deliberately via ``xavani setup`` or config.yaml.
"""
...
# Same entry delimiter as the Xavani memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `xavani config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`xavani config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_xavani.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Xavani command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Xavani glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Xavani equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if srv.get("url"):
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        xavani_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → XAVANI_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``xavani import-agent`` (invoked from xavani_cli.main)."""
    from xavani_cli.config import get_config_path, load_config, save_config
    from xavani_constants import get_xavani_home
    from xavani_cli.colors import Colors, color
    from xavani_cli.cli_output import (
        print_header,
        print_info,
        print_success,
        print_error,
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: xavani import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: xavani import-agent claude-code   or   xavani import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Xavani — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: xavani import-agent "
    ...
    print_info(f"Target:      {xavani_home}")
    print_info(f"Overwrite:   {'yes' if overwrite else 'no (skip conflicts)'}")
    print_info("Secrets:     never imported — run 'xavani setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: xavani import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'xavani setup' "
               "to configure providers, or add them to ~/.xavani/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from xavani_cli.colors import Colors, color
    from xavani_cli.cli_output import print_header, print_info
