...
"""Implementation of the ``xavani bundles`` CLI subcommand.

Mirrors the structure of ``xavani_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `xavani bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  xavani bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``xavani bundles`` argparse tree.

    Called from ``xavani_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `xavani bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``xavani bundles <subcommand>`` to the right handler."""
