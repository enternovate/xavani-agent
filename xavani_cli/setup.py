...
"""
Interactive setup wizard for Xavani Agent.
...
_DOCS_BASE = "https://enternovate.co.za/xavani-agent/docs"
...
# display_xavani_home imported lazily at call sites (stale-module safety during xavani update)
...
from xavani_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Xavani Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Xavani using environment variables or config commands:")
    print_info("  xavani config set model.provider custom")
    print_info("  xavani config set model.base_url http://localhost:8080/v1")
    print_info("  xavani config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'xavani setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from xavani_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from xavani_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'xavani setup')")


def _print_setup_summary(config: dict, xavani_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'xavani setup' to configure"))
    ...
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (Xavani subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (Xavani Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Xavani), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (Xavani subscription)", True, None))
        ...
        try:
            ...
            from xavani_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `xavani tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from xavani_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via Xavani subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'xavani setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'xavani setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (Xavani subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'xavani setup terminal'"))
    elif managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via Xavani subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via xavani auth spotify — check auth.json, not env vars)
    try:
        from xavani_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'xavani setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {xavani_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('xavani setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('xavani setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('xavani setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('xavani setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('xavani setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('xavani config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('xavani config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('xavani config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('xavani', Colors.GREEN)}              Start chatting")
    print(f"   {color('xavani gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('xavani doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `xavani tools` and `xavani setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``xavani model``)
    ...
    provider added to ``xavani model`` is automatically available here.
    ...
    # Delegate to the shared xavani model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from xavani_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: xavani model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from xavani_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Xavani can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        _prov_names = {
            "nous-api": "Xavani Portal API key",
        ...
        print_info("doesn't currently provide one Xavani can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'xavani setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``xavani model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("Xavani Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    if selected == "nous-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Xavani gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.xavani/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Xavani auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via xavani setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'xavani setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Xavani runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Xavani gateway and bill to your subscription.")
        ...
        print_info("Requires the optional SDK: pip install 'xavani-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'xavani-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into XAVANI_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("XAVANI_MAX_ITERATIONS")
    ...
    print_info("  Run `xavani setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # XAVANI_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Xavani delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `xavani update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://enternovate.co.za/xavani-agent/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under XAVANI_HOME, and print
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Xavani",
            bot_description="Your Xavani agent on Slack",
        )
        target = Path(get_xavani_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `xavani slack manifest --write` anytime to refresh after "
            "Xavani adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Xavani delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Xavani delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  xavani config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Xavani to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from xavani_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://enternovate.co.za/xavani-agent/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://enternovate.co.za/xavani-agent/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  xavani config edit")
    print_info("   Open config in your editor:  xavani config edit")
...
def setup_gateway(config: dict):
    ...
    from xavani_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Xavani from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'xavani setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     xavani config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_xavani_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  xavani gateway")
        else:
            from xavani_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   xavani gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   xavani gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `xavani setup tools` and `xavani tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from xavani_cli.config
    ...
    if section_key == "model":
        ...
        from xavani_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_xavani migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_xavani", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Xavani.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Xavani to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Xavani at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Xavani at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Xavani at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Xavani at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Xavani equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Xavani config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Xavani.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Xavani's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(xavani_home: Path) -> bool:
    ...
    print_info("Xavani can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: xavani claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=xavani_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: xavani claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Xavani configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Xavani config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Xavani (use xavani claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      xavani setup           — full or quick (auto-detected)
      xavani setup model     — just model/provider
      xavani setup tts       — just text-to-speech
      xavani setup terminal  — just terminal backend
      xavani setup gateway   — just messaging platforms
      xavani setup tools     — just tool configuration
      xavani setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Xavani Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Xavani Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Xavani Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, xavani_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Xavani configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'xavani setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(xavani_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Xavani?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, xavani_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {xavani_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'xavani config edit'")
    ...
    _print_setup_summary(config, xavani_home)


def _run_first_time_quick_setup(config: dict, xavani_home, is_existing: bool):
    ...
    the user can customize later via ``xavani setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'xavani setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    xavani setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  xavani setup gateway")
    print()

    _print_setup_summary(config, xavani_home)


def _run_quick_setup(config: dict, xavani_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'xavani setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Xavani to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'xavani setup gateway'.")
