...
on Japanese locales, etc.).  Xavani's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``xavani_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Xavani is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``XAVANI_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("XAVANI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Xavani-managed Git install directories so
    ...
    # already-running shells don't see the change — which means xavani
    ...
    # install without Xavani-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Xavani's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Xavani's
    ``xavani config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Xavani (User env var in Windows Settings, or
    export in a PowerShell profile) and Xavani picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Xavani-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\xavani\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``xavani`` from right after install) retain their old PATH.

    Any subprocess Xavani spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Xavani-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "xavani", "git", "cmd"),
        os.path.join(local_appdata, "xavani", "git", "bin"),
        os.path.join(local_appdata, "xavani", "git", "usr", "bin"),
        # Xavani venv Scripts directory — host of the xavani.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes xavani, but harmless to include.
        os.path.join(local_appdata, "xavani", "xavani-agent", "venv", "Scripts"),
