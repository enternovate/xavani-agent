"""Safe Xavani Console command engine.

This module backs ``xavani console`` and is intentionally narrower than the
full Xavani CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["XavaniConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'xavani doctor'")
        and last.startswith("Run 'xavani setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="xavani", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `xavani "):
        return ""
    return summary
...
# dashboard opens a fresh XavaniConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("xavani_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["XavaniConsoleEngine", list[str]], str]:
    def handler(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "XavaniConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["XavaniConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `xavani {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `xavani {usage}`?",
        )


class XavaniConsoleEngine:
    """Curated line-command executor for Xavani Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Xavani Console does not run shell syntax. Use one supported "
                    "Xavani command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Xavani component status.", _status)
        self.register(("version",), "version", "Show Xavani version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Xavani logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Xavani configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Xavani Console."""

        extracted = {
            "dump": (
                "xavani_cli.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "prompt-size": (
                "xavani_cli.subcommands.prompt_size",
            ...
            "insights": (
                "xavani_cli.subcommands.insights",
            ...
            "security": (
                "xavani_cli.subcommands.security",
            ...
            "import": (
                "xavani_cli.subcommands.import_cmd",
            ...
            "plugins": (
                "xavani_cli.subcommands.plugins",
            ...
            "pairing": (
                "xavani_cli.subcommands.pairing",
            ...
            "webhook": (
                "xavani_cli.subcommands.webhook",
            ...
            "profile": (
                "xavani_cli.subcommands.profile",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Xavani configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "xavani_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("xavani_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "xavani_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("xavani_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("xavani_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Xavani Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Xavani Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`xavani {first}` is not available in Xavani Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Xavani Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Xavani Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Xavani Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Xavani Console.",
            ("skills", "publish"): "`skills publish` is not available in Xavani Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Xavani Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Xavani Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Xavani Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Xavani Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Xavani Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Xavani Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Xavani Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Xavani Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Xavani Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Xavani Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Xavani Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Xavani Console.")
...
def _version(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "version")
    from xavani_cli._startup_fast import print_fast_version_info

    return _capture_output(lambda: print_fast_version_info(check_updates=True))


def _status(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Xavani Console.")
    ...
    from xavani_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from xavani_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _config_set(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from xavani_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: XavaniConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        try:
            from xavani_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
        except ImportError:
            raise ConsoleCommandError("sessions repair needs state repair helpers.")
...
def _profile_status(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="xavani_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from xavani_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from xavani_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from xavani console")
...
def _cron_resume(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    ...
    from cron.jobs import AmbiguousJobReference, rearm_oneshot, resume_job
    from xavani_time import now as _xavani_now

    try:
        if ns.at or ns.run_now:
            job = rearm_oneshot(ns.job, _xavani_now().isoformat() if ns.run_now else ns.at)
        else:
...
def _cron_run(_engine: XavaniConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``xavani console`` REPL."""
    ...
    engine = XavaniConsoleEngine()
    if interactive:
        print("Xavani Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("xavani> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
