"""``xavani cron`` subcommand parser.

Extracted verbatim from ``xavani_cli/main.py:main()`` — same arguments, same
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        ...
        help=(
            ...
            "state stays visible in `xavani cron list`. Omit = failures "
            "follow --deliver."
        ),
    )
    ...
    cron_create.add_argument(
        ...
        help=(
            ...
            "~/.xavani/scripts/ that runs each tick BEFORE the agent. "
    ...
    cron_edit.add_argument(
        ...
        dest="monitor_script",
        help=(
            "Set/replace the monitor source script (see `xavani cron create "
