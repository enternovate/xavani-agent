"""``xavani sync`` subcommand parser — Skill Sync.

Cloned from ``xavani_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    xavani sync status                 show gate/opt-in/head state
    xavani sync pull                   pull and materialize opted-in skills
    xavani sync push                   push opted-in skills
    xavani sync now                    reconcile: pull then push
    xavani sync enable <skill>         opt a skill into sync
    xavani sync disable <skill>        opt a skill out of sync
    xavani sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    xavani sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Xavani token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  xavani sync status            what is synced, and from where\n"
            "  xavani sync enable my-skill   include a skill in your sync\n"
            "  xavani sync now               pull, then push\n"
            "  xavani sync propose my-skill  share a skill with your team\n"
        ),
