"""``xavani skills`` subcommand parser.
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_trust = skills_subparsers.add_parser(
        "trust",
        help="Trust a project so its repo-local skills (./.xavani/skills, ./.agents/skills) load",
    )
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `xavani update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `xavani update` reports as user-modified and skips. "
            "Use `xavani skills diff <name>` to see changes and `xavani skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`xavani update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `xavani update`. Pass --sync to re-seed now."
        ),
    )
