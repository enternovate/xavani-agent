"""``xavani dashboard`` / ``xavani serve`` subcommand parsers.
...
``xavani_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `xavani dashboard` / `xavani serve` cmdlines and SIGTERM them directly —
    # the same path `xavani update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Xavani web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Xavani web server processes and exit",
    )
...
def build_serve_parser(
    *,
    ...
    parser = argparse.ArgumentParser(
        prog="xavani serve",
        description=(
            "Run the Xavani backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    # Backward-compat shim: older Xavani desktop app shells (<= 0.15.x) spawn the
    # backend as `xavani dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Xavani couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Xavani Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Xavani backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Xavani backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI."
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://xavani.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
