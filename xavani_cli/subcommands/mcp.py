"""``xavani mcp`` subcommand parser.

Extracted from ``xavani_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    # ── Catalog (Xavani-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `xavani mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Xavani-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `xavani mcp install n8n`)",
    )
