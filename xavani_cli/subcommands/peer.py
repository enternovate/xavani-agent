"""``xavani peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Xavani gateway (any machine: homelab, Spark, Xavani
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    xavani peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>
    xavani peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    xavani peer dm spark/researcher "..."      # named profile (multiplexed peer)
    xavani peer run spark --idempotency-key ticket-123 < /tmp/long-task.txt
    xavani peer status spark run_abc123
...
``xavani -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.xavani/.env`` as
  ``XAVANI_PEER_<NAME>_KEY``.
...
def _peer_key_env(name: str) -> str:
    return f"XAVANI_PEER_{name.upper().replace('-', '_')}_KEY"
...
def _request(
    ...
    *,
    ...
    from xavani_cli.urllib_security import open_credentialed_url
    data = json.dumps(body).encode("utf-8") if body is not None else None
    request_headers = {
        "Authorization": f"Bearer {key}",
        "Content-Type": "application/json",
        "User-Agent": "xavani-peer-dm",
    }
    if headers:
    ...
    # The peer URL is user-registered (``xavani peer add``); a redirect to a
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's xavani-agent, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}."
            ) from exc
        raise
    # Real api_server wraps the row: {"object": "xavani.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (xavani peer dm <peer>[/<agent>] ...)")
...
def _resolve_peer_target(target: str) -> tuple[str, str | None, dict, str]:
    ...
    if not isinstance(peer, dict) or not peer.get("url"):
        raise LookupError(f"No peer named '{peer_name}'. Run: xavani peer list")
    key = _peer_secret(peer_name)
    if not key:
        raise PermissionError(
            f"No API key for peer '{peer_name}'. Set it: xavani peer add {peer_name} "
            f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.xavani/.env)"
        )
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.xavani/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  xavani peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.xavani/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: xavani peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    print("Unknown peer action. See: xavani peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Xavani gateways)",
        description=(
            "Register other Xavani gateways as peers and message their agents. "
            "'xavani peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'xavani -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.xavani/.env."
        ),
        epilog=(
            "Examples:\n"
            "  xavani peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  xavani peer list\n"
            '  xavani peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  xavani peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  xavani peer run spark --idempotency-key ticket-123 < long-task.txt\n"
            "  xavani peer status spark run_abc123\n"
            "  xavani peer stop spark run_abc123\n"
            "  xavani peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.xavani/.env)")
    ...
    status_p.add_argument("run_id", help="Run ID returned by 'xavani peer run'")
    status_p.add_argument(
        "--json", action="store_true", default=False, help="Emit a JSON result"
    )
    ...
    stop_p.add_argument("run_id", help="Run ID returned by 'xavani peer run'")
    stop_p.add_argument(
        "--json", action="store_true", default=False, help="Emit a JSON result"
    )
