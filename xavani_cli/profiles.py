...
"""
Profile management for multiple isolated Xavani instances.

Each profile is a fully independent XAVANI_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.xavani/profiles/<name>/`` by default.

The "default" profile is ``~/.xavani`` itself — backward compatible,
zero migration needed.

Usage::

    xavani profile create coder          # fresh profile + bundled skills
    xavani profile create coder --clone  # also copy config, .env, SOUL.md, skills
    xavani profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    xavani -p coder chat                 # or via flag
    xavani profile use coder             # set as sticky default
    xavani profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See xavani_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.xavani``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   xavani-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `xavani profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `xavani
...
# `xavani skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Xavani infrastructure directories that only the default profile
         (``~/.xavani``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_xavani_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.xavani) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "xavani-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "xavani", "default", "test", "tmp", "root", "sudo",
})

# Xavani subcommands that cannot be used as profile names/aliases
_XAVANI_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the xavani root, NOT to the current XAVANI_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where XAVANI_HOME points outside
    ``~/.xavani``, profiles live under ``XAVANI_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_xavani_home() / "profiles"


def _get_default_xavani_home() -> Path:
    """Return the default (pre-profile) XAVANI_HOME path.

    In standard deployments this is ``~/.xavani``.
    In Docker/custom deployments where XAVANI_HOME is outside ``~/.xavani``
    (e.g. ``/opt/data``), returns XAVANI_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_xavani_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Xavani installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its XAVANI_HOME directory.

    The directory is built through :func:`gateway.security.validate_path`,
    whose return value (the canonical, traversal-checked Path) is
    propagated to the caller. The earlier implementation discarded that
    return value and re-joined ``profiles_root / canon`` unchecked, which
    made CodeQL's data-flow analysis (correctly) flag every downstream
    path operation as ``py/path-injection``: the validation barrier was
    ...
    return validate_path(canon, profiles_root, allow_create=True)
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, xavani subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _XAVANI_SUBCOMMANDS:
        return f"'{canon}' conflicts with a xavani subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``xavani profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Xavani config
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``xavani update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.xavani)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.xavani/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `xavani update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`xavani profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `xavani update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `xavani profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches XAVANI_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``xavani profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "XAVANI_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.xavani).\n"
            "To remove everything, use: xavani uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set XAVANI_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("XAVANI_HOME")
    try:
        os.environ["XAVANI_HOME"] = str(profile_dir)
        from xavani_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["XAVANI_HOME"] = old_home
        elif "XAVANI_HOME" in os.environ:
            del os.environ["XAVANI_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.xavani/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: xavani profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from XAVANI_HOME.

    Returns ``"default"`` if XAVANI_HOME is not set or points to ``~/.xavani``.
    Returns the profile name if XAVANI_HOME points into ``~/.xavani/profiles/<name>``.
    Returns ``"custom"`` if XAVANI_HOME is set to an unrecognized path.
    """
    from xavani_constants import get_xavani_home
    xavani_home = get_xavani_home()
    resolved = xavani_home.resolve()

    default_resolved = _get_default_xavani_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.xavani itself — its parent is ~/ and its
        # directory name is ".xavani", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: xavani profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.xavani itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.xavani). "
            "Specify a different name: xavani profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="xavani_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"xavani.{old_name}"
    new_host = f"xavani.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_xavani_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a XAVANI_HOME path string.

    Called early in the CLI entry point, before any xavani modules
    are imported, to set the XAVANI_HOME environment variable.
    """
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    from xavani_constants import named_profile_is_deleted
    ...
    default_env = _get_default_xavani_home() / ".env"
    ...
    for entry in sorted(profiles_root.iterdir()):
        ...
        try:
            if default_env.is_file():
                ...
                env_path.write_text(
                    "# Per-profile secrets for this Xavani profile.\n"
