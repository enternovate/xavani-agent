...
"""Shell completion script generation for xavani CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_xavani_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Xavani Agent bash completion
# Add to ~/.bashrc:
#   eval "$(xavani completion bash)"

_xavani_profiles() {{
    local profiles_dir="$HOME/.xavani/profiles"
...
_xavani_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_xavani_profiles)" -- "$cur"))
        return
    fi
...
complete -F _xavani_completion xavani
"""
...
_xavani_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.xavani/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.xavani/profiles 2>/dev/null)}}")
    fi
...
_xavani() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_xavani_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'xavani command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Xavani Agent fish completion",
        "# Add to your config:",
        "#   xavani completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __xavani_profiles",
        "    echo default",
        "    if test -d $HOME/.xavani/profiles",
        "        ls $HOME/.xavani/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c xavani -f -s p -l profile"
        " -d 'Profile name' -xa '(__xavani_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__xavani_profiles)' -d 'Profile name'"
                )
