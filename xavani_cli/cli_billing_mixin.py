...
Import discipline mirrors ``xavani_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``xavani_cli.auth``): ``webbrowser.open()`` returns ``True`` even when
        ...
        if not url:
            return False
        try:
            from xavani_cli.auth import _can_open_graphical_browser, _is_remote_session
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from xavani_cli.nous_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_apply(self, state, action, idempotency_key=None, *, allow_stepup=True):
        ...
        from xavani_cli.nous_billing import (
            BillingError,
            BillingTransient,
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `xavani portal` and allow Remote Spending, then re-run /subscription.")
            return
        confirm_choices = [
        ...
        try:
            from xavani_cli.auth import step_up_nous_billing_scope
        ...
        try:
            from xavani_cli import nous_billing as _nb
    ...
    def _billing_overview(self, state):
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Xavani Agent page to add funds here.",
            )
            return
    ...
    def _billing_require_admin(self, state) -> bool:
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Xavani Agent page before adding funds.",
            )
            return False
        return True
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `xavani portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `xavani portal` to log in again.")
            ...
            print("  Remote spending is off for this account — a billing admin can turn it on from the portal's Xavani Agent page.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `xavani portal` and allow Remote Spending, then retry.")
            return
        confirm_choices = [
            ("yes", "Allow Remote Spending", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Xavani Agent page, then run /topup again.")
        ...
        from xavani_cli.nous_billing import BillingError, post_charge
