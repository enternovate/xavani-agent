...
Both the interactive CLI (``cli.py``) and the gateway call into this module so
the pending-review UX (list / approve / reject / diff / mode) lives in one
place. Each caller owns only its surface concerns: formatting the returned
text and, for the gateway, persisting config + evicting the cached agent on a
mode change.
...
def _set_approval(subsystem: str, rest: List[str], set_mode_fn) -> str:
    ...
    if set_mode_fn is None:
        ...
        return (f"To change the {subsystem} approval gate, run:\n"
                f"  xavani config set {subsystem}.write_approval {val}")
    try:
        set_mode_fn(enabled)
    except Exception as e:
