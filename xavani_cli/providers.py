...
"""
Single source of truth for provider identity in Xavani Agent.
...
2. **Xavani overlays** — transport type, auth patterns, aggregator flags,
...
# -- Xavani overlay ----------------------------------------------------------
# Xavani-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class XavaniOverlay:
    """Xavani-specific provider metadata layered on top of models.dev."""
...
XAVANI_OVERLAYS: Dict[str, XavaniOverlay] = {
    "openrouter": XavaniOverlay(
    ...
    "xai-oauth": XavaniOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": XavaniOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="XAVANI_QWEN_BASE_URL",
    ),
    "google-gemini-cli": XavaniOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": XavaniOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": XavaniOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": XavaniOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": XavaniOverlay(
    ...
    "kimi-for-coding": XavaniOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": XavaniOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": XavaniOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": XavaniOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": XavaniOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": XavaniOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": XavaniOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": XavaniOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": XavaniOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": XavaniOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": XavaniOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": XavaniOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": XavaniOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": XavaniOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": XavaniOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": XavaniOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": XavaniOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": XavaniOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "xavani", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Xavani overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Xavani overlay
    ...
    overlay = XAVANI_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + xavani extra
    ...
    if overlay is not None:
        # Xavani-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in XAVANI_OVERLAYS
    if provider == "bedrock":
