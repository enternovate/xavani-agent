...
"""`xavani checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.xavani/checkpoints/``.  Actions:

    xavani checkpoints               # same as `status`
    xavani checkpoints status        # total size, project count, breakdown
    xavani checkpoints list          # per-project checkpoint counts + workdir
    xavani checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    xavani checkpoints clear [-f]    # nuke the entire base (asks first)
    xavani checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    xavani checkpoints
    xavani checkpoints prune --retention-days 3 --max-size-mb 200
    xavani checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: xavani checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``xavani checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `xavani checkpoints` → status
