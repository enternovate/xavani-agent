"""Image-authored deployment provenance for immutable Xavani runtimes.

The published image bakes ``/etc/xavani/image-provenance.json`` outside both
``$XAVANI_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/xavani/image-provenance.json")
