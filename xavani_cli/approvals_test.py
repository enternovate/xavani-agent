"""``xavani approvals test`` — dry-run approval verdict for a command.
...
def evaluate_command(command: str, env_type: str = "local") -> dict:
    ...
    _detection_variants = getattr(approval, "_command_detection_variants", None)
    if callable(_detection_variants):
        variants = list(_detection_variants(command))
    else:
        _normalize = getattr(approval, "_normalize_command_for_detection", None)
        if callable(_normalize):
            try:
                variants = [_normalize(command)]
            except Exception:
                variants = [command]
        else:
            variants = [command]
    ...
    _skip_guards = getattr(approval, "_should_skip_container_guards", None)
    if callable(_skip_guards) and _skip_guards(env_type):
        return result(
            "allow",
    ...
    _match_deny = getattr(approval, "_match_user_deny_rule", None)
    deny_pattern = _match_deny(command) if callable(_match_deny) else None
    ...
    # 5. Yolo / approvals.mode=off bypass.
    if (getattr(approval, "_YOLO_MODE_FROZEN", False)
    ...
    # 6. Permanent command_allowlist.
    _match_allow = getattr(approval, "_command_matches_permanent_allowlist", None)
    if callable(_match_allow) and _match_allow(command):
        return result(
            "allow",
...
def approvals_test_command(args) -> int:
    """Handle ``xavani approvals test <command...>``. Returns the exit code."""
    ...
    if words and words[0] == "--":
        words = words[1:]
    if not words:
        print("usage: xavani approvals test [--env-type TYPE] [--json] -- <command...>")
        return EXIT_USAGE
    command = " ".join(words)
