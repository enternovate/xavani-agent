...
``CommandDef.execute`` (xavani_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/xavani_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``xavani_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_egress(ctx: CommandContext) -> CommandReply:
    ...
    try:
        from xavani_cli.proxy_cli import format_status_text
    except ImportError:
        return CommandReply("Egress proxy status unavailable in this build.")
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_xavani_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: xavani bundles create <name> --skill <s1> --skill <s2>\n"
