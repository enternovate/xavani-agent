...
"""
Status command for xavani CLI.

Shows the status of all Xavani Agent components.
"""
...
from xavani_cli.auth import AuthError, resolve_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``xavani config``'s
...
def show_status(args):
    """Show status of all Xavani Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Xavani Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: xavani model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: xavani auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: xavani auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: xavani model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from xavani_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'xavani-agent[vercel]')"
    ...
    try:
        from xavani_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   xavani gateway")
    ...
    jobs_file = get_xavani_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_xavani_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'xavani doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'xavani setup' to configure", Colors.DIM))
    print()
