...
This module is imported by ``xavani_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no xavani_cli.config,
...
``xavani_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(xavani_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect XAVANI_HOME?"""
    active_profile = os.path.join(xavani_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    xavani_home = os.environ.get("XAVANI_HOME", "").strip()
    if xavani_home:
        return xavani_home
    return os.path.join(os.path.expanduser("~"), ".xavani")
...
def container_mode_may_be_active() -> bool:
    ...
    xavani_home = os.environ.get("XAVANI_HOME", "").strip()
    if xavani_home:
        if os.path.exists(os.path.join(xavani_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(xavani_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(xavani_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".xavani")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect XAVANI_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``xavani --version`` output (also used by /version).
    ...
    ``xavani version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from xavani_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from xavani_cli import __release_date__, __version__
    ...
    try:
        from xavani_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``xavani --version`` before the heavy import wall.
    ...
    container. Termux keeps the XAVANI_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("XAVANI_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
