...
the tool registry or launching Xavani (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``xavani-cli`` to the non-existent ``xavani``. ``resolve_toolset('xavani')``
...
from xavani_cli.platforms import PLATFORMS
from xavani_cli.toolset_scope import toolset_allowed_for_platform


def _platform_default_toolset(platform: object) -> str:
    info = PLATFORMS.get(platform)
    return info.default_toolset if info is not None else f"xavani-{platform}"
...
def _platform_default_is_valid(
    ...
    # though their synthesized xavani-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``xavani-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``xavani`` instead of ``xavani-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if not isinstance(raw, list):
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `xavani tools` to configure explicitly."
            )
            if platform_valid_count == 0:
                warnings.append(
                    ...
                    f"Run `xavani tools` to reconfigure."
                )
            continue
        names = raw
        for name in names:
        ...
        if platform_valid_count == 0:
            ...
            warnings.append(
                ...
                f"tools on this platform. Run `xavani tools` to reconfigure."
            )

    if valid_count == 0:
        warnings.append(
            "platform_toolsets resolves to zero valid toolsets — the agent will "
            "have no tools. Run `xavani tools` to reconfigure."
        )
    return warnings
