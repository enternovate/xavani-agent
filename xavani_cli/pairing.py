...
Usage:
    xavani pairing list              # Show all pending + approved users
    xavani pairing approve <platform> <code>  # Approve a pairing code
    xavani pairing revoke <platform> <user_id> # Revoke user access
    xavani pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle xavani pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: xavani pairing {list|approve|revoke|clear-pending}")
        print("Run 'xavani pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.xavani/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'xavani pairing list' to see pending codes.\n")
