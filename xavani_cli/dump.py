...
"""
Dump command for xavani CLI.

Outputs a compact, plain-text summary of the user's Xavani setup
...
from xavani_cli.config import get_xavani_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``xavani dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from xavani_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(xavani_home: Path) -> int:
...
def _cron_summary(xavani_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = xavani_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"xavani_home:      {display_xavani_home()}")
    ...
    toolsets = config.get("toolsets", ["xavani-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(xavani_home)}")
    lines.append(f"  skills:             {_count_skills(xavani_home)}")
