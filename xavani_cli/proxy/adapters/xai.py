...
# xAI's public API is OpenAI-compatible for the endpoints Xavani commonly
# uses. The Responses endpoint is included because Xavani' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Xavani-managed OAuth credentials."""

    auth_hint = "xavani auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`xavani auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`xavani auth reset xai-oauth` or re-authenticate with "
                    "`xavani auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `xavani auth add xai-oauth --type oauth`."
            )
