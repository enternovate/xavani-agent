...
``<root>`` is the **shared Xavani root** (the parent of any active
...
``xavani -p <profile>`` joins the same board as the dispatcher that
...
the profile's Xavani home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``XAVANI_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``XAVANI_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``xavani kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.xavani``. In Docker / custom
deployments where ``XAVANI_HOME`` points outside ``~/.xavani`` (e.g.
``/opt/xavani``), ``<root>`` is ``XAVANI_HOME``. Legacy env-var
overrides still work:

* ``XAVANI_KANBAN_DB`` — pin the database file path directly.
* ``XAVANI_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``XAVANI_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``XAVANI_KANBAN_DB``,
``XAVANI_KANBAN_WORKSPACES_ROOT``, and ``XAVANI_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/xavani-kanban-v1-spec.pdf`` for the full
...
# ``XAVANI_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``XAVANI_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("XAVANI_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``xavani-agent``
...
def kanban_home() -> Path:
    """Return the shared Xavani root that anchors the kanban board.

    Resolution order:

    1. ``XAVANI_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_xavani_root()``, which already returns ``<root>``
       when ``XAVANI_HOME`` is ``<root>/profiles/<name>``, and returns
       ``XAVANI_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``XAVANI_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("XAVANI_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from xavani_constants import get_default_xavani_root
    return get_default_xavani_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``xavani kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``XAVANI_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``xavani kanban boards
    ...
    """
    env = os.environ.get("XAVANI_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``xavani kanban boards switch <typo>`` returns an error
...
def board_dir(board: Optional[str] = None) -> Path:
    ...
    br = boards_root()
    # Return the validated Path directly instead of discarding it; the
    # earlier ``validate_path(...); return br / slug`` pattern was correct
    # at runtime but invisible to CodeQL's data-flow analyser, which then
    # flagged every downstream read/write as ``py/path-injection``.
    return validate_path(slug, br, allow_create=True)
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``XAVANI_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("XAVANI_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``XAVANI_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("XAVANI_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``xavani kanban log`` unambiguous even when multiple
...
def remove_board(slug: str, *, archive: bool = True) -> dict:
    ...
    if archive:
        ...
        ts = int(time.time())
        # Route every candidate archive target through validate_path so
        # the slug (user-supplied at board create time) cannot traverse
        ...
        while target.exists():
            target = validate_path(
                f"{normed}-{ts}-{suffix}", archive_root, allow_create=True
            )
            suffix += 1
        d.rename(target)
...
class Task:
    ...
    # within an agent loop that propagated ``XAVANI_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``XAVANI_SESSION_ID``. NULL
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``XAVANI_KANBAN_DB`` env → ``XAVANI_KANBAN_BOARD`` env →
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # See xavani_state._WAL_INCOMPAT_MARKERS for detection logic.
            from xavani_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``xavani kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``XAVANI_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from xavani_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``xavani --skills ...`` alongside the built-in
    ...
    # `xavani --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``xavani kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `xavani kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``xavani kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``xavani kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``xavani kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Xavani
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``xavani kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Xavani profile.
    ...
    if not rows:
        return False
    try:
        from xavani_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Xavani profile.
        # `_default_spawn` invokes ``xavani -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Xavani
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `xavani kanban tail` — without
...
def _module_xavani_argv() -> list[str]:
    """Return the interpreter-bound Xavani CLI invocation."""
    # ``xavani_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``xavani`` package — there is no
    # ``xavani`` package to import.
    return [sys.executable, "-m", "xavani_cli.main"]


def _absolute_xavani_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Xavani shim."""
...
def _xavani_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Xavani executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_xavani_argv()
    return [_absolute_xavani_path(path)]


def _resolve_xavani_argv() -> list[str]:
    """Resolve the ``xavani`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$XAVANI_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("xavani")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\xavani.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m xavani_cli.main`` — fallback for setups where
       Xavani is launched from a venv and the ``xavani`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_xavani_bin`` for the same reason. Kept
    local (not imported from gateway) because ``xavani_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("XAVANI_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _xavani_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _xavani_path_argv(resolved_env_bin)
        return _module_xavani_argv()

    xavani_bin = _safe_which_no_cwd("xavani") if _IS_WINDOWS else shutil.which("xavani")
    if xavani_bin:
        return _xavani_path_argv(xavani_bin)
    return _module_xavani_argv()


def _kanban_worker_skill_available(xavani_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``xavani -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset XAVANI_HOME means the worker falls back to the default root
    # home (``~/.xavani``), which ships the bundled skill.
    base = _Path(xavani_home) if xavani_home else (_Path.home() / ".xavani")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``xavani -p <profile> chat -q ...`` subprocess.
    ...
    ``XAVANI_KANBAN_DB`` / ``XAVANI_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject XAVANI_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `xavani -p <name>` the
    # _apply_profile_override() runs *before* xavani_constants is imported.
    # If XAVANI_HOME is absent from the child's env, get_xavani_home() falls
    # back to Path.home() / ".xavani" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from xavani_cli.profiles import resolve_profile_env
    try:
        env["XAVANI_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via XAVANI_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # XAVANI_HOME never had profiles created.
        pass
    if task.tenant:
        env["XAVANI_TENANT"] = task.tenant
    env["XAVANI_KANBAN_TASK"] = task.id
    env["XAVANI_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["XAVANI_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["XAVANI_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["XAVANI_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`xavani -p <name>`
    # rewrites XAVANI_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_xavani_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["XAVANI_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["XAVANI_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["XAVANI_KANBAN_BOARD"] = resolved_board
    # XAVANI_PROFILE is the author the kanban_comment tool defaults to.
    # `xavani -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["XAVANI_PROFILE"] = profile_arg

    cmd = [
        *_resolve_xavani_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped XAVANI_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("XAVANI_HOME")):
    ...
    # `xavani kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`xavani` executable not found on PATH. "
            "Install Xavani Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``xavani kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled XAVANI_PROFILE values like "xavani-system"
...
def worker_log_path(task_id: str, *, board: Optional[str] = None) -> Path:
    ...
    ``task_id`` is user-supplied (URL parameter on the dashboard
    endpoint); we route the constructed path through
    :func:`gateway.security.validate_path` so traversal segments and
    NUL/CR/LF byte smuggling are refused and the returned Path is
    canonicalised + contained in the logs directory.
    """
    logs_dir = worker_logs_dir(board=board)
    return validate_path(f"{task_id}.log", logs_dir, allow_create=True)
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Xavani root exists

    Reads profile paths directly so this module has no import dependency on
    ``xavani_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from xavani_constants import get_default_xavani_root
        default_root = get_default_xavani_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``xavani kanban assignees`` for the terminal.
