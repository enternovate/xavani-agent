...
Add, remove, or reorder entries here — both `xavani setup` and
`xavani` provider-selection will pick up the change automatically.
"""
...
_XAVANI_USER_AGENT = f"xavani-cli/{_XAVANI_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `xavani model`
    flow without maintaining a separate static list.
    """
    from xavani_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $XAVANI_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $XAVANI_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Xavani refreshes the cache from
...
# ---------------------------------------------------------------------------
# Xavani Portal free-model helper
# ---------------------------------------------------------------------------
# The Xavani Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Xavani Portal account tier detection
# ---------------------------------------------------------------------------

def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    """Fetch the user's Xavani Portal account/subscription info.
...
def partition_nous_models_by_tier(
    ...
    """Split Xavani models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Xavani that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Xavani that doesn't ship
...
def check_nous_free_tier() -> bool:
    """Check if the current Xavani Portal user is on a free (unpaid) tier.
    ...
    try:
        from xavani_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# ---------------------------------------------------------------------------
# Xavani Portal recommended models
#
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the Xavani Portal's curated recommended-models payload.
...
# this list:  xavani model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `xavani model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `xavani model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``xavani model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``xavani auth add openai-codex`` without
    ``xavani model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    xavani-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Xavani Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from xavani_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — xavani-agent
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated Xavani Portal model-id list.
    ...
    """
    try:
        from xavani_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from xavani_constants import AI_GATEWAY_BASE_URL
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Xavani Portal).
    """
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return xavani-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; xavani's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Xavani Portal pricing.

    The Xavani inference ``/v1/models`` endpoint exposes pricing without
    ...
    """
    try:
        from xavani_cli.auth import resolve_nous_runtime_credentials
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Portal base_url typically looks like https://inference-api.enternovate.co.za/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``xavani model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from xavani_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Xavani, etc. all support /models)
...
def _resolve_static_model_alias(
    ...
    try:
        from xavani_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Xavani' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``xavani_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Xavani should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``xavani auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.xavani/.env``.
    ...
    """
    try:
        from xavani_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from xavani_cli.auth import read_credential_pool
        from xavani_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Xavani),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Xavani release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Xavani release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        # Try live Xavani Portal /models endpoint
        try:
            from xavani_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _XAVANI_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Xavani' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _XAVANI_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from xavani_constants import get_xavani_home
    return get_xavani_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Xavani will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Xavani cannot verify the model name."
