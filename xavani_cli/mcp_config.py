...
"""
MCP Server Management CLI — ``xavani mcp`` subcommand.

Implements ``xavani mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.xavani/config.yaml under the ``mcp_servers`` key.
"""
...
from xavani_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_xavani_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from xavani_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # xavani_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  xavani mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  xavani mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  xavani mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_xavani_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: xavani mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_xavani_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `xavani mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  xavani mcp add <name> --url <endpoint>')
        _info('  xavani mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `xavani mcp remove` + `xavani mcp add` to reconfigure auth.")
        return
...
# ─── xavani mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'xavani mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``xavani mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("xavani mcp serve                              Run as MCP server")
        _info("xavani mcp add <name> --url <endpoint>        Add an MCP server")
        _info("xavani mcp add <name> --command <cmd>         Add a stdio server")
        _info("xavani mcp add <name> --preset <preset>       Add from a known preset")
        _info("xavani mcp remove <name>                      Remove a server")
        _info("xavani mcp list                               List servers")
        _info("xavani mcp test <name>                        Test connection")
        _info("xavani mcp configure <name>                   Toggle tools")
        _info("xavani mcp login <name>                       Re-authenticate OAuth")
        print()
