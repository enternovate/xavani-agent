...
``cli.py``'s ``XavaniCLI`` class. ``XavaniCLI`` inherits ``CLICommandsMixin`` so
...
from xavani_constants import display_xavani_home, is_termux as _is_termux_environment
...
try:
    from xavani_cli.browser_connect import launch_chrome_debug
except ImportError:
    from types import SimpleNamespace as _SimpleNamespace
    from xavani_cli.browser_connect import (
        DEFAULT_BROWSER_CDP_PORT as _DEFAULT_BROWSER_CDP_PORT,
        try_launch_chrome_debug as _try_launch_chrome_debug,
    )

    def launch_chrome_debug(port=_DEFAULT_BROWSER_CDP_PORT, system=None):
        return _SimpleNamespace(
            launched=bool(_try_launch_chrome_debug(port, system)), hint=None
        )
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after Xavani did)
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Xavani changed (checkpoint baseline)
        ...
        try:
            ...
            print("  (._.) /diff unavailable: working-diff tool not in this build.")
            return
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Xavani hasn't edited any files here yet.")
            return
    ...
    def _handle_export_command(self, command: str):
        ...
        try:
            from xavani_cli.profiles import get_profile_export_path
        except ImportError:
            from pathlib import Path as _Path

            def get_profile_export_path(name, *, timestamp=None):
                return (
                    _Path(os.path.expanduser("~/.xavani"))
                    / "profile-exports"
                    / f"{name}.tar.gz"
                )
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `xavani profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: xavani -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `xavani journey`).
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from xavani_cli.clipboard import (
    ...
    def _handle_profile_command(self):
        ...
        try:
            from xavani_cli.slash_exec import CommandContext, execute_command
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        # "Is `xavani gateway` running?" is the correct diagnosis, and the
        ...
        # CLI printed "Is `xavani gateway` running?" while the gateway was
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        try:
            ...
            def _sync_process_session_id(_session_id):
                return None
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `xavani sessions list` to see available sessions.")
            return
        ...
        try:
            ...
            def is_user_originated_turn(m):
                return isinstance(m, dict) and m.get("role") == "user" and not m.get("display_kind")
        ...
        # same directory as a startup `xavani -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``xavani -w``).

        ``prune`` is the same attended reclaim as ``xavani worktree prune``
        (xavani_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"prune", "gc", "clean"}:
            ...
            from xavani_cli import worktree_gc
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `xavani -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `xavani -w`.
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        All resolution/persistence goes through xavani_cli.personality —
        the single owner of personality state on every surface.
        """
        from xavani_cli.personality import (
        ...
        if len(parts) > 1:
            ...
            if not name:
                ...
                try:
                    from xavani_cli.config import cfg_get, read_raw_config
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        try:
            ...
            from xavani_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
        except ImportError:
            print("(x_x) Pet subsystem unavailable in this build.")
            return
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        try:
            ...
            from xavani_cli.pets import _set_active
        except ImportError:
            print("(x_x) Pet hatching unavailable in this build.")
            return
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from xavani_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from xavani_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_plan_command(self, cmd: str):
        ...
        ``.xavani/plans/`` via ``write_file``. No engine, no model-tool
    ...
    def _handle_init_command(self, cmd: str):
        ...
        """
        from xavani_cli.init_command import build_init_prompt_for_cwd
    ...
    def _handle_background_command(self, cmd: str):
        ...
        try:
            from cli import AIAgent, set_approval_callback, set_secret_capture_callback, set_sudo_password_callback
        except ImportError:
            _cprint("  (._.) Background tasks unavailable in this build.")
            return
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        try:
            ...
            _cprint("  Bundle subsystem unavailable in this build.")
            return
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Xavani-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `xavani cron`.
        """
        from cli import _DIM, _RST, _cprint
        from xavani_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from xavani_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Xavani process runs — use `xavani cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        try:
            from xavani_cli.goals import parse_contract
        except ImportError:
            parse_contract = None

        if parse_contract is None:
            headline, contract = arg, None
        else:
        ...
        _has_contract = getattr(state, "has_contract", None)
        _contract_active = bool(callable(_has_contract) and _has_contract())
        if _contract_active:
        ...
        _cprint(
            ...
            f"{' against the contract above' if _contract_active else ''}. "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        try:
            from xavani_cli.goals import draft_contract
        except ImportError:
            draft_contract = None
        ...
        try:
            contract = draft_contract(objective) if draft_contract else None
        except Exception as exc:
        ...
        _cprint(f"  ⊙ Goal set ({state.max_turns}-turn budget): {state.goal}")
        _has_contract = getattr(state, "has_contract", None)
        if callable(_has_contract) and _has_contract():
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        from xavani_cli.loops import dispatch_loop_command
        ...
        if result.get("created"):
            try:
                from xavani_cli.loops import goal_blocks_loop_tick
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="xavani_prompt_")
        try:
    ...
    def _handle_focus_command(self, cmd_original: str) -> None:
        ...
        from xavani_cli.focus_view import (
            FOCUS_CONFIG_KEY,
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from xavani_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from xavani_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from xavani_cli.focus_view import format_hidden_line
    ...
    def _handle_approvals_command(self, cmd_original: str) -> None:
        ...
        from cli import _cprint
        from xavani_cli.approval_mode import run_approval_mode_command
    ...
    def _handle_busy_command(self, cmd: str):
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                ...
                behavior = "Enter will redirect the current run while Xavani is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        try:
            from xavani_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
        except ImportError:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Xavani' hotword listener.
