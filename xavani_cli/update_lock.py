"""Cross-process mutual exclusion for in-flight Xavani updates.

Three different surfaces can start an update of the same install tree:

* ``xavani update`` from a terminal,
* the dashboard's Update button (``POST /api/xavani/update`` →
  ``_spawn_xavani_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``xavani-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``xavani update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <XAVANI_HOME>/.xavani-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``xavani update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Xavani is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``xavani-setup`` binary under ``~/.xavani`` is only refreshed by a full
  installer run (``copy_self_to_xavani_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Xavani is still running") on every
  GUI update forever, with no Xavani process actually running.
"""
...
MARKER_NAME = ".xavani-update-in-progress"

# Set by an orchestrating updater (the Tauri `xavani-setup --update` flow) to
# its own pid before spawning `xavani update` as a child stage. The parent
...
HANDOFF_PID_ENV = "XAVANI_UPDATE_HANDOFF_PID"
...
# to show "Xavani is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Xavani home (never the context-local profile override):
    the Rust updater resolves ``$XAVANI_HOME`` or the platform default, and the
    ...
    return get_process_xavani_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``xavani update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``xavani-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Xavani update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `xavani update` as a stage): we run under ITS claim
