...
"""xavani hooks — inspect and manage shell-script hooks.

Usage::

    xavani hooks list
    xavani hooks test <event> [--for-tool X] [--payload-file F]
    xavani hooks revoke <command>
    xavani hooks doctor

Consent records live under ``~/.xavani/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.xavani/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``xavani hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: xavani hooks {list|test|revoke|doctor}")
        print("Run 'xavani hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.xavani/config.yaml.")
        print("See `xavani hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `xavani hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `xavani hooks test` and `xavani hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Xavani wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `xavani hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `xavani hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `xavani hooks doctor`.")
