"""``xavani worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``xavani_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    xavani worktree list             # audit: verdict + reason per tree
    xavani worktree prune            # reap safe trees + merged branches
    xavani worktree prune --dry-run  # show the plan, change nothing
    xavani worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `xavani worktree prune`."
        )
