"""Windows ConPTY bridge for the `xavani dashboard` chat tab.

Drop-in counterpart to ``xavani_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``xavani_cli.web_server`` consumes: ``spawn``, ``read``,
