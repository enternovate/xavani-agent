...
"""Slash command definitions and autocomplete for the Xavani CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("branch", "Branch the current session (explore a different path)", "Session",
               aliases=("fork",), args_hint="[name] [at N]"),
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("stash", "Save and restore draft prompts across sessions", "Session",
               args_hint="[name <text> | list | show <name> | load <name> | rm <name>]",
               subcommands=("list", "show", "load", "rm")),
    ...
    CommandDef("plan", "Toggle read-only plan mode (only read-only tools allowed)", "Configuration",
               cli_only=True, args_hint="[on|off]"),
    ...
    CommandDef("busy", "Control what Enter does while Xavani is working", "Configuration",
    ...
    CommandDef("skills", "Search, install, inspect, or manage skills",
               ...
               args_hint="<query> | search <query> | install <name> | approve|reject <name>"),
    CommandDef("memory", "Review pending memory writes or toggle the memory write-approval gate",
               "Tools & Skills", cli_only=True,
               args_hint="[pending|approve <id>|reject <id>|approval <on|off>]",
               subcommands=("pending", "approve", "reject", "approval")),
    CommandDef("bundles", "List skill bundles (aliases /<name> for multiple skills)",
               "Tools & Skills"),
    CommandDef("cron", "Manage scheduled tasks", "Tools & Skills",
               cli_only=True, args_hint="[list|add|create|edit|pause|resume|run|remove]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("curator", "Background skill maintenance (status, run, pin, archive, list-archived)",
               "Tools & Skills", args_hint="[status|run|pause|resume|pin|unpin|restore|list-archived]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore", "list-archived")),
    CommandDef("kanban", "Multi-profile collaboration board (tasks, links, comments)",
               "Tools & Skills", args_hint="<subcommand> — init, boards, create, show, claim, complete, stats, ...",
    ...
    CommandDef("guidelines", "Research guidelines enforcement (list, show, check)",
               "Tools & Skills", args_hint="[list|show|check]",
               subcommands=("list", "show", "check")),
    CommandDef("reload", "Reload .env variables into the running session", "Tools & Skills",
               cli_only=True),
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.xavani/skills/ for newly installed or removed skills",
    ...
    CommandDef("cost", "Show per-turn token meter and estimated session cost", "Info"),
    ...
    CommandDef("update", "Update Xavani Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`xavani_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/xavani`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``xavani skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``xavani skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``xavani skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/xavani <verb>`` subcommand.
    ...
    can still reach them via ``/xavani <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/xavani`` is always reserved as the first entry so the
    legacy ``/xavani <subcommand>`` form keeps working for anything that
    ...
    # Reserve /xavani as the catch-all top-level command.
...
def slack_app_manifest(request_url: str = "https://xavani-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /xavani handler.

    Maps both canonical names and aliases so /xavani bg do stuff works
    the same as /xavani background do stuff.

    Plugin-registered slash commands are included so ``/xavani <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from xavani_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
