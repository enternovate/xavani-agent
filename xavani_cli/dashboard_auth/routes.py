...
from xavani_cli.dashboard_auth import (
    get_provider,
    list_providers,
    list_session_providers,
)
from xavani_cli.dashboard_auth.audit import AuditEvent, audit_log
from xavani_cli.dashboard_auth.base import (
    InvalidCodeError,
    InvalidCredentialsError,
    ProviderError,
)
from xavani_cli.dashboard_auth.cookies import (
    clear_pkce_cookie,
...
from xavani_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``XAVANI_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /xavani`` (Mission Control deploys) — we
    ...
    from xavani_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _provider_pkce_segments(cookie_payload: dict[str, str]) -> dict[str, str]:
    ...
    :class:`~xavani_cli.dashboard_auth.base.LoginStart` (values are
    ...
    wire encoding.
    """
    flat = cookie_payload.get("xavani_session_pkce", "")
    return dict(
...
def _prefix(request: Request) -> str:
    ...
    ``xavani_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from xavani_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_native_authorize(
    ...
    from xavani_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from xavani_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from xavani_cli.dashboard_auth import list_session_providers
    from xavani_cli.dashboard_auth.base import RefreshExpiredError
