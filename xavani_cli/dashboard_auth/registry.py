...
def _xavani_home_key(path: str | Path | None = None) -> str:
    candidate = Path(path) if path is not None else get_xavani_home()
    raw = str(candidate)
...
def _merged(scope: Optional[str] = None) -> dict[str, DashboardAuthProvider]:
    ...
    providers.update(_scoped_providers.get(scope or _xavani_home_key(), {}))
    return providers
