...
Profile-aware location: ``$XAVANI_HOME/logs/dashboard-auth.log``.
...
from ``xavani_constants`` or other xavani_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$XAVANI_HOME/logs/dashboard-auth.log``.

    Uses ``xavani_constants.get_xavani_home()`` (a leaf module — no import
    ...
    return get_xavani_home() / "logs" / "dashboard-auth.log"
