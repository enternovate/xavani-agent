...
Implements the Xavani Agent gateway commands that extend the base Xavani CLI:
...
Each command handler follows the pattern used by ``xavani_cli/commands.py``:
...
from xavani_cli.commands import CommandDef
...
# Map canonical name -> handler callable. The handler signature is
# ``(args: str, cli: Optional[Any] = None) -> str`` — the return value is
# the markup-formatted user response. ``cli`` is the active ``XavaniCLI``
# instance, threaded through so handlers can inspect/mutate the live
# session (rare; only ``gateway-up``/``-down`` currently do).
_OAGHandler = Callable[..., str]
OAG_COMMAND_HANDLERS: Dict[str, _OAGHandler] = {}


def _register_handler(name: str) -> Callable[[_OAGHandler], _OAGHandler]:
    """Decorator that registers a handler for the given canonical command name."""

    def wrapper(fn: _OAGHandler) -> _OAGHandler:
...
def oag_gateway_up(args: str, cli=None) -> str:
    ...
    thread, bypassing the old Xavani messaging gateway.
    """
...
def oag_policy_add(args: str, cli=None) -> str:
    ...
    # Resolve the user-supplied path under the user's home directory using
    # the central path-validation helper, which rejects traversal attempts
    # (``../``), absolute escapes, NUL/CRLF smuggling, and symlinks that
    # point outside the home tree. The startswith-based check this
    # replaced was vulnerable to the ``~/-evil`` string-prefix bypass.
    from gateway.security import PathValidationError, validate_path

    try:
        policy_path = validate_path(path_raw, Path.home())
    except PathValidationError as exc:
        return f"  [bold #FF3366]✗ {exc}[/]"
    except FileNotFoundError:
        return f"  [bold #FF3366]✗ File not found: {path_raw!r}[/]"
...
def register_oag_commands() -> None:
    ...
    command namespace so the XavaniCLI dispatcher can resolve them.
    ...
    try:
        ...
        _register_into_xavani_commands()


def _register_into_xavani_commands() -> None:
    """Fallback registration that injects OAG commands directly into the
    Xavani commands structures so they are discoverable.
    """
    try:
        from xavani_cli.commands import COMMAND_REGISTRY, _COMMAND_LOOKUP, _build_command_lookup
        ...
        logger.warning("Failed to register OAG commands into Xavani dispatch: %s", exc)
