"""``xavani sessions`` command — extracted from ``xavani_cli/main.py``.
...
Helpers that stay in ``xavani_cli.main`` (``get_xavani_home``,
...
``xavani_cli.main.<name>`` keep reaching this code path, and so imports stay
...
#: Default age floor for `xavani sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`xavani sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from xavani_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        from xavani_state import DEFAULT_DB_PATH, _db_opens_cleanly

        try:
            from xavani_state import repair_state_db_schema
        except ImportError:
            repair_state_db_schema = None
        ...
        if repair_state_db_schema is None:
            print("✗ In-place repair is unavailable in this build.")
            print("  Use offline recovery into a NEW database instead:")
            print(f"    xavani sessions recover --source {db_path} \\")
        ...
        if report.get("repaired"):
            ...
            print(f"    xavani sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from xavani_cli.session_recovery import (
    ...
    if action == "import":
        from xavani_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        try:
            from xavani_state import workspace_key as _ws_key
        except ImportError:
            def _ws_key(s):
                return ""
        ...
        from xavani_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        def _redact(data):
            if not args.redact or data is None:
                return data
            try:
                from xavani_cli.session_export_md import redact_session_data
            except ImportError:
                return data
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            try:
                from xavani_cli.session_export_html import (
                    generate_html_export,
                    generate_multi_session_html_export,
                )
            except ImportError:
                print("HTML export is unavailable in this build.")
                return
        ...
        if args.format == "trace":
            ...
            try:
                ...
                print("Trace upload is unavailable in this build.")
                db.close()
                return
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_xavani_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        try:
            from xavani_cli.session_export_md import (
                append_manifest_entry,
                verify_export_file,
                write_session_markdown,
            )
        except ImportError:
            print("Markdown export is unavailable in this build.")
            return
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_xavani_home() / "session-exports"
        ...
        # `xavani sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `xavani sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `xavani sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `xavani sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: xavani sessions pin <session_id>"
            )
            return
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`xavani sessions optimize` later to reclaim freed space.)")
