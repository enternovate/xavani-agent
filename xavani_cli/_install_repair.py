...
- ``xavani_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``xavani_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``xavani_cli.main._recover_core_update_marker_locked`` — the historical
...
from xavani_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (xavani_cli.main's version lives behind imports)."""
...
def _venv_is_relocatable(venv_dir: Path) -> bool:
    ...
    uv writes the flag; ``xavani_cli.managed_uv`` builds its replacement
...
def ensure_windows_bin_launchers(
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    ...
    # every xavani_cli.main process start (right after the profile
...
def _read_user_path_raw() -> tuple[list[str], int]:
    ...
    (same discipline as ``xavani_cli.uninstall``). Only called on Windows.
    """
    import winreg
