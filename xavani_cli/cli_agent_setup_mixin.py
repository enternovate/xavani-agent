"""Agent-construction and session-resume display methods for ``XavaniCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.xavani/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``XavaniCLI``. ``self.*``
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    ...
    ``xavani chat -q`` runs one turn without ever building the
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``XavaniCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
            ...
            if isinstance(_primary_exc, AuthError):
                ...
                for _fb in _fb_chain:
                    ...
                    if not _fb_provider or not _fb_model:
                        continue
                    try:
                        from xavani_cli.fallback_config import resolve_entry_api_key
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("   Run 'xavani model' to choose a provider, or "
                      "'xavani setup' for first-time setup.")
                return False
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'xavani model' or 'xavani setup' any time.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'xavani model' to retry.")
        return False
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        from xavani_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (xavani sessions list).",
                        file=sys.stderr,
                    )
                else:
        ...
        try:
            ...
            from xavani_constants import partial_update_hint
