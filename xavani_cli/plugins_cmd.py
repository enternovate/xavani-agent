...
"""``xavani plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.xavani/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Xavani subprocess resolution: :func:`shutil.which` first,
...
def _plugins_dir() -> Path:
    ...
    plugins = get_xavani_home() / "plugins"
...
def _sanitize_plugin_name(name: str, plugins_dir: Path) -> Path:
    ...
    # Single canonicalising step. ``validate_path`` resolves symlinks,
    # collapses ``..`` segments, refuses NUL/CR/LF bytes, and confirms
    # containment inside ``plugins_dir`` using ``Path.is_relative_to``;
    # the returned Path is the only authoritative target the caller
    # should write to. Previously we called ``validate_path`` for its
    # side-effect and then rebuilt the path again unchecked — invisible
    # to CodeQL's data-flow analyser.
    target = validate_path(name, plugins_dir, allow_create=True)
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.xavani/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from xavani_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_xavani_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_xavani_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_xavani_home()}/.env later)[/dim]")
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.xavani/plugins``.
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Xavani.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `xavani plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Xavani plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `xavani plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  xavani gateway restart[/dim]")
    console.print()
...
def _plugin_exists(name: str) -> bool:
    ...
    Routes the user-supplied ``name`` through
    :func:`gateway.security.validate_path` against each candidate base
    directory. If the name fails containment (NUL/CR/LF, ``..``, etc.)
    we return False without ever touching the filesystem with the raw
    string — which both protects against traversal and lets CodeQL
    follow the validation barrier.
    """
    from gateway.security import PathValidationError

    # Installed: directory name or manifest name match in user plugins dir
    user_dir = _plugins_dir()
    if user_dir.is_dir():
        try:
            candidate = validate_path(name, user_dir, allow_create=True)
        ...
        for child in user_dir.iterdir():
            ...
            if manifest.get("name") == name:
                return True
    # Bundled: <repo>/plugins/<name>/ (or XAVANI_BUNDLED_PLUGINS on Nix).
    from xavani_cli.plugins import get_bundled_plugins_dir
    repo_plugins = get_bundled_plugins_dir()
    if repo_plugins.is_dir():
        try:
            bundled_candidate = validate_path(name, repo_plugins, allow_create=True)
        except PathValidationError:
            return False
        if bundled_candidate.is_dir() and (
            (bundled_candidate / "plugin.yaml").exists()
            or (bundled_candidate / "plugin.yml").exists()
        ):
            return True
    return False
...
def _discover_all_plugins() -> list:
    ...
    user types into ``xavani plugins enable <key>``. For category-namespaced
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] xavani plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] xavani plugins")
    console.print("[dim]Enable/disable:[/dim] xavani plugins enable/disable <name>")
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.xavani/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.xavani/plugins/<name>``."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.xavani/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def plugins_command(args) -> None:
    """Dispatch xavani plugins subcommands."""
