"""Xavani update pipeline — extracted from ``xavani_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``xavani_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``xavani_cli.main`` reference — so existing call sites and test monkeypatches
that target ``xavani_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``xavani_cli.main``.
...
Imports are one-way: ``xavani_cli.main`` imports this module, never the reverse
...
try:
    from xavani_constants import venv_python_path
except ImportError:
...
def _m():
    """Lazy ``xavani_cli.main`` reference.

    Lets callers keep patching ``xavani_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``xavani_cli.main`` -> ``xavani_cli.update_cmd`` stays one-way
    at import time.
    """
    from xavani_cli import main
...
#: ``_purge_stale_xavani_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        "xavani_cli",
        "xavani_cli.main",
        "xavani_cli.update_cmd",
        "xavani_cli.xavani_logging",
    }
)


def _purge_stale_xavani_modules() -> None:
    """Evict every cached Xavani module after the checkout changed in-place.

    ``xavani update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from xavani_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``xavani_cli.gateway`` does
    ``from xavani_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Xavani
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Xavani module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Xavani modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``xavani update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``xavani update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``xavani_cli.config_defaults``,
    ``xavani_cli.config``, and ``xavani_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``xavani_cli._subprocess_compat`` and
    ``xavani_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "xavani_cli.config_defaults",
        "xavani_cli.config",
        "xavani_cli.config_migrations",
        "xavani_cli._subprocess_compat",
        "xavani_cli.dashboard_procs",
    ):
...
def _run_config_check_fresh() -> tuple:
    ...
    from xavani_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from xavani_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``xavani update`` historically
    ...
    caller): scope config reads/writes via the context-local XAVANI_HOME
    ...
    try:
        from xavani_constants import (
            get_process_xavani_home,
            reset_xavani_home_override,
            set_xavani_home_override,
        )
        from xavani_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_xavani_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_xavani_home_override(entry)
            try:
                ...
                reset_xavani_home_override(token)
    except Exception as exc:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'xavani config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'xavani config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'xavani config migrate' manually to configure."
                )
                response = "n"
    ...
    # context-local XAVANI_HOME override (never os.environ — other
    ...
    try:
        try:
            from xavani_cli.backup import restore_cron_jobs_if_emptied
        except ImportError:
            restore_cron_jobs_if_emptied = None
    ...
    try:
        try:
            from xavani_cli.backup import restore_config_model_settings_if_rewritten
        except ImportError:
            restore_config_model_settings_if_rewritten = None
    ...
    try:
        try:
            from xavani_cli.backup import restore_cron_jobs_all_profiles
        except ImportError:
            restore_cron_jobs_all_profiles = None
    ...
    try:
        try:
            from xavani_cli.backup import restore_config_model_settings_all_profiles
        except ImportError:
            restore_config_model_settings_all_profiles = None
...
# Critical files that Xavani must be able to import immediately after an
...
def _prune_orphan_rescue_refs(
    ...
    ``refs/xavani-update-backups/orphan-<branch>-<ts>-<sha>``. A rescue ref
    ...
    try:
        list_result = subprocess.run(
            git_cmd + [
                ...
                "--format=%(refname)",
                "--sort=refname",
                f"refs/xavani-update-backups/orphan-{branch}-*",
            ],
            cwd=cwd,
            capture_output=True,
            text=True, encoding="utf-8", errors="replace",
        ...
        if max_age_days > 0:
            ...
            prefix = f"refs/xavani-update-backups/orphan-{branch}-"
            for ref in refs:
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``xavani.exe`` has
    ...
    Skipping is safe because Xavani pins its editable finder to a *static*
...
def _validate_python_files_syntax(
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="xavani-syntax-check-") as tmpdir:
...
def _critical_module_import_failures(
    ...
    ``_venv_core_imports_healthy``): ``xavani update`` can be driven by a
    ...
    """
    try:
        from xavani_constants import FIRST_PARTY_MODULE_ROOTS
    except ImportError:
        FIRST_PARTY_MODULE_ROOTS = None

    import secrets

    marker = f"__XAVANI_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        # The root set is injected from xavani_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('xavani_') or %r:\n"
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `xavani update`.
    ...
    try:
        ...
        return
    db_path = get_xavani_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `xavani sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    xavani sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  xavani sessions optimize-storage")
    print(
...
def _reload_process_scan_modules() -> None:
    ...
    ``from xavani_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``xavani_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.xavani-update-staging"
    backup = f"{dst}.xavani-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `xavani update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``xavani_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``xavani update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && xavani update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("XAVANI_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== xavani-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Xavani launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["xavani.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Xavani")
    print("  Desktop, a gateway, or another xavani REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `xavani` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``xavani.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.xavani-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".xavani-update-staging", ".xavani-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `xavani update`.")
...
def _post_update_sqlite_runtime_status():
    ...
    from xavani_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Xavani runtime."""
    ...
    print(
        "  Rebuild the Xavani venv with a uv-managed Python, restart Xavani, "
        "then verify with `xavani doctor`."
    )
    return False
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Xavani process the updater's drain
...
def _print_update_summary(
    *,
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `xavani desktop` to retry the desktop rebuild.")
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `xavani "
                "update` again; if SQLite is unchanged, rebuild the Xavani venv "
                "with a uv-managed Python, restart Xavani, then verify with "
                "`xavani doctor`."
            )
    else:
...
def _write_gateway_update_exit_code(ok: bool) -> None:
    path = get_xavani_home() / ".update_exit_code"
    try:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    try:
        from xavani_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    except ImportError:
        _foreign_db_holder_pids = None
        verify_sqlite_integrity = None
    from xavani_cli.sqlite_safe_read import LiveConnectionError, offline_file_access

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (xavani gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Xavani) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    """
    try:
        from xavani_cli.backup import _quick_snapshot_root
        try:
            from xavani_cli.backup import verify_sqlite_integrity
        except ImportError:
            verify_sqlite_integrity = None
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    try:
        try:
            from xavani_cli.backup import _sibling_profile_homes
        except ImportError:
            _sibling_profile_homes = None
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `xavani update "
            f"--branch {branch}`, or update against main with `xavani update`."
        )
        _m().sys.exit(1)
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for xavani-agent itself
        ...
        try:
            for item in entries:
                ...
                # Xavani.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `xavani update` to retry; if the agent won't start, "
            "reinstall from https://enternovate.co.za/xavani-agent"
        )
        _m().sys.exit(1)
    finally:
    ...
    try:
        from xavani_cli.managed_uv import ensure_uv, update_managed_uv
    except ImportError:
        ensure_uv = None
        update_managed_uv = None
    ...
    if uv_bin:
        ...
        try:
            from xavani_cli.managed_uv import managed_python_env
        except ImportError:
            managed_python_env = None
    ...
    if not import_ok:
        ...
        print("  Re-run `xavani update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: xavani skills list-modified  "
    ...
    try:
        try:
            from xavani_cli.model_catalog import seed_cache_from_checkout
        except ImportError:
            seed_cache_from_checkout = None
    ...
    try:
        from xavani_cli.update_receipt import finalize_update_receipt
...
_AUTOSTASH_NAME_PREFIX = "xavani-update-autostash-"

#: Age past which a leftover ``xavani-update-autostash-*`` entry is called out
...
def _warn_orphaned_update_autostashes(git_cmd: list[str], cwd: Path) -> int:
    ...
    Xavani never drops one automatically.
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``xavani update --keep-stash`` (the desktop updater's mode): the
...
def _reject_unsafe_stash_restore(
    ...
    print()
    print("✗ Restored local changes made the Xavani agent unexecutable.")
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Xavani couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Xavani couldn't drop "
    ...
    *,
    ...
    if not has_upstream:
        ...
        if assume_yes or (
            ...
            print(
                "  Add it later with: git remote add upstream https://github.com/Enternovate/xavani-agent.git"
            )
            return False
...
# ``fleet_restart_pending`` lives under XAVANI_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """XAVANI_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_xavani_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from xavani_cli.build_info import get_code_identity
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    """
    try:
        from xavani_cli.update_receipt import read_latest_receipt
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `xavani update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `xavani update` or `xavani gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every xavani-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        _m()._purge_stale_xavani_modules()
    except Exception:
        pass
    try:
    ...
    try:
        ...
        if is_windows():
            try:
                from xavani_cli import gateway_windows
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``xavani update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: xavani gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "xavani.exe"
    lines = ["✗ Another xavani.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Xavani Desktop, exit any open `xavani` REPLs, and")
    lines.append("  stop the gateway (`xavani gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `xavani update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    try:
        from xavani_cli._scan_venv_blockers import _is_pausable_gateway
    except ImportError:
        _is_pausable_gateway = None
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``xavani update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Xavani Desktop backend child, an unrelated ``xavani`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``xavani tools``."""
    try:
        from xavani_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Xavani Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``xavani tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Xavani Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Xavani Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `xavani update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `xavani update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Xavani' editable-install extras or
    ...
    try:
        from xavani_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from xavani_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$XAVANI_HOME/bin/uv``, but on Termux the official
    ...
    """
    try:
        from xavani_cli.managed_uv import resolve_uv
    except ImportError:
        resolve_uv = None
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `xavani update` is exactly the step
...
def _npm_lockfile_changed(xavani_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `xavani
    ...
    try:
        ...
        cache_file = xavani_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(xavani_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    xavani update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `xavani update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from xavani_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `xavani update`.")
            failed = []
            if any(
    ...
    # Xavani profile using this checkout. Keep one per-checkout cache under the
    # shared Xavani root rather than rerunning npm once per named profile.
    shared_xavani_root = get_default_xavani_root()
    ...
    # `xavani update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_xavani_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `xavani update`.")
        return list(labels)
    ...
    try:
        from xavani_constants import with_xavani_node_path
    except ImportError:
        with_xavani_node_path = None

    nixos_env = with_xavani_node_path(_m()._nixos_build_env())
    ...
    # `xavani update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_xavani_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.xavani/logs/update.log`` only, never the terminal.

    During ``xavani update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    from xavani_cli.update_contract import (
    ...
    from xavani_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            try:
                from xavani_cli.banner import _github_compare_behind
            except ImportError:
                _github_compare_behind = None
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``xavani-acp`` launcher next to the ``xavani`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``xavani update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``xavani-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Xavani as not installed even when it is.

    The shim simply delegates to the sibling ``xavani`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``xavani`` / ``xavani-acp``
    launchers into the managed binary dir ``$XavaniHome\bin`` and puts THAT
    ...
    ``xavani_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``xavani-acp`` is already present next to the
    ``xavani`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        xavani_cmd = bin_dir / "xavani"
        acp_cmd = bin_dir / "xavani-acp"
        try:
            if not (xavani_cmd.is_file() or xavani_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of XAVANI_HOME under
      ``backups/`` (restorable via ``xavani import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from xavani_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from xavani_constants import
        # get_xavani_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from xavani_cli.config import get_xavani_home as _get_home
        ...
        try:
            try:
                from xavani_cli.backup import create_pre_update_snapshots_all_profiles
            except ImportError:
                create_pre_update_snapshots_all_profiles = None
            ...
            if _sibling_snaps:
                ...
                try:
                    from xavani_cli.update_receipt import record_step
    ...
    # Human-readable size
    from xavani_cli.sizefmt import format_bytes
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``xavani update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``xavani update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run xavani from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.xavani-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".xavani-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The xavani.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m xavani_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``xavani
    ...
    try:
        ...
        # fetch cmdline/cwd lazily for plausible Python/uv/Xavani candidates.
    ...
    for proc in proc_iter:
        ...
        if not is_holder and not (
            ...
            or name_low in {"uv.exe", "uvx.exe", "xavani.exe"}
        ):
            continue
        ...
        # this venv's path, or a `-m xavani_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "xavani_cli.main" in cmdline_low:
            try:
        ...
        # the `-m xavani_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``xavani_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``xavani.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``xavani`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `xavani` launch will complete it")
...
def _xavani_holder_subcommand(cmdline: str) -> str | None:
    """The actual Xavani SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``xavani_cli.main`` / ``xavani(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("xavani_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("xavani", "xavani.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``xavani dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Xavani processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Xavani backend (if the Desktop app is open, close it)",
        "dashboard": "  ← xavani dashboard (stop it: xavani dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _xavani_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the Xavani desktop app / other Xavani terminals, then re-run:"
    )
    lines.append("    xavani update")
    lines.append("  (or use `xavani update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``xavani update``
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    A chat agent can launch plain ``xavani update`` through its terminal tool.
    ...
    try:
        from xavani_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print("  Otherwise, run `xavani update` from a separate terminal.")
    return True
...
def _ledger_manual_serve_holders(
    ...
    Xavani spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from xavani_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    try:
        ...
        if scripts_dir is not None:
            for name in ("xavani.exe", "xavani"):
                candidate = scripts_dir / name
                if candidate.is_file():
                    xavani = str(candidate)
                    break
    except Exception:
        xavani = None
    if xavani is None:
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (xavani serve --host <ip> --port <port>)."
        )
    try:
        from xavani_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning xavani-setup, and the update-in-progress marker
    ...
    the update with "Xavani is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Xavani backend (``xavani_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.xavani-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "xavani_cli.main" in argv_low and (
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # value round-trips through pid_is_xavani at kill time. (Reading
...
def _ledger_reapable_backend_pids(
    ...
    (``xavani_cli.process_identity``). A holder qualifies when ALL of:
    ...
    """
    try:
        from xavani_cli.process_identity import (
            REAPABLE_PURPOSES,
            ledger_entries,
            spawner_is_dead,
        )
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Xavani ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``xavani.exe``) is open, NOTHING legitimate
    ...
    - Only Xavani backends (``xavani_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``xavani.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(
    ...
    ``.xavani-runtime`` interpreter child alive and holding the install open
    (#70026). Best effort; never raises.
    """
    from gateway.status import get_process_start_time
    from xavani_cli._subprocess_compat import pid_is_xavani, windows_hide_flags
    ...
    for entry in pids:
        ...
        try:
            if expected_start_time is None:
                ...
                continue
            if not pid_is_xavani(
                pid,
                expected_start_time=expected_start_time,
            ):
                logger.debug(
                    "Skipping taskkill of non-Xavani or changed PID %s",
                    pid,
                )
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``xavani serve`` / ``xavani dashboard`` argv.
    ...
    """
    if "xavani_cli.main" not in (cmdline or "").lower():
        return False
    return _xavani_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    xavani.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Xavani...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m xavani_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: xavani gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``xavani gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from xavani_cli import gateway_windows
        from xavani_cli.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``xavani-gateway*.service``/``xavani-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("xavani-serve")`` alone would also accept the
        # unrelated ``xavani-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "xavani-gateway.service"
            or unit.startswith("xavani-gateway-")
            or unit == "xavani-serve.service"
            or unit.startswith("xavani-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``xavani-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``xavani-serve*`` units (#83438) don't, so sending them
    ...
    (``xavani-gateway-helper`` is fine — profile units are
    ``xavani-gateway-<profile>`` — but ``xavani-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "xavani-gateway" or svc_name.startswith("xavani-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from xavani_cli.gateway import is_macos
    ...
    print("    xavani gateway status")
    if any(not name.startswith("ai.xavani.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.xavani.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: xavani gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: xavani gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.xavani.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``xavani_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``xavani-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"xavani-gateway-{profile}",
        f"ai.xavani.gateway-{profile}",
        f"gateway-{profile}",
    }


# Abort recovery lives in its own bounded module (review on #96235). Re-exported
# here because `xavani_cli.main` and the update flow below address these names
# through `update_cmd`.
try:
    from xavani_cli.update_abort_recovery import (  # noqa: E402
    ...
    _abort_recovery_is_complete = None
    _qualified_serve_skips = None
    _recover_gateway_restart_after_abort = None
    _serve_unit_recovery_available = None
    _surviving_pre_update_serve_runtimes = None
    _warn_stale_serve_runtimes = None
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``xavani_cli.gateway`` in the failing
    ...
    unrecoverable.  The fresh child runs a separate ``xavani-serve*`` systemd
    ...
    ``xavani serve``: it records no spawner, so it reads as ``manual-serve``.
        ...
        for runtime in getattr(plan, "runtimes", ()) or ():
            ...
            if kind == "gateway":
                ...
                if supervisor == "desktop":
                    ...
                    # `xavani serve` sets neither XAVANI_SPAWN nor
                    # XAVANI_PARENT_PID, so it lands here as "manual-serve".
                    # Unit-backed serve runtimes are recovered by the
                    # fresh child's systemd pass, which enumerates
                    # `xavani-serve*` from systemd instead of trusting this
                    ...
                    reason = (
                        ...
                        " unit pass when it owns a xavani-serve* unit, else"
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``xavani_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``xavani update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``xavani-setup.exe``) executes
    ``$XAVANI_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_xavani_home()) / "bootstrap-cache"
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    if relaunched or unmapped_relaunched:
        ...
        if not ready_pids:
            ...
            print("    Recover with: xavani gateway restart")
            raise RuntimeError(
...
def _portable_git_candidates() -> list:
    ...
    The Xavani-managed PortableGit tree lives under the SHARED root
    (``<root>/git/...``), not the profile-scoped XAVANI_HOME
    (``<root>/profiles/<name>``), so a profile-scoped ``xavani update`` must
    ...
    candidates = []
    try:
        for root in (get_default_xavani_root(), Path(get_xavani_home())):
            candidates.append(
...
def _locate_real_git() -> Optional[Path]:
    ...
    Xavani-managed PortableGit copy; accept the first candidate that runs and
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``xavani update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``xavani update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `xavani desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "xavani_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Xavani-managed Node on PATH: when
    # `xavani update` runs inside the desktop updater chain (Desktop →
    # xavani-setup → xavani update), the shell PATH customizations are lost,
    ...
    build_env = with_xavani_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `xavani desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from xavani_constants import display_xavani_home as _dhh
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv that was ever touched by ``sudo pip`` / ``sudo xavani`` contains
    root-owned files (classically ``*.dist-info/INSTALLER``). A later normal
    ``xavani update`` then dies mid-mutation inside ``uv pip install -e .``
    ("Permission denied (os error 13)") with ``venv/bin/xavani`` already
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Xavani broken.")
    print("  This usually happens after running xavani or pip with sudo. Offending paths:")
...
def _drain_or_signal_gateway_for_update(
    ...
    """Decide how ``xavani update`` hands a running gateway over to new code.
    ...
    1. **Gateway is an ancestor of this process** — THREE-WAY DEADLOCK BREAK
       (#100179). When ``xavani update`` runs INSIDE the gateway's own
       process tree (the xavani-auto-update cron job is the canonical case),
       waiting for that gateway to exit is a circular wait::

           gateway  waits on all in-flight work units (#77184)
             └─ cron agent session waits on the `xavani update` process
                  └─ `xavani update` waits on the gateway to exit  ← back to A
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from xavani_cli.update_receipt import begin_update_receipt
    ...
    # running Xavani runtime, its supervisor, and its running code version —
    ...
    try:
        from xavani_cli.update_inventory import (
    ...
    # On Windows, abort early if another xavani.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        scripts_dir = _m()._venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_xavani_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `xavani serve` backend) will keep .pyd
    ...
    # the xavani.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Xavani backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Xavani backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .xavani-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Xavani is still
        ...
        if _venv_holders:
            ...
            # `xavani serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Xavani backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_xavani_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Xavani backend process(es) "
    ...
    # process by design (a CLI `xavani update` IS the venv python), and an
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://enternovate.co.za/xavani-agent/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if current_branch != branch and current_branch != "HEAD":
            ...
            if switch_block_reason.startswith("unmerged:"):
                ...
                try:
                    from xavani_cli.config import load_config as _load_cfg
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by xavani.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                _write_update_incomplete_marker()
                try:
                    from xavani_cli.managed_uv import ensure_uv
                except ImportError:
                    ensure_uv = None
                ...
                if healthy_after:
                    ...
                    print("  Close all Xavani windows/gateways and re-run: xavani update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Xavani gateways, Desktop backends, or other "
        ...
        # Before this guard, ``xavani update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``xavani doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && xavani update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``xavani update`` can catch up even when git is already up to date.
        ...
        # marker survives and the next ``xavani`` launch finishes the install
        ...
        # symbols in xavani_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `xavani` again "
        ...
        if not import_ok:
            ...
            print("    Run `xavani update` again — if it persists, reinstall:")
            print("    https://enternovate.co.za/xavani-agent")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Xavani re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.enternovate.xavani` (repeat "
        ...
        try:
            try:
                from xavani_cli.macos_tcc_anchor import ensure_tcc_anchor
            except ImportError:
                ensure_tcc_anchor = None
        ...
        # current — so copy it straight over ~/.xavani/cache/model_catalog.json
        ...
        try:
            try:
                from xavani_cli.profiles import backfill_profile_envs
            except ImportError:
                backfill_profile_envs = None
        ...
        # Self-heal the xavani-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Xavani on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("xavani-acp launcher self-heal failed: %s", e)

        # Migrate the Windows xavani launchers to the managed binary dir
        # (the default Xavani root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (xavani-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `xavani`
        ...
        # the managed clone under the default Xavani root).
        try:
            from xavani_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``xavani update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `xavani update`
                # must stay fast; `xavani computer-use install --upgrade`
        ...
        # ``user/xavani-serve`` and ``system/xavani-serve`` are two different
        ...
        #
        # Purge stale cached Xavani modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... xavani-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... xavani-gateway*)
            ...
            try:
                try:
                    from xavani_cli.gateway import _get_restart_exit_wait_budget
                except ImportError:
                    _get_restart_exit_wait_budget = None
            ...
            # Discover all xavani-gateway* units (default + profiles) plus
            # xavani-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: xavani gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # xavani-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `xavani update` restart it automatically, allow\n"
            ...
            # Restart EVERY ai.xavani.gateway* LaunchAgent, not only the
            ...
            try:
                from xavani_cli.update_receipt import record_gateway_restart
            ...
            # the reporter hit through tui_gateway (hosted by `xavani serve`,
        ...
        #
        # Forward the systemd units restarted above (includes xavani-serve*,
        ...
        # `xavani serve` (no unit) is left running its pre-update
        ...
        try:
            from xavani_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import xavani_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``xavani update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from xavani_cli.main import main; main()" update --yes'
                    )
            try:
                from xavani_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``xavani_cli.gateway`` no longer imports,
