"""Xavani-managed uv and Python runtime repair.

Xavani owns its own uv binary at ``$XAVANI_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$XAVANI_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Xavani
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.xavani-runtime/python``. A
...
from xavani_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".xavani-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.enternovate.xavani.managed-python"
...
def managed_uv_path() -> Path:
    """Return the path where Xavani keeps *its* uv binary.

    ``$XAVANI_HOME/bin/uv`` on POSIX, ``$XAVANI_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Xavani-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever Xavani provisions a
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Xavani keeps databases "
            "out of WAL mode on this SQLite build. The next `xavani update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``xavani update`` runs
    the call site from the *old*, already-imported ``xavani_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``xavani update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under XAVANI_HOME
    caches the last successful self-update time.
    """
    try:
        from xavani_constants import get_xavani_home

        stamp = get_xavani_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``xavani update`` so the managed copy stays current.
...
def _reload_xavani_constants():
    """Re-execute ``xavani_constants`` from disk and return the fresh module.

    ``xavani update`` imports ``xavani_constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        cannot import name 'venv_python_path' from 'xavani_constants'
        (~/.xavani/xavani-agent/xavani_constants.py)
    ...
    return importlib.reload(xavani_constants)
...
def _venv_python(venv_dir: Path) -> Path:
    ...
    try:
        ...
        venv_python_path = _reload_xavani_constants().venv_python_path
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Xavani generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `xavani update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("xavani_cli.main")
    ...
    if holders:
        ...
        return True, f"other Xavani processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    ancestors on purpose — a CLI ``xavani update`` itself runs from the
    ...
    updater's own ``python.exe`` (or a waiting ``xavani.exe`` launcher
    ...
    # Belt-and-braces: the venv\Scripts\xavani.exe launcher stays mapped
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Xavani-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``xavani update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``xavani update`` repairs the runtime (issue class:
...
def repair_vulnerable_runtime(
    ...
    *,
    ...
    if self_locked:
        ...
        print(
            "    Retrying `xavani update` from inside this venv cannot help: "
        ...
        print(f"      cd {root}")
        print("      <system Python> -m xavani_cli.main update")
        print(
            "    Sessions stay protected meanwhile: Xavani keeps databases "
...
def _install_uv(target: Path) -> None:
    ...
    ``$XAVANI_HOME/bin/`` instead of ``~/.local/bin/``.
    """
