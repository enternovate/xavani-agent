"""``xavani approvals suggest`` — mine approval history into allowlist proposals.

Xavani has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.xavani/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, xavani self-management, …) are eligible.
...
def default_db_path() -> Path:
    ...
    return get_xavani_home() / "state.db"
...
def normalize_command(command: str) -> str:
    """Fold user/xavani home prefixes and collapse whitespace.
    ...
    folded = _rewrite_resolved_user_home(_rewrite_resolved_xavani_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  xavani approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.xavani/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``xavani approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``xavani approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from xavani_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: xavani approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `xavani approvals <subcommand> -h` for details."
    )
    return 1
