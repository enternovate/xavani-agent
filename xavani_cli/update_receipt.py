...
1. **Update receipt** — a machine-readable JSON record of what one
   ``xavani update`` run discovered, did, skipped (and why), written to
   ``<XAVANI_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``xavani_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``xavani update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``xavani update`` run."""
    ...
    def gateway_restart_result(
        ...
        *,
        ...
        if fresh_recovery is not None:
            ...
            # Serve/dashboard coverage (#92145). ``xavani serve`` hosts
...
def _receipt_dir() -> Path:
    ...
    return get_xavani_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``xavani update`` has many early-termination paths (Windows
