...
"""
Xavani CLI - Main entry point.

Usage:
    xavani                     # Interactive chat (default)
    xavani chat                # Interactive chat
    xavani gateway             # Run gateway in foreground
    xavani gateway start       # Start gateway as service
    xavani gateway stop        # Stop gateway service
    xavani gateway status      # Show gateway status
    xavani gateway install     # Install gateway service
    xavani gateway uninstall   # Uninstall gateway service
    xavani setup               # Interactive setup wizard
    xavani logout              # Clear stored authentication
    xavani status              # Show status of all components
    xavani cron                # Manage cron jobs
    ...
    xavani honcho map <name>               # Map current directory to a session name
    xavani honcho peer                     # Show peer names and dialectic settings
    xavani honcho peer --user NAME         # Set user peer name
    xavani honcho peer --ai NAME           # Set AI peer name
    xavani honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    xavani honcho mode                     # Show current memory mode
    xavani honcho mode [hybrid|honcho|local]  # Set memory mode
    xavani honcho tokens                   # Show token budget settings
    xavani honcho tokens --context N       # Set session.context() token cap
    xavani honcho tokens --dialectic N     # Set dialectic result char cap
    xavani honcho identity                 # Show AI peer identity representation
    xavani honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    xavani honcho migrate                  # Step-by-step migration guide: OpenClaw native → Xavani + Honcho
    xavani version             Show version
    xavani update              Update to latest version
    xavani uninstall           Uninstall Xavani Agent
    xavani acp                 Run as an ACP server for editor integration
    xavani sessions browse     Interactive session picker with search

    xavani claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: xavani_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``xavani_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``xavani update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``xavani_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, xavani crashes on import and the user can't run
# ``xavani update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import xavani_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to XAVANI_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (xavani tools, xavani setup, xavani model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'xavani {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any xavani module import.
#
# Many modules cache XAVANI_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("XAVANI_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.xavani/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set XAVANI_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors xavani_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If XAVANI_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.xavani/profiles/coder or
    # /opt/data/profiles/coder).  If XAVANI_HOME points to the xavani root
    # instead (e.g. systemd hardcodes XAVANI_HOME=/root/.xavani), we must
    # still read active_profile — the user may have switched profiles via
    # `xavani profile use` and the gateway should honour that choice.
    # See issue #22502.
    xavani_home_env = os.environ.get("XAVANI_HOME", "")
    if profile_name is None and xavani_home_env:
        if Path(xavani_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the xavani root
    if profile_name is None:
        try:
            from xavani_constants import get_default_xavani_root

            active_path = get_default_xavani_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set XAVANI_HOME
    if profile_name is not None:
        try:
            from xavani_cli.profiles import resolve_profile_env

            xavani_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent xavani from starting
            print(
...
# Load .env from ~/.xavani/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from xavani_cli.config import get_xavani_home
from xavani_cli.env_loader import load_xavani_dotenv

load_xavani_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → XAVANI_REDACT_SECRETS env
# var BEFORE xavani_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "XAVANI_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_xavani_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["XAVANI_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `xavani` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from xavani_logging import setup_logging as _setup_logging
...
try:
    from xavani_cli.config import load_config as _load_config_early
    from xavani_constants import apply_ipv4_preference as _apply_ipv4
...
from xavani_cli import __version__, __release_date__
from xavani_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from xavani_cli.config import get_env_path, get_xavani_home, load_config
    from xavani_cli.auth import get_auth_status

    # Determine whether Xavani itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_xavani_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from xavani_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for Xavani Portal OAuth credentials
    auth_file = get_xavani_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Xavani has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Xavani to use their tokens.
    if _has_xavani_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, xavani_bin
        cli_args: the original CLI arguments (everything after 'xavani')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    xavani_bin = container_info["xavani_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo xavani {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo xavani {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  xavani --tui --resume {target}")
    if title:
        print(f'  xavani --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @xavani/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@xavani" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``XAVANI_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("XAVANI_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    xavani_home = os.environ.get("XAVANI_HOME") or str(Path.home() / ".xavani")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "XAVANI_HOME": xavani_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(xavani_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(xavani_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if xavani_cli_dir is None:
        xavani_cli_dir = Path(__file__).parent
    bundled = xavani_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (XAVANI_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("XAVANI_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from xavani_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("XAVANI_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with XAVANI_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset XAVANI_TUI_DIR (e.g. `unset XAVANI_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("XAVANI_QUIET"):
    ...
    if tui_dev:
        # Keep the local @xavani/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @xavani/ink resolves through
        # packages/xavani-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "xavani-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from xavani_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="xavani-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["XAVANI_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["XAVANI_PYTHON_SRC_ROOT"] = os.environ.get(
        "XAVANI_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("XAVANI_PYTHON", sys.executable)
    env.setdefault("XAVANI_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["XAVANI_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["XAVANI_MODEL"] = model
        env["XAVANI_INFERENCE_MODEL"] = model
    if provider:
        env["XAVANI_TUI_PROVIDER"] = provider
        env["XAVANI_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["XAVANI_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["XAVANI_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["XAVANI_TUI_SKILLS"] = value
    if query:
        env["XAVANI_TUI_QUERY"] = query
    if image:
        env["XAVANI_TUI_IMAGE"] = image
    if checkpoints:
        env["XAVANI_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["XAVANI_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["XAVANI_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["XAVANI_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["XAVANI_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["XAVANI_ACCEPT_HOOKS"] = "1"
    ...
    # XAVANI_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `xavani --tui` try to
    ...
    # still set XAVANI_TUI_RESUME themselves.
    env.pop("XAVANI_TUI_RESUME", None)
    if resume_session_id:
        env["XAVANI_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `xavani update` so
    ...
    if code == 42:
        from xavani_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``XAVANI_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``xavani kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``xavani kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("XAVANI_KANBAN_BOARD"):
        return
    try:
        from xavani_cli.kanban_db import get_current_board

        os.environ["XAVANI_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("XAVANI_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'xavani sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Xavani isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  xavani setup")
        print()

        from xavani_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'xavani setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from xavani_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["XAVANI_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.xavani/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["XAVANI_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["XAVANI_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["XAVANI_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from xavani_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from xavani_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from xavani_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Xavani?")
        print()
    ...
    # has no creds.json.  Every subsequent `xavani gateway` then paid a 30s
    ...
    session_dir = get_xavani_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: xavani gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `xavani gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  xavani gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Xavani Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  xavani gateway")
        ...
        print()
        print("  Or install as a service: xavani gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'xavani whatsapp' to try again.")
...
def cmd_postinstall(args):
    ...
    from xavani_cli.config import stamp_install_method
    from xavani_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Xavani post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``xavani model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("XAVANI_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'xavani model' for "
                "available providers, or run 'xavani doctor' to diagnose config "
                "issues."
            )
    ...
    from xavani_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from xavani_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.xavani/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.xavani/.env if the active provider is not 'custom'.
    ...
    """
    from xavani_cli.config import get_env_value, save_env_value, load_config
...
#
# Xavani uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `xavani model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `xavani model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Xavani only falls back to a lightweight backend (OpenRouter,")
        print("  Xavani Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``xavani model`` flow, then route aux tasks to them here.
    """
    from xavani_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from xavani_cli.auth import _prompt_model_selection
    from xavani_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from xavani_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from xavani_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.xavani/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `xavani setup` from scratch. OpenRouter
    ...
    from xavani_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.xavani/.env by hand.
    ...
    from xavani_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    """Xavani Portal provider: ensure logged in, then pick model."""
    from xavani_cli.auth import (
    ...
    if not state or not state.get("access_token"):
        print("Not logged into Xavani Portal. Starting login...")
        print()
        try:
    ...
    try:
        ...
        if relogin:
            ...
            print("Re-authenticating with Xavani Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    if not model_ids and not unavailable_models:
        print("No models available for Xavani Portal after filtering.")
        return
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate Xavani as the provider and update config
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from xavani_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `xavani auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from xavani_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``xavani model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``xavani model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``xavani auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from xavani_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from xavani_cli.auth import _save_model_choice, deactivate_provider
    from xavani_cli.config import get_env_value, load_config, save_config
    ...
    from xavani_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `xavani model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Xavani URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from xavani_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from xavani_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    print("Anthropic-style API endpoints.  Xavani will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Xavani will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from xavani_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from xavani_cli.config import save_env_value, load_config, save_config
    from xavani_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from xavani_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from xavani_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Xavani turns to `copilot --acp`.")
    print("  Xavani currently starts its own ACP subprocess for each request.")
    print("  Xavani uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set XAVANI_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``xavani setup`` / ``xavani model``.
    ...
    recover from a malformed paste without editing ``~/.xavani/.env`` by hand.
    ...
    from xavani_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from xavani_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `xavani setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so xavani knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Xavani typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Xavani, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, Xavani."
                )
                print()
    ...
    if provider_id == "lmstudio":
        from xavani_cli.auth import AuthError
        from xavani_cli.models import fetch_lmstudio_models
        ...
        from xavani_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Xavani will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from xavani_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'xavani model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Xavani CLI with a provider."""
    from xavani_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from xavani_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from xavani_cli.auth_commands import auth_command
...
def cmd_webhook(args):
    ...
    from xavani_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``xavani slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: xavani slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `xavani slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from xavani_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from xavani_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from xavani_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from xavani_cli.doctor import run_doctor
...
def cmd_debug(args):
    ...
    from xavani_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from xavani_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Xavani home directory to a zip file."""
    if getattr(args, "quick", False):
        from xavani_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from xavani_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from xavani_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Xavani Agent v{__version__} ({__release_date__})")
    ...
    try:
        from xavani_cli.banner import check_for_updates
        from xavani_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Xavani Agent."""
    _require_tty("uninstall")
    from xavani_cli.uninstall import run_uninstall
...
# Critical files that every ``xavani`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``xavani update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "xavani_cli/main.py",
    "xavani_cli/config.py",
    "xavani_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``xavani`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``xavani update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``xavani_cli/web_dist/`` (per vite.config.ts
    outDir: "../xavani_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "xavani_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``xavani update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``xavani web``).
    ...
    # build path usable on a stock `py -m xavani_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (xavani web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``xavani dashboard`` processes other than ourselves.

    ``xavani dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``xavani update`` replaces files on disk, the running
    ...
    patterns = [
        "xavani dashboard",
        "xavani_cli.main dashboard",
        "xavani_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "xavani.*dashboard"`) keeps us consistent
            # with `xavani_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `xavani update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``xavani update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``xavani update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: xavani curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``xavani dashboard`` processes.

    Called at the end of ``xavani update`` (default ``reason``) and also
    from ``xavani dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    xavani dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Xavani Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/enternovate/xavani-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="xavani-update-")
        zip_path = os.path.join(tmp_dir, f"xavani-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to xavani-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"xavani-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "xavani-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Xavani behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes xavani completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Xavani couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Xavani couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Xavani behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `xavani update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/enternovate/xavani-agent.git",
    "git@github.com:enternovate/xavani-agent.git",
    "https://github.com/enternovate/xavani-agent",
    "git@github.com:enternovate/xavani-agent",
}
OFFICIAL_REPO_URL = "https://github.com/enternovate/xavani-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_xavani_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_xavani_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Xavani repository.")
        print("  This means you may miss updates from enternovate/xavani-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/enternovate/xavani-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/enternovate/xavani-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``xavani update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from xavani_constants import get_default_xavani_root

    default_home = get_default_xavani_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``xavani update`` is still progressing even if pip/uv itself is silent.
    """
...
def _xavani_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "xavani.exe",
        scripts_dir / "xavani-gateway.exe",
    ]


def _quarantine_running_xavani_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``xavani.exe``.
    ...
    when ``xavani update`` runs, ``xavani.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``xavani.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next xavani invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _xavani_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_xavani_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``xavani.exe.old.*`` left by prior updates.

    Called early on every xavani invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``xavani update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `xavani update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    ``_quarantine_running_xavani_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_xavani_exe(scripts_dir)
        try:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        urllib.request.urlretrieve(psutil_url, archive)  # nosec B310 — hardcoded HTTPS PyPI URL
        with tarfile.open(archive) as tar:
            # Defensive extraction (tar-slip hardening): reject absolute paths
            # and ".." traversal, and prefer the 'data' filter on Python 3.12+.
            for member in tar.getmembers():
                if member.name.startswith("/") or ".." in Path(member.name).parts:
                    raise RuntimeError(
                        f"refusing to extract unsafe path from psutil archive: {member.name!r}"
                    )
            try:
                tar.extractall(tmp_path, filter="data")  # type: ignore[call-arg]
            except TypeError:
                # Python < 3.12 — no filter kwarg. Members validated above.
                tar.extractall(tmp_path)  # nosec B202
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.xavani/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``xavani update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.xavani/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``xavani update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``xavani update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.xavani/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``xavani update --gateway``) the update is already
    ...
    try:
        ...
        # xavani_cli.config.get_xavani_home to simulate setup failure.
        from xavani_cli.config import get_xavani_home as _get_xavani_home

        logs_dir = _get_xavani_home() / "logs"
...
def _cmd_update_check():
    """Implement ``xavani update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``xavani update`` without requiring a reinstall.  The
    ...
    ``xavani`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/xavani.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v xavani'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/xavani, code at /usr/local/lib/xavani-agent).
    fhs_link = Path("/usr/local/bin/xavani")
    ...
    path_comment = (
        "# Xavani Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of XAVANI_HOME before running the update.
    ...
    XAVANI_HOME directories.  The ``--backup`` flag on ``xavani update``
    ...
    try:
        from xavani_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_xavani_home so the user sees ~/.xavani/...
    try:
        from xavani_constants import get_xavani_home, display_xavani_home

        home = get_xavani_home()
        try:
            display_path = f"{display_xavani_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  xavani import {out_path}")
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update Xavani Agent")
        return
...
def _cmd_update_pip(args):
    """Update Xavani via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "xavani-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "xavani-agent"]
    ...
    print("✓ Update complete! Restart xavani to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # orphan merge-conflict markers in xavani_cli/config.py bricked
        # every user who ran ``xavani update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``xavani
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``xavani update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_xavani_home added to xavani_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload xavani_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_xavani_home() added since the last release.
        try:
            import importlib
            import xavani_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit XAVANI_HOME so
        # it is not affected by sync_skills()'s module-level XAVANI_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's XAVANI_HOME env var points at the default or a named profile.
        try:
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: xavani config migrate")
            else:
                print()
                print("Skipped. Run 'xavani config migrate' later to configure.")
        else:
        ...
        # check `xavani curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``xavani update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from xavani_cli.tools_config import install_cua_driver
        ...
        # When running as ``xavani update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_xavani_home() / ".update_exit_code"
            try:
        ...
        try:
            from xavani_cli.gateway import (
                is_macos,
            ...
            # Discover all xavani-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. xavani-gateway.service or xavani-gateway-coder.service
                            ...
                            # path in `xavani gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: xavani gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: xavani -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Xavani gateway unit files are still installed.
        # When both xavani.service (from a pre-rename install) and the
        # current xavani-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `xavani update` surfaces the issue until the user migrates.
        try:
            from xavani_cli.gateway import (
                has_legacy_xavani_units,
                _find_legacy_xavani_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_xavani_units():
                print()
                print("⚠ Legacy Xavani gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_xavani_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (xavani.service) fight the current")
                print("  xavani-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    xavani gateway migrate-legacy")
                print()
        ...
        print("  xavani model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``xavani -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from xavani_constants import display_xavani_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `xavani profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_xavani_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → xavani -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.xavani)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  xavani profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     xavani -p {name} chat")
                else:
        ...
        from xavani_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from xavani_constants import get_xavani_home as _hh
        ...
        from xavani_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `xavani profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from xavani_cli.profiles import profile_exists
        ...
        from xavani_cli.profiles import rename_profile
        ...
        from xavani_cli.profiles import export_profile
        ...
        from xavani_cli.profiles import import_profile
        ...
        import tempfile
        from xavani_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="xavani_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  xavani -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      xavani -p {plan.manifest.name} chat")
        ...
        from xavani_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `xavani profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"xavani -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from xavani_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("xavani_requires"):
            print(f"Requires:     Xavani {data['xavani_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from xavani_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.xavani_requires:
        print(f"  Requires: Xavani {mf.xavani_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``xavani dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``xavani dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No xavani dashboard processes running.")
        return 0

    print(f"{len(pids)} xavani dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No xavani dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `xavani update`.
    ...
    if "XAVANI_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["XAVANI_WEB_DIST"])
            if "XAVANI_WEB_DIST" in os.environ
            else PROJECT_ROOT / "xavani_cli" / "web_dist"
        )
    ...
    from xavani_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("XAVANI_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from xavani_cli.completion import generate_bash, generate_zsh, generate_fish
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from xavani_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``xavani -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``xavani_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``xavani -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``xavani`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for xavani CLI."""
    ...
    # Sweep stale ``xavani.exe.old.*`` quarantine files left by previous
    # ``xavani update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_xavani_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from xavani_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--portal-url",
        help="Portal base URL for Xavani login (default: production portal)",
    )
    model_parser.add_argument(
        "--inference-url",
        help="Inference API base URL for Xavani login (default: production inference API)",
    )
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Xavani login (default: xavani-cli)",
    )
    model_parser.add_argument(
        "--scope", default=None, help="OAuth scope to request for Xavani login"
    )
    model_parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not attempt to open the browser automatically during Xavani login",
    )
    model_parser.add_argument(
        "--manual-paste",
        action="store_true",
    ...
    model_parser.add_argument(
        ...
        type=float,
        default=15.0,
        help="HTTP request timeout in seconds for Xavani login (default: 15)",
    )
    model_parser.add_argument(
        "--ca-bundle", help="Path to CA bundle PEM file for Xavani TLS verification"
    )
    model_parser.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for Xavani login (testing only)",
    )
    ...
    # =========================================================================
    from xavani_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `xavani model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy xavani.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Xavani gateway unit files "
            "(e.g. xavani.service) left over from older installs. Profile "
            "units (xavani-gateway-<profile>.service) and unrelated "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Xavani Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `xavani proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Xavani Agent with an interactive wizard. "
        "Run a specific section: xavani setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'xavani setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Xavani.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $XAVANI_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Xavani")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from xavani_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Xavani CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: xavani-cli)"
    )
    ...
    auth_add.add_argument("--portal-url", help="Xavani portal base URL")
    auth_add.add_argument("--inference-url", help="Xavani inference base URL")
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Xavani with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set XAVANI_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Xavani Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.xavani/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Xavani profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.xavani/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Xavani profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from xavani_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.xavani/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.xavani/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Xavani Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `xavani "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Xavani setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Xavani Agent. Use 'xavani debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    xavani debug share              Upload debug report and print URL
    xavani debug share --lines 500  Include more log lines
    xavani debug share --expire 30  Keep paste for 30 days
    xavani debug share --local      Print report locally (no upload)
    xavani debug share --no-redact  Disable upload-time secret redaction
    xavani debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'xavani debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Xavani home directory to a zip file",
        description="Create a zip archive of your entire Xavani configuration, "
        "skills, sessions, and data (excludes the xavani-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/xavani-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.xavani/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo xavani uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from xavani_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Xavani backup from a zip file",
        description="Extract a previously created Xavani backup into your "
        "Xavani home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Xavani Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from xavani_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.xavani/skills/.bundled_manifest) "
            "so future 'xavani update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from xavani_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from xavani_cli.skills_hub import skills_command
    ...
    from xavani_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/xavani-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `xavani plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from xavani_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``xavani --help``, ``xavani version``, ``xavani logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from xavani_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from xavani_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_xavani_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_xavani_home()}/memories/\n")
        else:
            from xavani_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'xavani tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # xavani tools list [--platform cli]
    ...
    # xavani tools disable <name...> [--platform cli]
    ...
    # xavani tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            ...
            _require_tty("tools")
            from xavani_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `xavani computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `xavani tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: xavani computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: xavani computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Xavani as an MCP server",
        description=(
            "Manage MCP server connections and run Xavani as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'xavani mcp add' to connect to a new server, or\n"
            "'xavani mcp serve' to expose Xavani conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Xavani as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `xavani mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from xavani_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_xavani_home() / "sessions"
            ...
            # Launch xavani --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Xavani",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Xavani",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.xavani/ (by default a "
        "single restore-point archive is written to ~/.xavani/backups/ "
        "before apply; restorable with 'xavani import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from xavani_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Xavani Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'xavani config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Xavani Agent",
        description="Remove Xavani Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Xavani Agent as an ACP (Agent Client Protocol) server",
        description="Start Xavani Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Xavani ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Xavani provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.xavani/node/ "
    ...
    def cmd_acp(args):
        """Launch Xavani Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Xavani instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `xavani update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `xavani profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Xavani profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Xavani Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `xavani --tui` via PTY/WebSocket). "
            "Alternatively set XAVANI_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `xavani dashboard` cmdlines and SIGTERM them directly — the same
    # path `xavani update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running xavani dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running xavani dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Xavani log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    xavani logs                    Show last 50 lines of agent.log
    xavani logs -f                 Follow agent.log in real time
    xavani logs errors             Show last 50 lines of errors.log
    xavani logs gateway -n 100     Show last 100 lines of gateway.log
    xavani logs --level WARNING    Only show WARNING and above
    xavani logs --session abc123   Filter by session ID
    xavani logs --component tools  Only show tool-related lines
    xavani logs --since 1h         Lines from the last hour
    xavani logs --since 30m -f     Follow, starting from 30 min ago
    xavani logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``xavani -c Pokemon Agent Dev`` → ``xavani -c 'Pokemon Agent Dev'``
    ...
    from xavani_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'xavani -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (xavani hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from xavani_cli.plugins import discover_plugins
