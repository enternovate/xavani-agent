...
"""
xavani fallback — manage the fallback provider chain.
...
Subcommands:
  ...
  xavani fallback add      Pick provider + model via the same picker as `xavani model`,
                           then append the selection to the chain
  xavani fallback remove   Pick an entry to delete from the chain
  xavani fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.xavani/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from xavani_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `xavani model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  xavani fallback add")
        print()
        return
    ...
    print("  Docs: https://xavani-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `xavani model`, then append the selection to the chain."""
    from xavani_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `xavani model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `xavani fallback list` to view, or `xavani fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``xavani fallback [subcommand]``."""
