...
supported components into records consumed by Xavani's existing skill and MCP
...
def _translate_remote(config: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    The returned record targets Xavani's existing URL-based MCP runtime.
...
def has_enabled_agent_plugin_mcp(raw_config: Mapping[str, Any]) -> bool:
    ...
    Directory scanning belongs to :mod:`xavani_cli.plugins` so startup gating
    ...
    try:
        from xavani_cli.plugins import has_enabled_agent_plugin_mcp as _probe
    except ImportError:
        _probe = None
