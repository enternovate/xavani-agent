...
"""Profile distributions — shareable, packaged Xavani profiles via git.

A distribution is a Xavani profile published as a git repository (or
...
* ``xavani profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``xavani skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``xavani profile``, not a parallel tree):

    xavani profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "xavani-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            xavani_requires=str(data.get("xavani_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.xavani_requires:
            out["xavani_requires"] = self.xavani_requires
        if self.author:
...
def check_xavani_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Xavani {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``xavani profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Xavani profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Xavani distribution."
        )

    # Version check up-front so we fail fast
    check_xavani_requires(manifest.xavani_requires, xavani_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.xavani).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="xavani_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `xavani profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`xavani profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="xavani_dist_update_") as tmp:
        plan = plan_install(
