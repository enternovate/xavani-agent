...
from xavani_constants import get_default_xavani_root, get_xavani_home
...
def active_session_limit_message(
    ...
    return (
        f"Xavani is at the active session limit ({active_count}/{max_sessions})."
        f"{detail} Try again when another session finishes."
    )


def _registry_home(registry_home: str | Path | None = None) -> Path:
    return Path(registry_home) if registry_home is not None else Path(get_xavani_home())
...
class ActiveSessionLease:
    ...
    # root ``XAVANI_HOME`` must release against the same registry even when
    ...
    with _FileLock(lock_path):
        try:
            ...
            return None, ActiveSessionRefusal(
                (
                    "Xavani could not read the active-session registry at "
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile XAVANI_HOME override (#85431).
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``xavani dashboard`` / ``serve``) never trips that
