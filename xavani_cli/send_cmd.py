...
"""CLI subcommand: ``xavani send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"xavani send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"xavani send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"xavani send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"xavani send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"xavani send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `xavani gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.xavani/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_xavani_env() -> None:
    """Populate ``os.environ`` from ``~/.xavani/.env`` AND bridge top-level
    ...
    startup that ``xavani send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.xavani/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.xavani/config.yaml`` into
       ...
       via ``xavani config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``xavani send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.xavani/.env and ~/.xavani/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "xavani send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  xavani send --to telegram \"hello\"\n"
            "  xavani send --to discord:#ops --file report.md\n"
            "  xavani send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "xavani send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `xavani send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Xavani "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.xavani/.env + ~/.xavani/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  xavani send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  xavani send --list                  # all platforms\n"
            "  xavani send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `xavani send --list telegram`.",
    )
