...
"""
Unified tool configuration for Xavani Agent.

`xavani tools` and `xavani setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.xavani/config.yaml under
...
from xavani_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _XAVANI_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `xavani tools` → Video Generation, which walks
...
# `xavani tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from xavani_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `xavani tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``xavani tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from xavani_cli.plugins import discover_plugins, get_plugin_toolsets
...
from xavani_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        #   - "Xavani Subscription" — managed Firecrawl billed via Xavani
    ...
    "x_search": {
        ...
        "setup_note": (
            "Xavani routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        #   - "Xavani Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Xavani subscription (requires_nous_auth +
        ...
        "providers": [
            {
                "name": "Xavani Subscription (Browser Use cloud)",
                "badge": "subscription",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. XAVANI_CUA_DRIVER_VERSION is an
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``xavani update`` and
      by ``xavani computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `xavani update` calls this for every
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    try:
        result = subprocess.run(install_cmd, shell=True, timeout=300)  # nosec B602 — curl pipe to bash for upstream installer; URL is hardcoded
        if result.returncode == 0 and shutil.which("cua-driver"):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Xavani process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_xavani_home()}/xavani-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in xavani-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/enternovate/xavani-agent:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `xavani update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.xavani/config.yaml")
        ...
        # Run the full `xavani auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.xavani/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from xavani_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: xavani auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `xavani auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: xavani auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: xavani auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `xavani auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: xavani auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: xavani auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "xavani-cli" (which include all _XAVANI_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[xavani-cli, spotify]`` after enabling Spotify via ``xavani
        ...
        # (e.g. "xavani-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``xavani tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `xavani tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"xavani-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("xavani-")}
    ...
    # shipped as a bundled plugin but user must opt in via `xavani tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `xavani tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by xavani tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (xavani-cli, xavani-telegram, etc.)
    ...
    # Opening `xavani tools` is the user's opt-in to reconfigure tools, so treat
...
# ("Xavani Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Xavani Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    # later) so the picker lists them alongside FAL / Xavani Subscription.
    ...
    # remaining hardcoded rows ("Xavani Subscription", "Firecrawl
    ...
    # the hardcoded "Xavani Subscription" / "Local Browser" / "Camofox" rows
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `xavani tools` gets a silent no-op
...
def _configure_provider(provider: dict, config: dict):
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Xavani subscription.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `xavani tools` and `xavani setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Xavani Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://enternovate.co.za/xavani-agent/docs/user-guide/features/tools", Colors.DIM))
    print()
        ...
        for pkey in enabled_platforms:
            ...
            if managed_nous_tools_enabled():
                for ts_key in sorted(auto_configured):
                    ...
                    print(color(f"  ✓ {label}: using your Xavani subscription defaults", Colors.GREEN))
    ...
    print()
    from xavani_constants import display_xavani_home
    print(color(f"  Tool configuration saved to {display_xavani_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'xavani' or gateway restart.", Colors.DIM))
    print()
