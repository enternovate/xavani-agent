...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or xavani -c.",
    ...
    "Ctrl+Z suspends Xavani to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "xavani -c resumes your most recent CLI session. xavani -c \"project name\" resumes by title.",
    "xavani -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "xavani -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "xavani chat -t web,terminal enables only specific toolsets for a focused session.",
    "xavani chat -s github-pr-workflow preloads a skill at launch.",
    "xavani chat -q \"query\" runs a single non-interactive query and exits.",
    "xavani chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "xavani chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "xavani --yolo bypasses all dangerous command approval prompts for the entire session.",
    "xavani chat --source telegram tags the session for filtering in xavani sessions list.",
    "xavani -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "xavani doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "xavani dump outputs a compact setup summary — great for bug reports.",
    "xavani config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "xavani config edit opens config.yaml in your default editor.",
    "xavani config check scans for missing or stale configuration options.",
    "xavani sessions browse opens an interactive session picker with search.",
    "xavani sessions stats shows session counts by platform and database size.",
    "xavani sessions prune --older-than 30 cleans up old sessions.",
    "xavani skills search react --source skills-sh searches the skills.sh public directory.",
    "xavani skills check scans installed hub skills for upstream updates.",
    "xavani skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "xavani skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "xavani mcp add github --command npx adds MCP servers from the command line.",
    "xavani mcp serve runs Xavani itself as an MCP server for other agents.",
    "xavani auth add lets you add multiple API keys for credential pool rotation.",
    "xavani completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "xavani logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "xavani backup creates a zip backup of your entire Xavani home directory.",
    "xavani profile create coder creates an isolated profile that becomes its own command.",
    "xavani profile create work --clone copies your current config and keys to a new profile.",
    "xavani update syncs new bundled skills to ALL profiles automatically.",
    "xavani gateway install sets up Xavani as a system service (systemd/launchd).",
    "xavani memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "xavani webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: xavani tools disables unused tools, xavani skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "xavani models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as xavani -w).",
    ...
    "XAVANI_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Xavani tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'xavani profile create coder' creates the 'coder' command.",
    "xavani profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Xavani prints a resume command with session ID and stats.",
    "xavani sessions export backup.jsonl exports all sessions for backup or analysis.",
    "xavani -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "xavani skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .xavani/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: xavani cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Xavani runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "xavani gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Xavani via a local macOS server.",
    ...
    "Context files (.xavani.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.xavani/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Xavani loads project context from .xavani.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — xavani tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.xavani/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "xavani chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "xavani plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "xavani acp runs Xavani as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "XAVANI_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "xavani login supports OAuth-based auth for Xavani and OpenAI Codex providers.",
    ...
    "xavani status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.xavani/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Xavani your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.xavani/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Xavani auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.xavani/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at XAVANI_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "XAVANI_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in XAVANI_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.xavani/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'xavani profile create ls' would be rejected.",
    "xavani profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "xavani claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "XAVANI_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via xavani tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.xavani/.env.",
    "Skills from trusted repos (Enternovate) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.xavani/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Xavani auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Xavani config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Xavani is working.',
    ...
    '/reload reloads ~/.xavani/.env into the running session — pick up new API keys without restarting.',
    ...
    "XAVANI_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.xavani/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.xavani/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'xavani curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "xavani curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'xavani curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'xavani auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Xavani subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'XAVANI_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "XAVANI_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'xavani dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'xavani dashboard --tui embeds the full Xavani TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.xavani/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.xavani/dashboard-plugins/ — no npm build required.',
    ...
    'XAVANI_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'XAVANI_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'XAVANI_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'XAVANI_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.xavani/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.xavani/skills/ so drop-in skills appear without restarting the session.',
    ...
    'xavani -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'xavani chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'xavani chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'xavani chat --ignore-user-config skips ~/.xavani/config.yaml — reproducible bug reports and CI runs.',
    "xavani chat --source tool tags programmatic chats so they don't clutter xavani sessions list.",
    'xavani dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'xavani sessions rename <ID> "new title" renames any past session; xavani sessions delete <ID> removes one.',
    'xavani import restores a session export or profile archive produced by sessions export or profile export.',
    'xavani fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'xavani pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'xavani setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'xavani status --deep runs the full health sweep across every component; plain xavani status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'XAVANI_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'XAVANI_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.xavani/plugins/ — trust-gated by design.',
    "XAVANI_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'XAVANI_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'XAVANI_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'XAVANI_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'XAVANI_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'XAVANI_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'XAVANI_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'XAVANI_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'XAVANI_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'XAVANI_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'XAVANI_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Xavani block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from xavani sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.xavani/dashboard-plugins/.',
]
