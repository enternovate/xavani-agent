...
"""
Xavani Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m xavani_cli.main web          # Start on http://127.0.0.1:9119
    python -m xavani_cli.main web --port 8080
"""
...
from gateway.security import validate_path
...
try:
    ...
    # running `xavani dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["XAVANI_WEB_DIST"]) if "XAVANI_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
app = FastAPI(title="Xavani Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Xavani-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``xavani dashboard --tui``
# or XAVANI_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.xavani/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_xavani_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "xavani-update": "xavani-update.log",
}
...
def _spawn_xavani_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``xavani <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``xavani_cli.main`` module so the action
    inherits the same venv/PYTHONPATH the web server is using.
    """
    log_file_name = _ACTION_LOG_FILES[name]
    _ACTION_LOG_DIR.mkdir(parents=True, exist_ok=True)
    # Use the validated Path directly — discarding it and re-joining
    # ``_ACTION_LOG_DIR / log_file_name`` hides the validation barrier
    # from CodeQL's taint tracker.
    log_path = validate_path(log_file_name, _ACTION_LOG_DIR, allow_create=True)
    log_file = open(log_path, "ab", buffering=0)
    log_file.write(
        f"\n=== {name} started {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n".encode()
    )

    cmd = [sys.executable, "-m", "xavani_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "XAVANI_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``xavani gateway restart`` in the background."""
    try:
        proc = _spawn_xavani_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/xavani/update")
async def update_xavani():
    """Kick off ``xavani update`` in the background."""
    try:
        proc = _spawn_xavani_action(["update"], "xavani-update")
    except Exception as exc:
        _log.exception("Failed to spawn xavani update")
        raise HTTPException(status_code=500, detail="Internal error")
    return {
        "ok": True,
        "pid": proc.pid,
...
async def get_action_status(name: str, lines: int = 200):
    ...
    # Propagate the validated Path (see L686 site for rationale).
    log_path = validate_path(log_file_name, _ACTION_LOG_DIR, allow_create=True)
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Xavani supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in xavani_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.xavani/config.yaml`` — applies to **new** sessions only.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Xavani resolves Anthropic creds in this order at runtime:
    1. ``~/.xavani/.anthropic_oauth.json`` — Xavani-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_xavani_oauth_credentials,
            read_claude_code_credentials,
            _XAVANI_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_xavani_oauth_credentials = None  # type: ignore
        _XAVANI_OAUTH_FILE = None  # type: ignore

    xavani_creds = None
    if read_xavani_oauth_credentials:
        try:
            xavani_creds = read_xavani_oauth_credentials()
        except Exception:
            xavani_creds = None
    if xavani_creds and xavani_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "xavani_pkce",
            "source_label": f"Xavani PKCE ({_XAVANI_OAUTH_FILE})",
            "token_preview": _truncate_token(xavani_creds.get("accessToken")),
            "expires_at": xavani_creds.get("expiresAt"),
            "has_refresh_token": bool(xavani_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Xavani even
    when they also have a separate Xavani-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "xavani auth add anthropic",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "xavani auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "xavani auth add qwen-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    if status_fn is not None:
        try:
            ...
            return {"logged_in": False, "error": "Internal error"}
    try:
        from xavani_cli import auth as hauth
        if provider_id == "nous":
        ...
        _log.exception("OAuth status check failed")
        return {"logged_in": False, "error": "Internal error"}
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("xavani_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Xavani-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _XAVANI_OAUTH_FILE
            if _XAVANI_OAUTH_FILE.exists():
                _XAVANI_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from xavani_cli.auth import clear_provider_auth
...
# Guarded so xavani web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Xavani file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``xavani auth add anthropic``.
    """
    from agent.anthropic_adapter import _XAVANI_OAUTH_FILE
    payload = {
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "xavani-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("XAVANI_PORTAL_BASE_URL")
    ...
    if provider_id == "openai-codex":
        ...
        if s.get("status") == "error":
            _log.warning("OAuth device-auth session %s failed: %s", sid, s.get("error_message"))
            raise HTTPException(status_code=500, detail="Internal error")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from xavani_cli.auth import persist_nous_credentials
        ...
        with _oauth_sessions_lock:
            sess["status"] = "error"
            sess["error_message"] = "Internal error"
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = (
            os.getenv("XAVANI_CODEX_BASE_URL", "").strip().rstrip("/")
...
async def get_logs(
    ...
    from xavani_cli.logs import _read_tail, LOG_FILES
    ...
    # Propagate the validated Path (see L686 site for rationale).
    log_path = validate_path(log_name, logs_dir, allow_create=True)
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from xavani_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, XAVANI_HOME)."""
    from xavani_cli import profiles as profiles_mod
    ...
    # ``get_profile_dir`` now returns a Path that has been canonicalised
    # and containment-checked via ``validate_path`` (see profiles.py).
    # The previous defence-in-depth re-validation here is redundant and
    # was discarding its return value, which kept CodeQL flagging the
    # downstream ``profile_dir`` reads as ``py/path-injection``.
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["xavani_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process XAVANI_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_xavani_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "xavani setup" if name == "default" else f"{name} setup"


def _profile_setup_argv(name: str) -> list[str]:
    """Return the argv list used to configure a profile.

    Uses argv-form rather than the shell-string returned by
    :func:`_profile_setup_command` so callers can ``Popen(argv)`` directly
    without going through ``sh -lc``. The name must already have been
    validated against the ``[a-z0-9_-]+`` allow-list; this function does
    not re-validate.
    """
    if name == "default":
        return ["xavani", "setup"]
...
async def open_profile_terminal_endpoint(name: str):
    try:
        # Allowlist validation: the profile name reaching subprocess is a
        # constrained string. Any character outside ``[a-z0-9_-]`` is
        # rejected before reaching the spawn helpers below.
        if not re.fullmatch(r"[a-z0-9_-]+", name):
            raise HTTPException(status_code=400, detail="Invalid profile name")

        # Build the argv list and pass it directly to ``Popen`` — no
        # ``sh -lc`` indirection. This eliminates the CodeQL
        # ``py/command-line-injection`` flow even though ``name`` is
        # already regex-restricted, because the user-controlled string
        # no longer reaches a shell at all.
        setup_argv = _profile_setup_argv(name)

        if sys.platform.startswith("win"):
            # ``start`` on Windows is a cmd built-in that parses its first
            # arg as a window title; we still pass argv to keep parsing
            # explicit.
            subprocess.Popen(["cmd.exe", "/c", "start", "", *setup_argv])
        elif sys.platform == "darwin":
            # AppleScript ``do script`` consumes a string. Build it from
            # the regex-validated argv components and quote each piece
            # with ``shlex.quote`` so any future relaxation of the
            # allow-list still produces well-formed AppleScript.
            import shlex

            quoted = " ".join(shlex.quote(part) for part in setup_argv)
            applescript = (
            ...
            # Linux terminals — invoke the argv directly with ``-e <argv>``
            # or ``-- <argv>``. Neither form parses the arguments as a
            # shell string, so no quoting is needed.
            terminal_commands = [
                ("x-terminal-emulator", ["x-terminal-emulator", "-e", *setup_argv]),
                ("gnome-terminal", ["gnome-terminal", "--", *setup_argv]),
                ("konsole", ["konsole", "-e", *setup_argv]),
                ("xfce4-terminal", ["xfce4-terminal", "-e", *setup_argv]),
                ("mate-terminal", ["mate-terminal", "-e", *setup_argv]),
                ("lxterminal", ["lxterminal", "-e", *setup_argv]),
                ("tilix", ["tilix", "-e", *setup_argv]),
                ("alacritty", ["alacritty", "-e", *setup_argv]),
                ("kitty", ["kitty", *setup_argv]),
                ("xterm", ["xterm", "-e", *setup_argv]),
            ]
        ...
        raise HTTPException(status_code=500, detail="Internal error")
    return {"ok": True, "command": setup_argv}
...
async def get_profile_soul(name: str):
    ...
    if soul_path.exists():
        try:
            ...
            _log.exception("GET /api/profiles/%s/soul failed", name)
            raise HTTPException(status_code=500, detail="Internal error")
...
async def get_skills():
    ...
    from xavani_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from xavani_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from xavani_cli.tools_config import (
...
#
# The endpoint spawns the same ``xavani --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``xavani --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``XAVANI_TUI_RESUME`` env var —
    matching what ``xavani_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``XAVANI_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from xavani_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("XAVANI_TUI_DISABLE_MOUSE", "1")
    env.setdefault("XAVANI_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["XAVANI_TUI_RESUME"] = resume

    if sidecar_url:
        env["XAVANI_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Xavani inside WSL2 to use the dashboard's /chat "
...
# XAVANI_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/xavani"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/xavani/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /xavani`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__XAVANI_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/xavani``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__XAVANI_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__XAVANI_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__XAVANI_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/xavani`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Xavani backend. Intercept CSS asset
    # requests BEFORE the StaticFiles mount and rewrite the absolute paths
    # when a prefix is in play.
    @application.get("/assets/{filename}.css")
    async def serve_css(filename: str, request: Request):
        # ``validate_path`` canonicalises + contains the URL-supplied
        # filename to ``WEB_DIST/assets``. Returns a Path that is
        # guaranteed in-base; raises on traversal / NUL bytes /
        # missing file. CodeQL recognises this as a path-injection
        # barrier (vs the prior is_relative_to side-check on a path
        # built from raw ``filename``).
        assets_dir = WEB_DIST / "assets"
        try:
            css_path = validate_path(f"{filename}.css", assets_dir)
    ...
    async def serve_spa(full_path: str, request: Request):
        prefix = _normalise_prefix(request.headers.get("x-forwarded-prefix"))
        # ``validate_path`` canonicalises the URL-derived path, refuses
        # traversal segments / NUL bytes / out-of-base resolves, and
        # raises ``FileNotFoundError`` when the asset doesn't exist —
        # for those cases the SPA falls back to serving index. The
        # earlier code paired a discarded ``validate_path`` call with
        # a separate ``is_relative_to`` check, which kept CodeQL
        # flagging ``py/path-injection`` because the data-flow into
        # ``FileResponse`` started from raw ``full_path``.
        try:
            file_path = validate_path(full_path, WEB_DIST)
        except (ValueError, FileNotFoundError):
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Xavani Teal",         "description": "Classic dark teal — the canonical Xavani look"},
    {"name": "default-large", "label": "Xavani Teal (Large)", "description": "Xavani Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.xavani/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.xavani/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_xavani_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.xavani/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as xavani_cli.plugins):
    1. User plugins:    ~/.xavani/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.xavani/plugins/  (only if XAVANI_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from xavani_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_xavani_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("XAVANI_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".xavani" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from xavani_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_xavani_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"xavani auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from xavani_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from xavani_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from xavani_cli.plugins_cmd import dashboard_remove_user_plugin
...
async def serve_plugin_asset(plugin_name: str, file_path: str):
    ...
    # Use the canonical validator instead of (resolve + is_relative_to)
    # so CodeQL sees the path-injection barrier directly on the
    # data-flow into FileResponse(target). Same guarantees as before:
    # rejects traversal, NUL/CR/LF, and out-of-base resolves.
    try:
        target = validate_path(file_path, base)
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"xavani_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Xavani Web UI → http://{host}:{port}")
