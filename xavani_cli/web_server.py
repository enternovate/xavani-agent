...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    ...
    # Defense-in-depth: validate the resolved path stays within profiles root
    if canon != "default":
        validate_path(canon, profiles_mod._get_profiles_root())  # blocks traversal
...
async def open_profile_terminal_endpoint(name: str):
    try:
        ...
        if sys.platform.startswith("win"):
            subprocess.Popen(["cmd.exe", "/c", "start", "", safe_command])
        elif sys.platform == "darwin":
            # AppleScript injections are prevented by shlex.quote;
            # the command is a validated profile name + " setup".
            applescript = (
                'tell application "Terminal"\n'
                "activate\n"
                f'do script {safe_command}\n'
                "end tell"
            )
            subprocess.Popen(["osascript", "-e", applescript])
        else:
            # All terminal commands use shlex.quote to prevent shell injection.
            terminal_commands = [
                ("x-terminal-emulator", ["x-terminal-emulator", "-e", "sh", "-lc", safe_command]),
                ("gnome-terminal", ["gnome-terminal", "--", "sh", "-lc", safe_command]),
                ("konsole", ["konsole", "-e", "sh", "-lc", safe_command]),
                ("xfce4-terminal", ["xfce4-terminal", "-e", "sh", "-lc", safe_command]),
                ("mate-terminal", ["mate-terminal", "-e", "sh", "-lc", safe_command]),
                ("lxterminal", ["lxterminal", "-e", "sh", "-lc", safe_command]),
                ("tilix", ["tilix", "-e", "sh", "-lc", safe_command]),
                ("alacritty", ["alacritty", "-e", "sh", "-lc", safe_command]),
                ("kitty", ["kitty", "sh", "-lc", safe_command]),
                ("xterm", ["xterm", "-e", "sh", "-lc", safe_command]),
            ]
...
def mount_spa(application: FastAPI):
    ...
    async def serve_spa(full_path: str, request: Request):
        ...
        # and validate the path stays within WEB_DIST
        try:
            validate_path(full_path, WEB_DIST)  # blocks traversal
