...
``$XAVANI_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-xavani-setup
(the stage2 hook) has chowned the volume and seeded $XAVANI_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$XAVANI_HOME``,
    not under ``profiles/``). The dispatcher in ``xavani_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``xavani gateway start`` (no ``-p``) targets. Without it,
    bare ``xavani gateway start`` inside the container would land on
    ...
    ``$XAVANI_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        xavani_home: The container's XAVANI_HOME (typically /opt/data).
            Profiles live under ``<xavani_home>/profiles/<name>/``;
            the default profile lives at ``<xavani_home>`` itself.
    ...
    # ``xavani gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(xavani_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(xavani_home)
    ...
    actions.append(ReconcileAction(
        ...
        prior_exit=_read_prior_exit_label(xavani_home),
    ))

    profiles_root = xavani_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `xavani profile create` (config.yaml
            # is not — that comes later via `xavani setup`). Use it as the
            ...
            # via ``xavani -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(xavani_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = xavani_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("XAVANI_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the xavani args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/xavani/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/xavani/docker/main-wrapper.sh
    ...
    ``xavani`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``xavani <subcommand>``, so an
    explicit leading ``xavani`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `xavani <subcommand>`; peel an explicit xavani.
    if args and Path(args[0]).name == "xavani":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``xavani dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted XAVANI_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    from xavani_cli.service_manager import (
        S6ServiceManager,
    ...
    try:
        ...
        # it up explicitly with `xavani -p <profile> gateway start`
        # (which routes through the Phase 4
        # _dispatch_via_service_manager_if_s6 helper to `s6-svc -u`).
        if not start:
            (tmp_dir / "down").touch()

        # Pre-create the supervise/ skeleton with xavani ownership
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit xavani
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the xavani user) won't hit EACCES. See
...
def _write_reconcile_log(
    xavani_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $XAVANI_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted XAVANI_HOME, both race to flock() the same s6-log lock
    ...
    xavani_home = Path(os.environ.get("XAVANI_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        xavani_home=xavani_home, scandir=scandir,
    )
    for a in actions:
        print(
