...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local XAVANI_HOME override (profile
        ...
        try:
            ...
            home_override = get_xavani_home_override()
        except Exception:
            home_override = None
        ...
        def _discover() -> None:
            token = None
            try:
                from xavani_constants import set_xavani_home_override

                token = set_xavani_home_override(home_override)
            except Exception:
                token = None
            try:
                ...
                if token is not None:
                    try:
                        from xavani_constants import reset_xavani_home_override
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``xavani -z "..."`` / ``-q``), the larger
    ...
    try:
        from xavani_cli.config import load_config, DEFAULT_CONFIG
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``xavani dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``xavani -z``) can construct
