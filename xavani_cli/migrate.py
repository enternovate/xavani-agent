"""CLI handlers for ``xavani migrate ...``.

Currently exposes only ``xavani migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `xavani migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `xavani doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
