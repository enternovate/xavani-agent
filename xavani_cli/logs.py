...
"""``xavani logs`` — view and filter Xavani log files.
...
Usage examples::
    ...
    xavani logs                    # last 50 lines of agent.log
    xavani logs -f                 # follow agent.log in real time
    xavani logs errors             # last 50 lines of errors.log
    xavani logs gateway -n 100    # last 100 lines of gateway.log
    xavani logs --level WARNING    # only WARNING+ lines
    xavani logs --session abc123   # filter by session ID substring
    xavani logs --component tools  # only tool-related lines
    xavani logs --since 1h         # lines from the last hour
    xavani logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_xavani_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Xavani runs — try 'xavani chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_xavani_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_xavani_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_xavani_home()}/logs/")
        return

    print(f"Log files in {display_xavani_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'xavani chat' to generate logs)")
