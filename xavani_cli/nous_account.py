    ...
    *,
    ...
    if account_info.paid_service_access is None:
        ...
        if include_refresh_hint:
            detail += " Run `xavani model` to refresh your session."
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `xavani model` to refresh Xavani."
        return message
...
def nous_policy_present() -> Optional[bool]:
    ...
    """
    try:
        from xavani_cli.auth import get_provider_auth_state, _decode_jwt_claims
...
def nous_policy_notice(*, removed: bool) -> str:
    ...
    A blocked model is omitted rather than marked, which reads as "Xavani does
...
def _fresh_account_info(
    *,
    ...
    try:
        from xavani_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from xavani_cli.auth import _decode_jwt_claims
    except Exception:
        return None
