...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m xavani_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Xavani_Gateway"
_TASK_DESCRIPTION = "Xavani Agent Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    Default profile: ``Xavani_Gateway``
    Named profile X: ``Xavani_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during xavani_cli boot.
    from xavani_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\xavani\\gateway-service\\<task_name>.cmd``
    (or ``<XAVANI_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Xavani installs stay self-contained).
    """
    _assert_windows()
    from xavani_cli.config import get_xavani_home

    script_dir = Path(get_xavani_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports XAVANI_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m xavani_cli.main [--profile X] gateway run --replace``
    ...
    lines.append(f'set "XAVANI_HOME={xavani_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "XAVANI_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports xavani_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    prog_args = [python_path, "-m", "xavani_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    xavani_home = str(Path(get_xavani_home()).resolve())
    profile_arg = _profile_arg(xavani_home)

    content = _build_gateway_cmd_script(python_path, working_dir, xavani_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "xavani_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m xavani_cli.main gateway run --replace``
    ...
    log_dir = Path(get_xavani_home()) / "logs"
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from xavani_cli.config import get_xavani_home
        print(f"    type {Path(get_xavani_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_xavani_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from xavani_cli.config import get_xavani_home

    xavani_home = Path(get_xavani_home()).resolve()
    print()
    print("Next steps:")
    print("  xavani gateway status                      # Check status")
    print(f"  type {xavani_home}\\logs\\gateway.log       # View logs")
...
def stop() -> None:
    ...
    _assert_windows()
    from xavani_cli.gateway import kill_gateway_processes
