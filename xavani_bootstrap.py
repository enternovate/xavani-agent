...
"""Windows UTF-8 bootstrap for Xavani entry points.
...
should be imported at the very top of every Xavani entry point
(``xavani``, ``xavani-agent``, ``xavani-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import xavani_bootstrap``
# (or ``from xavani_bootstrap import apply_windows_utf8_bootstrap``) at
