...
_XAVANI_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (XAVANI_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `xavani tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (XAVANI_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Xavani toolsets (CLI + messaging platforms)
    #
    ...
    "xavani-cli": {
        ...
        "tools": _XAVANI_CORE_TOOLS,
        "includes": []
    },

    "xavani-cron": {
        # Mirrors xavani-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `xavani tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as xavani-cli; gated by `xavani tools`",
        "tools": _XAVANI_CORE_TOOLS,
        "includes": []
    },
    ...
    "xavani-email": {
        "description": "Email bot toolset - interact with Xavani via email (IMAP/SMTP)",
        "tools": _XAVANI_CORE_TOOLS,
        "includes": []
    },
    ...
    "xavani-sms": {
        "description": "SMS bot toolset - interact with Xavani via SMS (Twilio)",
        "tools": _XAVANI_CORE_TOOLS,
        "includes": []
    },
    ...
    "xavani-gateway": {
        ...
        "tools": [],
        "includes": ["xavani-telegram", "xavani-discord", "xavani-whatsapp", "xavani-slack", "xavani-signal", "xavani-bluebubbles", "xavani-homeassistant", "xavani-email", "xavani-sms", "xavani-mattermost", "xavani-matrix", "xavani-dingtalk", "xavani-feishu", "xavani-wecom", "xavani-wecom-callback", "xavani-weixin", "xavani-qqbot", "xavani-webhook", "xavani-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (xavani-<name>).
        # Gives them _XAVANI_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("xavani-"):
            platform_name = name[len("xavani-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_XAVANI_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
