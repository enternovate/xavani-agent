...
React + Ink terminal UI for Xavani. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `XAVANI_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.xavani/.xavani_history` or under `XAVANI_HOME`.
...
```text
ui-tui/
  packages/xavani-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      xavani-ink.d.ts    type declarations for @xavani/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent XavaniCLI subprocess for slash commands
```
