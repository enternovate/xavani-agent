import type { RunExternalProcess } from '@xavani/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`xavani ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching xavani: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`xavani ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
