...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.XAVANI_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.xavani', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `xavani-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
