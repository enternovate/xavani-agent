...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.XAVANI_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Xavani on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via XAVANI_TUI_TRUECOLOR=1.
