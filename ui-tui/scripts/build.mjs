...
await build({
  ...
  // Skip the prebuilt @xavani/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@xavani/ink': resolve(root, 'packages/xavani-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The xavani_cli launcher
