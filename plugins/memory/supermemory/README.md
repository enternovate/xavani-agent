...
Config file: `$XAVANI_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `xavani` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `xavani-{identity}` → `xavani-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Xavani profile:

```json
{
  "container_tag": "xavani-{identity}"
}
```

For a profile named `coder`, this resolves to `xavani-coder`. The default profile resolves to `xavani-default`. Without `{identity}`, all profiles share the same container.
