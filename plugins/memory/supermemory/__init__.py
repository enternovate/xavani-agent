...
def _load_supermemory_config(xavani_home: str) -> dict:
    config = _default_config()
    config_path = Path(xavani_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, xavani_home: str) -> None:
    config_path = Path(xavani_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `xavani memory setup`.
        # All other options are documented for $XAVANI_HOME/supermemory.json
    ...
    def save_config(self, values, xavani_home):
        ...
        _save_supermemory_config(sanitized, xavani_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from xavani_constants import get_xavani_home
        self._xavani_home = kwargs.get("xavani_home") or str(get_xavani_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._xavani_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "xavani", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "xavani_memory", "target": target, "type": "explicit_memory"},
