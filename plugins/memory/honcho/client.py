...
Resolution order for config file:
  1. $XAVANI_HOME/honcho.json  (instance-local, enables isolated Xavani instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Xavani profile.

    Resolution order:
      1. XAVANI_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``xavani.<profile>``
      3. Fallback: ``"xavani"`` (default profile)
    """
    explicit = os.environ.get("XAVANI_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $XAVANI_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.xavani/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_xavani_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_xavani_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Xavani system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.xavani
    ...
    def from_global_config(
        ...
        Resolution: $XAVANI_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Xavani profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.xavani block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Xavani session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Xavani session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Xavani session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'xavani honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run xavani honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `xavani honcho setup` to configure)."
        )
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            xavani_cfg = load_config()
            honcho_cfg = xavani_cfg.get("honcho", {})
