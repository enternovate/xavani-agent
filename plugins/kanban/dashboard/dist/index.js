...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "xavani-kanban-dot-triage",
    todo: "xavani-kanban-dot-todo",
    ready: "xavani-kanban-dot-ready",
    running: "xavani-kanban-dot-running",
    blocked: "xavani-kanban-dot-blocked",
    done: "xavani-kanban-dot-done",
    archived: "xavani-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-xavani-task";
  ...
  const DOCS_URL = "https://enternovate.co.za/xavani-agent/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://enternovate.co.za/xavani-agent/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "xavani.kanban.selectedBoard";
...
function renderInline(esc) {
    ...
    // Security: escapeHtml is applied before renderInline, but we must
    // also escape href/text for safe HTML attribute interpolation.
    return esc
      // inline code
      .replace(/`([^`\n]+)`/g, (_m, c) => `<code>${c}</code>`)
      // bold
      ...
      // Security: escape quotes in href to prevent attribute injection
      .replace(
        /\[([^\]\n]+)\]\((https?:\/\/[^\s)]+|mailto:[^\s)]+)\)/g,
        (_m, text, href) => {
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="xavani-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "xavani-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "xavani-kanban-md",
  ...
  // standard `drop` event and our `xavani-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("xavani-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("xavani-kanban-column--drop");
          if (target) target.classList.add("xavani-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("xavani-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("xavani-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("xavani-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "xavani-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "xavani-kanban-attention",
        "xavani-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "xavani-kanban-attention-bar" },
        h("span", { className: "xavani-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "xavani-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "xavani-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "xavani-kanban-attention-row",
                  "xavani-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "xavani-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "xavani-kanban-attention-row-id" }, task.id),
                h("span", { className: "xavani-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "xavani-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "xavani-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "xavani-kanban-diag-action-btn",
      action.suggested ? "xavani-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " xavani-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".xavani-kanban-drawer-comment-row input, .xavani-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "xavani-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("xavani-kanban-diag", sevClass) },
      h("div", { className: "xavani-kanban-diag-header" },
        h("span", { className: "xavani-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "xavani-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "xavani-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "xavani-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "xavani-kanban-diag-data-row" },
                  h("span", { className: "xavani-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "xavani-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "xavani-kanban-diag-data-row" },
                h("span", { className: "xavani-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "xavani-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "xavani-kanban-diag-reassign-row" },
            h("span", { className: "xavani-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "xavani-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "xavani-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "xavani-kanban-diag-msg",
              msg.ok ? "xavani-kanban-diag-msg--ok" : "xavani-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "xavani-kanban-section" },
      h("div", { className: "xavani-kanban-section-head-row" },
        h("span", { className: "xavani-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "xavani-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "xavani-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "xavani-kanban-diag-list" },
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `xavani kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "xavani-kanban-msg-ok" : "xavani-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "xavani-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "xavani-kanban-boardswitcher" },
      h("div", { className: "xavani-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "xavani-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "xavani-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "xavani-kanban-dialog-title" },
        ...
        h("div", { className: "xavani-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "xavani-kanban-bulk" },
      h("span", { className: "xavani-kanban-bulk-count" },
      ...
      h("div", { className: "xavani-kanban-bulk-priority",
      ...
      h("label", { className: "xavani-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("xavani-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("xavani-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "xavani-kanban-trash",
        dragOver ? "xavani-kanban-trash--drop" : "",
        props.draggingTaskId ? "xavani-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "xavani-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "xavani-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".xavani-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "xavani-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("xavani-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("xavani-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "xavani-kanban-column",
        dragOver ? "xavani-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "xavani-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "xavani-kanban-col-check",
        ...
        h("span", { className: cn("xavani-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "xavani-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "xavani-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "xavani-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "xavani-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "xavani-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "xavani-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "xavani-kanban-lane" },
                  h("div", { className: "xavani-kanban-lane-head" },
                    h("span", { className: "xavani-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "xavani-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "xavani-kanban-card--stale-red";
    if (age >= tier.amber) return "xavani-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".xavani-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "xavani-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "xavani-kanban-card",
        props.selected ? "xavani-kanban-card--selected" : "",
        props.failed ? "xavani-kanban-card--failed" : "",
        props.draggingSource ? "xavani-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "xavani-kanban-card-content" },
          h("div", { className: "xavani-kanban-card-row" },
            h("label", {
              className: "xavani-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "xavani-kanban-card-check",
            ...
            h("span", { className: "xavani-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or xavani kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "xavani-kanban-warning-badge",
                    "xavani-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "xavani-kanban-needs-assignee",
          ...
          h("div", { className: "xavani-kanban-card-title" },
          ...
          h("div", { className: "xavani-kanban-card-row xavani-kanban-card-meta" },
            ...
            t.comment_count > 0
              ? h("span", { className: "xavani-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "xavani-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "xavani-kanban-inline-create" },
      h("textarea", {
        value: title,
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "xavani-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "xavani-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "xavani-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "xavani-kanban-drawer-close",
        ...
        data ? h("div", { className: "xavani-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "xavani-kanban-drawer-body" },
      h("div", { className: "xavani-kanban-drawer-title" },
        h("span", { className: cn("xavani-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "xavani-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "xavani-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "xavani-kanban-section" },
        h("div", { className: "xavani-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "xavani-kanban-section" },
        h("div", { className: "xavani-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "xavani-kanban-comment" },
                h("div", { className: "xavani-kanban-comment-head" },
                  h("span", { className: "xavani-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "xavani-kanban-comment-ago" },
      ...
      h("div", { className: "xavani-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "xavani-kanban-event",
              isDiag ? "xavani-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "xavani-kanban-event-header" },
                  h("span", { className: "xavani-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "xavani-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "xavani-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "xavani-kanban-event-header-plain" },
                  h("span", { className: "xavani-kanban-event-kind" }, e.kind),
                  h("span", { className: "xavani-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "xavani-kanban-event-phantom-row" },
                  h("span", { className: "xavani-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "xavani-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "xavani-kanban-event-payload" },
    ...
    return h("div", { className: "xavani-kanban-section" },
      h("div", { className: "xavani-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "xavani-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `xavani-kanban-run--${r.outcome || r.status || "ended"}`
          : "xavani-kanban-run--active";
        return h("div", { key: r.id, className: cn("xavani-kanban-run", outcomeClass) },
          h("div", { className: "xavani-kanban-run-head" },
            h("span", { className: "xavani-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "xavani-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "xavani-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "xavani-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "xavani-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "xavani-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "xavani-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "xavani-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "xavani-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "xavani-kanban-pre xavani-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "xavani-kanban-meta-row" },
      h("span", { className: "xavani-kanban-meta-label" }, props.label),
      h("span", { className: "xavani-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "xavani-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "xavani-kanban-meta-row" },
        h("span", { className: "xavani-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "xavani-kanban-meta-value xavani-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "xavani-kanban-meta-row" },
        h("span", { className: "xavani-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "xavani-kanban-meta-value xavani-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "xavani-kanban-section" },
      h("div", { className: "xavani-kanban-section-head-row" },
        h("span", { className: "xavani-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "xavani-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "xavani-kanban-section" },
      h("div", { className: "xavani-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "xavani-kanban-deps-row" },
        h("span", { className: "xavani-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "xavani-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "xavani-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "xavani-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "xavani-kanban-dep-chip-x",
      ...
      h("div", { className: "xavani-kanban-deps-row" },
        h("span", { className: "xavani-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "xavani-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "xavani-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "xavani-kanban-msg-ok"
          : "xavani-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "xavani-kanban-section" },
      ...
      h("div", { className: "xavani-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "xavani-kanban-home-sub xavani-kanban-home-sub--on"
              : "xavani-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__XAVANI_PLUGINS__ && typeof window.__XAVANI_PLUGINS__.register === "function") {
    window.__XAVANI_PLUGINS__.register("kanban", KanbanPage);
  }
})();
