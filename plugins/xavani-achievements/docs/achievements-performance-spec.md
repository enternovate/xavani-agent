# Xavani Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: xavani-achievements plugin
...
### `GET /api/plugins/xavani-achievements/achievements`
...
### `POST /api/plugins/xavani-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/xavani-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/xavani-achievements/overview`
...
- Persist plugin data under:
  - `~/.xavani/plugins/xavani-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.xavani/plugins/xavani-achievements/scan_checkpoint.json`
...
- `~/.xavani/plugins/xavani-achievements/`
