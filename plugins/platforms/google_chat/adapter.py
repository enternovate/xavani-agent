...
# ``model_tools`` import time, so ``xavani status``, ``xavani chat``, etc.
...
# ``xavani_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Xavani is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a xavani MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${XAVANI_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _xavani_home = _get_xavani_home()
        except (ModuleNotFoundError, ImportError):
            _xavani_home = _Path.home() / ".xavani"
        self._thread_count_store = _ThreadCountStore(
            _xavani_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Xavani is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("XAVANI_HOME", str(_Path.home() / ".xavani"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${XAVANI_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a xavani MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Xavani is thinking…' marker message.
        ...
        "Xavani is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Xavani is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Xavani is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Xavani is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Xavani is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``xavani setup``.

    The setup wizard at ``xavani_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.xavani/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. xavani-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.xavani/.env")
    print_info("Restart the gateway: xavani gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``xavani cron`` running as a
    separate process from ``xavani gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Xavani plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'xavani-agent[google_chat]'",
        ...
        # the "Xavani is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Xavani is thinking…' marker message "
