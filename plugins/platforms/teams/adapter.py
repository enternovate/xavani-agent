...
"""
Microsoft Teams platform adapter for Xavani Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Xavani"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        xavani_action = data.get("xavani_action", "")
        session_key = data.get("session_key", "")

        if not xavani_action or not session_key:
        ...
        choice = choice_map.get(xavani_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="xavani_approve",
                    data={**btn_data_base, "xavani_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="xavani_approve",
                    data={**btn_data_base, "xavani_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="xavani_approve",
                    data={**btn_data_base, "xavani_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="xavani_approve",
                    data={**btn_data_base, "xavani_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Xavani\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.xavani/.env")
