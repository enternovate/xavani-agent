...
Auto-tracks and cleans up ephemeral files created during Xavani Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$XAVANI_HOME` and `/tmp/xavani-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `XAVANI_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `XAVANI_HOME` or `/tmp/xavani-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$XAVANI_HOME/disk-cleanup/` is itself excluded
- `$XAVANI_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
