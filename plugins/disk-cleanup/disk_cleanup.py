...
"""disk_cleanup — ephemeral file cleanup for Xavani Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under XAVANI_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly XAVANI_HOME and /tmp/xavani-*
Never touches: ~/.xavani/logs/ or any system directory.
"""
...
try:
    ...
    def get_xavani_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("XAVANI_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".xavani").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$XAVANI_HOME/logs/``."""
    return get_xavani_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$XAVANI_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under XAVANI_HOME or ``/tmp/xavani-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    xavani_home = get_xavani_home()
    try:
        path.resolve().relative_to(xavani_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/xavani-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("xavani-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside XAVANI_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under XAVANI_HOME (but leave XAVANI_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "xavani-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(xavani_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == xavani_home:
                continue
            try:
                rel_parts = dirpath.relative_to(xavani_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    xavani_home = get_xavani_home()
    try:
        rel = path.resolve().relative_to(xavani_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "xavani-agent",
        }:
            return None
        ...
        # Path isn't under XAVANI_HOME (e.g. /tmp/xavani-*) — fall through.
        pass
