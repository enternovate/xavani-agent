...
session-scoped state file under ``$XAVANI_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $XAVANI_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_xavani_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this xavani install, leave it first —
    ...
    env = os.environ.copy()
    env["XAVANI_MEET_URL"] = url
    env["XAVANI_MEET_OUT_DIR"] = str(out)
    env["XAVANI_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["XAVANI_MEET_HEADED"] = "1"
    if auth_state:
        env["XAVANI_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["XAVANI_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if XAVANI_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["XAVANI_MEET_MODE"] = mode
    if realtime_model:
        env["XAVANI_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["XAVANI_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["XAVANI_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["XAVANI_MEET_REALTIME_KEY"] = realtime_api_key
