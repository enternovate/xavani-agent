...
# Bootstrap Open WebUI against Xavani Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.xavani/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-xavani.sh
...
#   XAVANI_API_PORT=8642
#   XAVANI_API_HOST=127.0.0.1
#   XAVANI_API_MODEL_NAME='Xavani Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Xavani Agent WebUI}"
...
XAVANI_ENV_FILE="${XAVANI_ENV_FILE:-$HOME/.xavani/.env}"
XAVANI_API_PORT="${XAVANI_API_PORT:-8642}"
XAVANI_API_HOST="${XAVANI_API_HOST:-127.0.0.1}"
XAVANI_API_CONNECT_HOST="${XAVANI_API_CONNECT_HOST:-127.0.0.1}"
XAVANI_API_MODEL_NAME="${XAVANI_API_MODEL_NAME:-Xavani Agent}"
XAVANI_API_BASE_URL="http://${XAVANI_API_CONNECT_HOST}:${XAVANI_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-xavani.sh"
LOG_DIR="$HOME/.xavani/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$XAVANI_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$XAVANI_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.xavani.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.xavani</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.xavani"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.xavani"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-xavani.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Xavani Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-xavani.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.xavani/logs/openwebui.log
StandardError=append:%h/.xavani/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-xavani.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$XAVANI_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Xavani API server is configured...'
  upsert_env API_SERVER_ENABLED true "$XAVANI_ENV_FILE"
  upsert_env API_SERVER_HOST "$XAVANI_API_HOST" "$XAVANI_ENV_FILE"
  upsert_env API_SERVER_PORT "$XAVANI_API_PORT" "$XAVANI_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$XAVANI_API_MODEL_NAME" "$XAVANI_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$XAVANI_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Xavani gateway so API server settings take effect...'
  xavani gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${XAVANI_API_CONNECT_HOST}:${XAVANI_API_PORT}/health" >/dev/null; then
    log 'Xavani API server did not answer on the first check. Trying to start gateway in the background...'
    nohup xavani gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${XAVANI_API_CONNECT_HOST}:${XAVANI_API_PORT}/health" >/dev/null
  ...
  log "Xavani API endpoint: ${XAVANI_API_BASE_URL}"
