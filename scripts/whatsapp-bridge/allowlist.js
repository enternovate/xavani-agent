...
function readMappingFile(sessionDir, identifier, suffix = '') {
  ...
  const safeSuffix = String(suffix || '').replace(/[/\\..]/g, '_');
  const filePath = path.join(sessionDir, `lid-mapping-${safeId}${safeSuffix}.json`);
  const resolved = path.resolve(filePath);
  const resolvedBase = path.resolve(sessionDir);
