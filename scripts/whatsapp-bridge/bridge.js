 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.xavani/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.xavani', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.xavani', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.xavani', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.xavani', 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '⚕ *Xavani Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Xavani Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Xavani' own reply messages in self-chat mode to avoid loops.
...
app.post('/send-media', async (req, res) => {
  ...
  // Containment check using path.relative() — robust against the
  // /base vs /base-evil string-prefix bypass that startsWith() would
  // miss. We assign the resolved value to ``validatedPath`` only after
  // a positive match against one of the allowed roots; from that point
  // on every filesystem call uses ``validatedPath`` rather than the
  // raw ``resolvedPath``, which gives CodeQL's js/path-injection
  // analyser an explicit barrier on the data-flow.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `xavani_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
