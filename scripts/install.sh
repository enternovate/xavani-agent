...
# Python-driven tool session (e.g. Xavani terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:enternovate/xavani-agent.git"
REPO_URL_HTTPS="https://github.com/enternovate/xavani-agent.git"
XAVANI_HOME="${XAVANI_HOME:-$HOME/.xavani}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${XAVANI_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$XAVANI_INSTALL_DIR"
...
#   code at /usr/local/lib/xavani-agent, command at /usr/local/bin/xavani,
#   data still at /root/.xavani (XAVANI_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.xavani/xavani-agent"
            echo "                   default (root, Linux): /usr/local/lib/xavani-agent"
            echo "  --xavani-home PATH  Data directory (default: ~/.xavani, or \$XAVANI_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Xavani installs the code under"
            echo "  /usr/local/lib/xavani-agent and links the command into"
            echo "  /usr/local/bin/xavani (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$XAVANI_HOME"
            echo "  (default /root/.xavani).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$XAVANI_HOME/xavani-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs xavani setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `xavani` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $XAVANI_HOME/xavani-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $XAVANI_HOME/xavani-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/xavani-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $XAVANI_HOME/xavani-agent — then preserve it)
#
# Always no-op when the user set --dir or $XAVANI_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in XAVANI_HOME.
    if is_termux; then
        INSTALL_DIR="$XAVANI_HOME/xavani-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$XAVANI_HOME/xavani-agent/.git" ]; then
            INSTALL_DIR="$XAVANI_HOME/xavani-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/xavani-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/xavani-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/xavani"
        log_info "  Data:    $XAVANI_HOME (unchanged)"
        return 0
    fi
...
get_xavani_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/xavani" ]; then
        echo "$link_dir/xavani"
    else
        echo "xavani"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/enternovate/xavani-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/xavani-uv-install.$$.log")"
...
check_node() {
    ...
    if [ -x "$XAVANI_HOME/node/bin/node" ]; then
        export PATH="$XAVANI_HOME/node/bin:$PATH"
        local found_ver=$("$XAVANI_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Xavani-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.xavani/node/..."
    ...
    # Place into ~/.xavani/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$XAVANI_HOME/node"
    mkdir -p "$XAVANI_HOME"
    mv "$extracted_dir" "$XAVANI_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$XAVANI_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$XAVANI_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$XAVANI_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$XAVANI_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$XAVANI_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.xavani/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Xavani install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Xavani Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="xavani-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Xavani behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up xavani command..."

    if [ "$USE_VENV" = true ]; then
        XAVANI_BIN="$INSTALL_DIR/venv/bin/xavani"
    else
        XAVANI_BIN="$(which xavani 2>/dev/null || echo "")"
        if [ -z "$XAVANI_BIN" ]; then
            log_warn "xavani not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$XAVANI_BIN" ]; then
        log_warn "xavani entry point not found at $XAVANI_BIN"
    ...
    # Create a user-facing shim for the xavani command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $XAVANI_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$XAVANI_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/xavani"
    cat > "$command_link_dir/xavani" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$XAVANI_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/xavani"
    log_success "Installed xavani launcher → $command_link_display_dir/xavani"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "xavani command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v xavani' \
                >/dev/null 2>&1; then
        ...
        log_info "xavani not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Xavani Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Xavani Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so xavani works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    if [ ! -f "$XAVANI_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$XAVANI_HOME/.env"
            log_success "Created ~/.xavani/.env from template"
        else
            touch "$XAVANI_HOME/.env"
            log_success "Created ~/.xavani/.env"
        fi
    else
        log_info "~/.xavani/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$XAVANI_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.xavani/config.yaml (top level, easy to find)
    if [ ! -f "$XAVANI_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$XAVANI_HOME/config.yaml"
            log_success "Created ~/.xavani/config.yaml from template"
        fi
    else
        log_info "~/.xavani/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$XAVANI_HOME/SOUL.md" ]; then
        cat > "$XAVANI_HOME/SOUL.md" << 'SOUL_EOF'
...
-->
SOUL_EOF
        log_success "Created ~/.xavani/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.xavani/"

    # Seed bundled skills into ~/.xavani/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.xavani/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.xavani/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$XAVANI_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$XAVANI_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.xavani/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$XAVANI_HOME/.env"
    ...
    {
        echo ""
        echo "# Xavani Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Xavani will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (xavani --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'xavani setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m xavani_cli.main setup < /dev/tty
    else
        python -m xavani_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Xavani to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$XAVANI_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'xavani whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                XAVANI_CMD="$(get_xavani_command_path)"
                $XAVANI_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'xavani whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'xavani gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $XAVANI_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $XAVANI_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: xavani gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: xavani gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $XAVANI_CMD gateway > "$XAVANI_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.xavani/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: xavani gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: xavani gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${GREEN}xavani${NC}              Start chatting"
    echo -e "   ${GREEN}xavani setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}xavani config${NC}       View/edit configuration"
    echo -e "   ${GREEN}xavani config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}xavani gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}xavani update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'xavani' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'xavani' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'xavani' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$XAVANI_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$XAVANI_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$XAVANI_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$XAVANI_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$XAVANI_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Xavani for pip install"
    ...
    XAVANI_CMD="$(command -v xavani 2>/dev/null || echo "")"
    if [ -n "$XAVANI_CMD" ]; then
        log_info "Running xavani setup..."
        "$XAVANI_CMD" setup
    else
        log_warn "xavani command not found on PATH"
        log_info "Try: python -m xavani_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$XAVANI_HOME/.install_method"
}
