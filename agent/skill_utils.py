...
def yaml_load(content: str):
    ...
    if _yaml_load_fn is None:
        ...
        def _load(value: str):
            # Loader is always CSafeLoader/SafeLoader (set above) — no arbitrary
            # object construction. Safe despite the generic yaml.load() call.
            return yaml.load(value, Loader=loader)  # nosec B506
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``XAVANI_PLATFORM`` or
            ``XAVANI_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``xavani`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.xavani/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``xavani`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against XAVANI_HOME, not cwd
        if not p.is_absolute():
            p = (xavani_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    xavani = metadata.get("xavani") or {}
    if not isinstance(xavani, dict):
        xavani = {}
    return {
        "fallback_for_toolsets": xavani.get("fallback_for_toolsets", []),
        "requires_toolsets": xavani.get("requires_toolsets", []),
        "fallback_for_tools": xavani.get("fallback_for_tools", []),
        "requires_tools": xavani.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    xavani = metadata.get("xavani")
    if not isinstance(xavani, dict):
        return []
    raw = xavani.get("config")
    if not raw:
        return []
