...
Plugins built on Xavani Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Xavani
...
:class:`~xavani_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Xavani' main config
...
handles every provider, fallback chain, and per-task override Xavani
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`xavani_cli.plugins.PluginContext`
