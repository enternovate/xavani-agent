...
row taxonomy is inferred from the atlas shape so Xavani can render both legacy
...
# rows (1536x1872), while older Xavani/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Xavani' activity state names. They are not always identical to the
...
# Legacy Xavani/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Xavani activity names -> accepted row-name aliases in descending
