...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``xavani logs``.
    ...
    if (
        ...
        # Xavani only initiates compaction in "xavani" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Xavani preflight compression for codex app-server "
                "(mode=%s); Xavani will not start thread compaction here.",
    ...
    plugin_user_context = ""
    try:
        from xavani_cli.lifecycle import invoke_hook as _invoke_hook
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Xavani (via _emit_status), so it always
