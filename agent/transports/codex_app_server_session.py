...
Owns one Codex thread per Xavani session. Drives `turn/start`, consumes
...
# Xavani' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_XAVANI_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Xavani session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _XAVANI_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("XAVANI_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="xavani",
            client_title="Xavani Agent",
            client_version=_get_xavani_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Xavani' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Xavani' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own xavani-tools callback we
            # auto-accept — the user already approved Xavani' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Xavani approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Xavani returns 'once', 'session', 'always', or 'deny'.
...
def _get_xavani_version() -> str:
    """Best-effort Xavani version string for codex's userAgent line."""
    try:
