...
def main():
    ...
    # Get text input. We validate the user-supplied path using the
    # ``is_relative_to`` semantics rather than ``startswith`` so that an
    # attacker-controlled sibling like ``/tmp-evil/secrets`` cannot slip
    # past a ``/tmp`` allow-entry. ``Path.resolve`` follows symlinks, so
    # symlink escapes are also blocked.
    text = args.text or ""
    if args.file:
        from pathlib import Path as _Path

        filepath = _Path(args.file).expanduser().resolve()
        allowed_bases = [
            _Path.cwd().resolve(),
            _Path.home().resolve(),
            _Path("/tmp").resolve(),
        ...
        if not any(
            filepath == base or filepath.is_relative_to(base)
            for base in allowed_bases
        ):
            print(
                f"Error: Path traversal blocked — {args.file!r} resolves "
                "outside the allowed directories (cwd, home, /tmp)."
            )
            return
