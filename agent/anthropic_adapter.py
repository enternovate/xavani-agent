...
"""Anthropic Messages API adapter for Xavani Agent.

Translates between Xavani's internal OpenAI-style message format and
...
# Xavani effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Xavani historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Xavani)
    ...
    # 1. Xavani-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Xavani configs.
...
# ── Xavani-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.xavani/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_XAVANI_OAUTH_FILE = get_xavani_home() / ".anthropic_oauth.json"
...
def run_xavani_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Xavani-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Xavani with your Claude Pro/Max subscription.")
    print()
...
def read_xavani_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Xavani-managed OAuth credentials from ~/.xavani/.anthropic_oauth.json."""
    if _XAVANI_OAUTH_FILE.exists():
        try:
            data = json.loads(_XAVANI_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Xavani OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See xavani-agent#13848.
            #
    ...
    # are preserved.  See xavani-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    *session_id* is accepted for transport-chain uniformity (the codex path
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Xavani Agent", "Claude Code")
                text = text.replace("Xavani agent", "Claude Code")
                text = text.replace("xavani-agent", "claude-code")
                text = text.replace("Enternovate", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Xavani surfaces in its CLI. We
