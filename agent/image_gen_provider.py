...
as ``kind: backend``) or ``~/.xavani/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$XAVANI_HOME/cache/images/``, creating parents as needed."""
    from xavani_constants import get_xavani_home

    path = get_xavani_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$XAVANI_HOME/cache/images/``.
