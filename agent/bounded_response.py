...
When a provider returns a non-OK status on a *streaming* request, Xavani reads
...
2. A server can open the body and then stall, so the read can hang for as long
   as the underlying socket timeout (often 30s+), blocking the error path.
...
def read_urllib_error_body(
    exc: Exception,
    *,
    max_bytes: int = DEFAULT_ERROR_BODY_MAX_BYTES,
) -> str:
    """Bounded read of a urllib HTTPError body.

    urllib error bodies are file-like and unbounded in principle. Read at most
    ``max_bytes`` bytes and never raise (error path must not mask the original
    HTTP failure).
    """
