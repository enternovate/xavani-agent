...
- **skills** → the skill name (e.g. ``"debugging-xavani-tui"``)
...
shared by the CLI (``xavani journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``xavani curator restore``); deleting a memory rewrites its
...
def _memories_dir() -> Path:
    ...
    return get_xavani_home() / "memories"
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (xavani curator unpin {name})"}

    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()

    return {"ok": ok, "message": f"archived '{name}' — restore with: xavani curator restore {name}" if ok else message}
