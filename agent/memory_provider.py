...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - xavani_home (str): The active XAVANI_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.xavani``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "xavani").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'xavani memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], xavani_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'xavani memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``xavani_home`` is the active XAVANI_HOME directory path.
