...
from xavani_constants import get_xavani_home, get_skills_dir, is_wsl
...
_XAVANI_MD_NAMES = (".xavani.md", "XAVANI.md")


def _find_xavani_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.xavani.md`` or ``XAVANI.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Xavani Agent, an intelligent AI assistant created by Nous Research. "
...
XAVANI_AGENT_HELP_GUIDANCE = (
    ...
    "before answering. Docs: https://xavani-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.xavani/kanban.db`. Your task id is in "
    "`$XAVANI_KANBAN_TASK`; your workspace is `$XAVANI_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $XAVANI_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `xavani kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Xavani WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Xavani itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Xavani runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Xavani itself is running. The host OS, home, and cwd "
                f"of the Xavani process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Xavani "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_xavani_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.xavani/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("XAVANI_PLATFORM")
        or get_session_env("XAVANI_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Xavani Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `xavani-agent` skill "
            "first. It has the actual commands (e.g. `xavani config set …`, `xavani tools`, "
            "`xavani setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from XAVANI_HOME and return its content, or None.
    ...
    """
    try:
        from xavani_cli.config import ensure_xavani_home
        ensure_xavani_home()
    except Exception as e:
        logger.debug("Could not ensure XAVANI_HOME before loading SOUL.md: %s", e)

    soul_path = get_xavani_home() / "SOUL.md"
...
def _load_xavani_md(cwd_path: Path) -> str:
    """.xavani.md / XAVANI.md — walk to git root."""
    xavani_md_path = _find_xavani_md(cwd_path)
    if not xavani_md_path:
        return ""
    try:
        content = xavani_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = xavani_md_path.name
        try:
            rel = str(xavani_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".xavani.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", xavani_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .xavani.md / XAVANI.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from XAVANI_HOME is independent and always included when present.
    ...
    # SOUL.md from XAVANI_HOME only — skip when already loaded as identity
    if not skip_soul:
