...
Storage
-------
Bundles live in ``~/.xavani/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``XAVANI_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``xavani bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under XAVANI_HOME.

    Honors ``XAVANI_BUNDLES_DIR`` for tests; falls back to
    ``<XAVANI_HOME>/skill-bundles``.
    """
    override = os.environ.get("XAVANI_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_xavani_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `xavani bundles` CLI subcommand.
# ---------------------------------------------------------------------------
