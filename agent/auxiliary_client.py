...
Resolution order for text tasks (auto mode):
  ...
  3. Xavani Portal (~/.xavani/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Xavani triggers summarization.  Higher values delay
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``XAVANI_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``XAVANI_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("XAVANI_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("XAVANI_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "XavaniAgent",
}
...
# Xavani Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Xavani Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with xavani_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    """Return a fresh Xavani Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``xavani_cli.__version__`` is
...
# Set at resolve time — True if the auxiliary client points to Xavani Portal
auxiliary_is_nous: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_NOUS_MODEL = "google/gemini-3-flash-preview"
_NOUS_DEFAULT_BASE_URL = "https://inference-api.enternovate.co.za/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_xavani_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Xavani Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``xavani_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.xavani/auth.json for an active Xavani provider.

    Returns the provider state dict if Xavani is active with tokens,
    otherwise None.
    """
    ...
    try:
        ...
        logger.debug("Could not read Xavani auth: %s", exc)
        return None


def _nous_api_key(provider: dict) -> str:
    """Extract the Xavani runtime credential from the compatibility field."""
    return provider.get("agent_key") or provider.get("access_token", "")


def _nous_base_url() -> str:
    """Resolve the Xavani inference base URL from env or default."""
    return os.getenv("NOUS_INFERENCE_BASE_URL", _NOUS_DEFAULT_BASE_URL)


def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Xavani runtime credentials when available.
    ...
    try:
        ...
        creds = resolve_nous_runtime_credentials(
            min_key_ttl_seconds=max(60, int(os.getenv("XAVANI_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
            timeout_seconds=float(os.getenv("XAVANI_NOUS_TIMEOUT_SECONDS", "15")),
            inference_auth_mode=(
                NOUS_INFERENCE_AUTH_MODE_LEGACY
                if force_refresh
        ...
        logger.debug("Auxiliary Xavani runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``xavani auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``xavani_cli.auth``'s singleton runtime resolver for older
    auth-store-only logins. Returns ``None`` if the user is not authenticated
    with xAI Grok OAuth.
    """
    try:
        from xavani_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Xavani auth store.
    ...
    try:
        from xavani_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from xavani_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from xavani_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Xavani Portal was removed as a provider; auxiliary fallback skips it.
    return None, None

    # The historical body below is unreachable; kept for diff readability
    # while the surrounding fallback chain is cleaned up in a follow-up pass.
    try:
        from agent.nous_rate_guard import nous_rate_limit_remaining
        _remaining = nous_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Xavani Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    if runtime is None and not nous:
        logger.warning(
            "Auxiliary Xavani client unavailable: no Xavani authentication found "
            "(run: xavani auth)."
        )
        _mark_provider_unhealthy("nous", ttl=60)
        return None, None
    if runtime is None and nous:
        # Runtime credential mint failed but stored Xavani auth is still present.
        ...
        logger.debug(
            "Auxiliary Xavani: runtime credential mint failed; falling back to "
            "stored auth.json token."
        )
    global auxiliary_is_nous
    auxiliary_is_nous = True
    logger.debug("Auxiliary client: Xavani Portal")
    ...
    model = _NOUS_MODEL
    try:
        from xavani_cli.models import get_nous_recommended_aux_model
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `xavani setup` or `xavani model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`xavani_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from xavani_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second xavani
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``xavani config aux reset``)."""
...
def _recoverable_pool_provider(resolved_provider: str, client: Any) -> Optional[str]:
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.enternovate.co.za"):
        return "nous"
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Xavani get their chosen chat model; users
      ...
      2. OpenRouter → Xavani → custom → Codex → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `xavani model` but the
    #    old OPENAI_BASE_URL lingers in ~/.xavani/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: xavani model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.xavani/.env",
    ...
    # on aggregators (OpenRouter, Xavani) who previously got routed to a
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from xavani_cli.copilot_auth import copilot_request_headers
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Xavani Portal not configured (run: xavani auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: xavani model)")
                return None, None
    ...
    # OpenRouter / Xavani bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: xavani model -> xAI Grok OAuth — SuperGrok Subscription)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from xavani_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: xavani doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("xavani_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from xavani_cli.models import _should_use_copilot_responses_api
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Xavani → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Xavani — skip if already covered by main provider.
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Xavani Portal product tags when the auxiliary client is backed
    by Xavani Portal. Returns empty dict otherwise.
    """
...
def _refresh_nous_auxiliary_client(
    *,
    ...
    """Refresh Xavani runtime creds, rebuild the client, and replace the cache entry."""
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        # Codex adapter handles max_tokens internally; OpenRouter/Xavani use max_tokens.
...
def call_llm(
    ...
    *,
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `xavani model`."
                )
    ...
    try:
        ...
        # ── Xavani auth refresh parity with main agent ──────────────────
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_base_info, "inference-api.enternovate.co.za")
        )
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Xavani runtime credentials after 401, retrying",
                            task or "call")
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.enternovate.co.za")
        )
        if _is_auth_error(first_err) and client_is_nous:
            refreshed_client, refreshed_model = _refresh_nous_auxiliary_client(
                cache_provider=resolved_provider or "nous",
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Xavani runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
