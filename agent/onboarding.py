...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Xavani starts and finds ``~/.openclaw/``.

    Points users at ``xavani claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``xavani claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Xavani, run "
        "`xavani claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `xavani claw cleanup` (renames it to ~/.openclaw.pre-migration — "
