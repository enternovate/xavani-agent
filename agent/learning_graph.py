"""Assemble the "learning made visible" graph.
...
def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or _xavani_meta(fm).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or _xavani_meta(fm).get("category")
    if cat:
        return str(cat)
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_xavani_home() / "skills" / ".usage.json"
        try:
