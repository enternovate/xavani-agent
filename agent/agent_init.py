...
from xavani_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.xavani/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from xavani_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.xavani/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from xavani_logging import setup_logging, setup_verbose_logging
    setup_logging(xavani_home=_ra()._xavani_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that xavani_logging.setup_logging() does not
        ...
        # handler level inside xavani_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from xavani_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `xavani model` to "
                        "select a provider, or run `xavani setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff XAVANI_KANBAN_TASK is set).
    ...
    os.environ["XAVANI_SESSION_ID"] = agent.session_id
    try:
    ...
    # Session logs go into ~/.xavani/sessions/ alongside gateway sessions
    xavani_home = get_xavani_home()
    agent.logs_dir = xavani_home / "sessions"
    ...
    try:
        from xavani_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "xavani"
                    except Exception:
                        pass
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from xavani_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Xavani Agent.  Choose a model with at least "
