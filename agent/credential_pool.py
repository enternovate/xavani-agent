...
from xavani_cli.config import get_env_value, load_env
import xavani_cli.auth as auth_mod
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "nous":
            # Xavani stores the runtime inference credential in agent_key for
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``xavani model`` / ``xavani auth`` which
        ...
        Mirrors the Xavani/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Xavani process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Xavani pool entry from auth.json if tokens differ.

        Xavani OAuth refresh tokens are single-use.  When another process
        ...
        avoiding a "refresh token reuse" revocation on the Xavani Portal.
        """
        ...
        try:
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Xavani state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Xavani entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Xavani, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Xavani/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``xavani`` invocation that defaults to the active provider
        (e.g. setup wizard, ``xavani auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("xavani_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Xavani
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("xavani_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                # in-memory pool.  Mirrors the Xavani quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Xavani process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Xavani quarantine paths.
            ...
            if self.provider == "nous":
                synced = self._sync_nous_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Xavani refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                ...
                if auth_mod._is_terminal_nous_refresh_error(exc):
                    logger.debug("Xavani refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                        ...
                        logger.debug("Failed to clear terminal Xavani OAuth state: %s", clear_exc)
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "nous":
            # Xavani refresh/mint can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Xavani profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `xavani model` / `xavani auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `xavani model` ->
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `xavani auth remove <provider> <N>` is stable across all source types.
    try:
        from xavani_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Xavani PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_xavani_oauth_credentials

        for source_name, creds in (
            ("xavani_pkce", read_xavani_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `xavani auth add nous --label <name>`).  Fall back to the
        ...
        # Xavani auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from xavani_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.xavani/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `xavani auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `xavani auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Xavani auth store.  Without this gate the removal is instantly
        ...
        # Xavani owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``xavani model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``xavani auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.xavani/.env over os.environ — the user's config file is the
    # authoritative source for Xavani credentials. Stale env vars from parent
    ...
    # Honour user suppression — `xavani auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.xavani/.env.
    ...
    try:
        from xavani_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.xavani/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "xavani_pkce"}
        )
    ]
    if len(retained) == len(entries):
        return False
