...
Every Xavani request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Xavani Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=xavani-agent",
        "client=xavani-client-v<__version__>",
    ]

The version is sourced live from ``xavani_cli.__version__`` so it auto-aligns
...
``xavani_cli.__version__`` is the canonical source of truth.
"""
...
def _xavani_version() -> str:
    """Return the current Xavani release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``xavani_cli`` cannot be imported (should
...
def xavani_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=xavani-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=xavani-client-v{_xavani_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=xavani-agent", xavani_client_tag()]
