...
A pre-run snapshot of ``~/.xavani/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.xavani/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.xavani/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_xavani_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.xavani/cron/jobs.json``)."""
    return get_xavani_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.xavani/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.xavani/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.xavani/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.xavani/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `xavani curator rollback --list` to see available snapshots)",
            None,
        )
    ...
    try:
        with tarfile.open(archive, "r:gz") as tf:
            ...
            try:
                ...
                # Python < 3.12 — no filter kwarg. Members were already
                # validated above (no absolute paths / ".." traversal).
