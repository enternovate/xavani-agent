...
it is automatically downloaded from GitHub releases to $XAVANI_HOME/bin/tirith.
...
def _get_xavani_home() -> str:
    """Return the Xavani home directory, respecting XAVANI_HOME env var."""
    return str(get_xavani_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_xavani_home(), ".tirith-install-failed")
...
def _xavani_bin_dir() -> str:
    """Return $XAVANI_HOME/bin, creating it if needed."""
    d = os.path.join(_get_xavani_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $XAVANI_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_xavani_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $XAVANI_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $XAVANI_HOME/bin/tirith
    ...
    xavani_bin = os.path.join(_xavani_bin_dir(), "tirith")
    if os.path.isfile(xavani_bin) and os.access(xavani_bin, os.X_OK):
        _resolved_path = xavani_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return xavani_bin
