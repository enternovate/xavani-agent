...
Copies bundled skills from the repo's skills/ directory into ~/.xavani/skills/
...
The manifest lives at ~/.xavani/skills/.bundled_manifest.
"""
...
from xavani_constants import get_bundled_skills_dir, get_xavani_home
...
def _get_bundled_dir() -> Path:
    ...
    Checks XAVANI_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.xavani/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.xavani/skills/ using the manifest.
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `xavani skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `xavani skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `xavani update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.xavani/skills/ ...")
