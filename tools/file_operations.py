...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__XAVANI_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__XAVANI_FENCE_" in line or "\x1b]" in line
...
def _get_safe_write_root() -> Optional[str]:
    """Return the resolved XAVANI_WRITE_SAFE_ROOT path, or None if unset.
...
class ShellFileOperations(FileOperations):
    ...
    def _check_git_baseline(self, path: str) -> Optional[str]:
        """Check if the git working tree is dirty before writing a file.

        Returns:
            None if git is unavailable, not in a git repo, or the tree is clean.
            A warning string containing 'dirty' and the branch name if there are
            uncommitted changes.
        """
        # Check git is on PATH
        git_check = self._exec("command -v git")
        if git_check.exit_code != 0:
            return None

        # Check if inside a git work tree
        repo_check = self._exec("git rev-parse --is-inside-work-tree 2>/dev/null")
        ...
        status_result = self._exec("git status --porcelain 2>/dev/null")
        if not status_result.stdout.strip():
            return None

        return (
            f"Warning: git working tree is dirty on branch '{branch}'. "
            "Uncommitted changes may conflict with this write."
        )
