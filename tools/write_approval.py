...
(see ``xavani_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<XAVANI_HOME>/pending/{memory,skills}/<id>.json``
...
def write_approval_enabled(subsystem: str) -> bool:
    ...
    if subsystem not in _SUBSYSTEMS:
        return False
    try:
        from xavani_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _pending_dir(subsystem: str) -> Path:
    return get_xavani_home() / "pending" / subsystem
