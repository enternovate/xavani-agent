...
transport, discovers their tools, and registers them into the xavani-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.xavani/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.xavani/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            xavani_home = os.path.expanduser(
                os.getenv(
                    "XAVANI_HOME", os.path.join(os.path.expanduser("~"), ".xavani")
                )
            )
            candidates = [
                os.path.join(xavani_home, "node", "bin", resolved_command),
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Xavani MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Xavani gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Xavani is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `xavani mcp login {server_name}` (or delete the tokens "
            f"file under ~/.xavani/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Xavani config file.
    ...
    ``os.environ`` (which includes ``~/.xavani/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Xavani' MEDIA: tag convention and out to messaging
            ...
            # Xavani' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Xavani's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Xavani registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``xavani tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Xavani registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple xavani
