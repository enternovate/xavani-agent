...
# Must match xavani_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (xavani-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("xavani-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``xavani-cli`` (which bundles
...
def _build_child_system_prompt(
    ...
    *,
    ...
    When *persona* is a known specialist key, the persona's system prompt
    is prepended so the child starts with deep domain expertise.
    """
    persona_preamble = ""
    if persona:
        try:
            from xavani_cli.subagent_personas import get_persona as _get_persona
            p = _get_persona(persona)
            if p:
                persona_preamble = p["system_prompt"]
        except Exception:
            logger.debug("persona lookup failed for %s", persona, exc_info=True)

    parts = []
    if persona_preamble:
        parts.append(persona_preamble)
        parts.append("")
...
def _build_child_agent(
    ...
    role: str = "leaf",
    # Specialist persona key — prepends domain-expert system prompt.
    persona: Optional[str] = None,
):
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. xavani-cli) so that individual
    ...
    # Merge persona-recommended toolsets (intersected with parent scope).
    if persona:
        try:
            from xavani_cli.subagent_personas import get_persona as _get_persona
            p = _get_persona(persona)
            if p and p.get("recommended_toolsets"):
                for ts in p["recommended_toolsets"]:
                    if ts not in child_toolsets and ts in _expand_parent_toolsets(parent_toolsets):
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.xavani/logs/subagent-<sid>-<ts>.log``
...
def delegate_task(
    ...
    if tasks and isinstance(tasks, list):
        ...
        task_list = [
            {"goal": goal, "context": context, "toolsets": toolsets, "role": top_role, "persona": persona}
        ]
    else:
    ...
    try:
        for i, t in enumerate(task_list):
            ...
            child = _build_child_agent(
                ...
                role=effective_role,
                persona=t.get("persona") or persona,
            )
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'xavani auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (xavani_cli/config.py load_config()) so that
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "tasks": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "persona": {
                            "type": "string",
                            "description": "Per-task specialist persona. See top-level 'persona' for available options.",
                        },
                    },
                    "required": ["goal"],
                },
            ...
            "persona": {
                "type": "string",
                "description": (
                    "Specialist persona key that equips the subagent with "
                    "domain-expert knowledge. Available personas: "
                    "security-auditor, code-reviewer, architect-reviewer, "
                    "performance-engineer, deployment-engineer, "
                    "backend-architect, event-sourcing-architect, "
                    "graphql-architect, temporal-python-pro, "
                    "threat-modeling-expert, penetration-tester, "
                    "tdd-orchestrator, test-automator, "
                    "reliability-engineer, infrastructure-engineer, "
                    "python-specialist, typescript-specialist, "
                    "rust-specialist, go-specialist, java-specialist, "
                    "kotlin-specialist, cpp-specialist, "
                    "dart-flutter-specialist, csharp-specialist, "
                    "frontend-architect, ui-ux-specialist, "
                    "database-specialist, "
                    "karpathy-researcher, chollet-researcher, "
                    "lecun-researcher, swyx-researcher, "
                    "willison-researcher, hightower-researcher, "
                    "vaswani-researcher, jim-fan-researcher. "
                    "When set, the persona's system prompt and recommended "
                    "toolsets are merged into the child agent."
                ),
            },
            "acp_command": {
                "type": "string",
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Xavani subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
