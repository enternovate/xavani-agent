...
from tools.environments.local import _XAVANI_PROVIDER_ENV_BLOCKLIST
...
def _load_xavani_env_vars() -> dict[str, str]:
    """Load ~/.xavani/.env values without failing Docker command execution."""
    try:
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``XAVANI_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("XAVANI_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using XAVANI_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'xavani'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.xavani/sandboxes/). Non-persistent
        ...
        container_name = f"xavani-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Xavani secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _XAVANI_PROVIDER_ENV_BLOCKLIST)
        xavani_env = _load_xavani_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = xavani_env.get(key)
    ...
    def cleanup(self):
        ...
        if self._container_id:
            try:
                ...
                subprocess.Popen(stop_cmd, shell=True)  # nosec B602 — docker stop/rm with validated container ID
            except Exception as e:
            ...
            if not self._persistent:
                ...
                try:
                    subprocess.Popen(
                        ...
                        shell=True,  # nosec B602 — docker rm with validated container ID
                    )
                except Exception:
                    pass
