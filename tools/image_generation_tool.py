...
selectable via ``xavani tools`` → Image Generation; the active model is
...
def _build_no_backend_setup_message() -> str:
    ...
    lines.append(
        "  3. Configure a different image_gen provider via `xavani tools` "
        "→ Image Generation (run `xavani plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping xavani with a FAL key configured
