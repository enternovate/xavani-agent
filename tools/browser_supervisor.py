...
One ``CDPSupervisor`` runs per Xavani ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "xavani-dialog-bridge.invalid"
...
(() => {
  if (window.__xavaniDialogBridgeInstalled) return;
  window.__xavaniDialogBridgeInstalled = true;
  const ENDPOINT = "http://xavani-dialog-bridge.invalid/";
