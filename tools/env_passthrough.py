...
def _is_xavani_provider_credential(name: str) -> bool:
    """True if ``name`` is a Xavani-managed provider credential (API key,
    token, or similar) per ``_XAVANI_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Xavani API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _XAVANI_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Xavani-managed provider credentials (from
    ``_XAVANI_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Xavani-managed
    ...
    Non-Xavani third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_xavani_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Xavani provider "
                "credential %r (blocked by _XAVANI_PROVIDER_ENV_BLOCKLIST). "
