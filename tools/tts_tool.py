...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.xavani/config.yaml``. Xavani
...
Configuration is loaded from ~/.xavani/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``xavani_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from xavani_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import xavani_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_xavani_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.xavani/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.xavani/config.yaml.
    ...
    try:
        ...
        logger.debug("xavani_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into Xavani without any Python code
...
#           output_format: wav
#
# Xavani writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__XAVANI_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `xavani model` or set XAI_API_KEY.")
    ...
    # output_format when Xavani actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": xavani_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
    ...
    with open(output_path, "wb") as f:
        f.write(response.content)  # nosec B105 - binary audio bytes, not sensitive text
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Xavani caches Piper voice models.

    Resolves to ``~/.xavani/cache/piper-voices/`` under the active
    XAVANI_HOME so voice downloads follow profile boundaries.
    """
    from xavani_constants import get_xavani_dir
    root = Path(get_xavani_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.xavani/config.yaml (tts: section).
    ...
    platform = get_session_env("XAVANI_SESSION_PLATFORM", "").lower()
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run xavani setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'xavani setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'xavani tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_xavani_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
