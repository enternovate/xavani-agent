...
CHECKPOINT_PATH = get_xavani_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/xavani_bg_{session.id}.log"
        pid_path = f"{temp_dir}/xavani_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/xavani_bg_{session.id}.exit"
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `xavani update` restarting the gateway) is still
