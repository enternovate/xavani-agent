...
function renderToolChart(data) {
  ...
  tools.forEach(([name, stats]) => {
    ...
    labels.innerHTML += `<span>${esc(name)}</span>`;
  });
}
...
function renderToolTable(data) {
  ...
  sorted.forEach(([name, stats]) => {
    ...
    tbody.innerHTML += `<tr><td>${esc(name)}</td><td>${stats.call_count}</td><td>${fmtMs(stats.avg_ms)}</td><td>${errBadge}</td></tr>`;
  });
}
...
function renderErrors(data) {
  ...
  Object.entries(data.error_rates).forEach(([tool, info]) => {
    ...
    errTypes.forEach(([etype, count]) => {
      html += `<tr><td>${esc(tool)}</td><td><span class="badge err">${esc(etype)}</span></td><td>${count}</td><td>${info.error_rate_pct || 0}%</td></tr>`;
    });
  });
  html += '</tbody></table>';
  el.innerHTML = html;
...
async function loadTraces() {
  ...
  data.traces.forEach(t => {
    ...
    tbody.innerHTML += `<tr>
      <td>${esc(t.name) || '-'}</td>
      <td><span class="badge info">${esc(t.span_kind) || '-'}</span></td>
