...
"""
Monitor a running video-production kanban. Polls `xavani kanban list` and
...
def xavani_available() -> bool:
    return shutil.which("xavani") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed xavani CLI."""
    try:
        out = subprocess.run(
            ["xavani", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `xavani kanban list`
    out = subprocess.run(
        ["xavani", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["xavani", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not xavani_available():
        print("ERROR: 'xavani' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
