...
Maps each role archetype to the Xavani skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public xavani-agent
...
## Xavani skills relevant to video production

### Visual / rendering skills (`xavani-agent/skills/creative/`)
...
### Design / pre-production skills (`xavani-agent/skills/creative/`)
...
### Audio / media skills (`xavani-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`xavani-agent/skills/devops/`)
...
These are **not** Xavani skills but external CLIs / APIs that profiles invoke.
...
## Built-in Xavani tools for media review

These are native Xavani tools — not invoked via terminal but through their own
...
key is present in `~/.xavani/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Xavani profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.xavani/.env` → environment variable.
...
When a new Xavani-public video skill ships:
