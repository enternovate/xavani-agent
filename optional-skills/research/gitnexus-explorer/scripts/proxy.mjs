...
function serveStatic(req, res) {
  ...
  // Use path.relative() rather than startsWith() so the containment
  // check is robust against the /base vs /base-evil string-prefix
  // bypass and is statically recognised by CodeQL's js/path-injection
  // analyser. An "outside" path produces a relative result that either
  // begins with ".." or is itself absolute.
  {
  ...
  {
    const rel = path.relative(resolvedBase, finalPath);
    if (rel.startsWith('..') || path.isAbsolute(rel)) {
      res.writeHead(403, { 'Content-Type': 'text/plain' });
      res.end('Forbidden');
      return;
