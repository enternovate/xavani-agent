...
def main() -> int:
    ...
    # Security: pickle.loads can execute arbitrary code. Only load snapshots from
    # sources you trust. We restrict the classes that can be unpickled to reduce risk.
    import io
    import pickle as _pkl

    class _RestrictedUnpickler(_pkl.Unpickler):
        """Restrict unpickling to safe built-in types + permit user-defined classes
        via the standard module resolution (no arbitrary global lookup)."""
        def find_class(self, module, name):
            # Allow standard library modules and user modules (not builtins like os.system)
            if module.startswith(("evolver", "collections", "typing", "__main__")):
                return super().find_class(module, name)
            # Allow any module except restricted ones — the darwinian-evolver needs
            # flexibility for custom driver classes, but block known-dangerous modules
            if module.split(".")[0] in ("os", "subprocess", "shutil", "ctypes", "multiprocessing"):
                raise _pkl.UnpicklingError(f"Forbidden module: {module}.{name}")
            return super().find_class(module, name)

        @classmethod
        def safe_loads(cls, data):
            return cls(io.BytesIO(data)).load()

    outer = _RestrictedUnpickler.safe_loads(args.snapshot.read_bytes())
    ...
    inner = _RestrictedUnpickler.safe_loads(outer["population_snapshot"])
