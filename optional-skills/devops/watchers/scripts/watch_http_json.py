...
Usage (via cron with --no-agent):
    ...
    xavani cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$XAVANI_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Xavani-Watcher/1.0"})
