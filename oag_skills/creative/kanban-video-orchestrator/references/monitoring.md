...
xavani kanban watch --tenant <project-slug>

# Snapshot of the board
xavani kanban list --tenant <project-slug>
xavani kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
xavani kanban stats --tenant <project-slug>

# Visual dashboard (browser)
xavani dashboard

# Inspect a specific task (includes comments + events)
xavani kanban show <task-id>

# Follow a single task's event stream
xavani kanban tail <task-id>
```

Verify available subcommands with `xavani kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `xavani kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `xavani kanban block <id>` then `xavani kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `xavani kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `xavani kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `xavani profile list` (profile exists?), `xavani status` (gateway/dispatcher up?), and verify tenant. |
...
xavani kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
xavani kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(xavani kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
xavani kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
xavani kanban block <task-id>
xavani kanban archive <task-id>

# Diagnose what happened
xavani kanban show <task-id>      # task body, comments, recent events
xavani kanban tail <task-id>      # follow the live event stream
xavani kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `xavani kanban create` for the director
...
while true; do
    clear
    xavani kanban list --tenant <slug>
    echo "---"
    xavani kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `xavani kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `xavani profile list` and start
