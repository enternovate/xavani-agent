...
../assets/, and writes a setup.sh that creates Xavani profiles and fires the
initial kanban task.

Profile-config patching, SOUL.md-per-profile, TEAM.md task-graph convention,
and the `xavani kanban create --workspace dir:` initial-task pattern are
    ...
    if "team" in plan:
        if not isinstance(plan["team"], list) or not plan["team"]:
            ...
            for i, t in enumerate(plan["team"]):
                ...
                # Profile name must match Xavani's regex (lowercase
                ...
                if "profile" in t:
                    if not PROFILE_NAME_RE.match(t["profile"]):
                        errors.append(
                            ...
                            f"[a-z0-9][a-z0-9_-]{{0,63}} per Xavani profile rules"
                        )
...
def render_setup_sh(plan: dict, brief_md: str, team_md: str) -> str:
    ...
    for key in plan.get("api_keys_required", []):
        key_checks.append(f'check_key {key} xavani {key} || exit 1')
    ...
    for t in plan["team"]:
        profile_creates.append(
            f'xavani profile create {t["profile"]} --clone 2>/dev/null || true'
        )
    ...
    for t in plan["team"]:
        soul_writes.append(
            f'cat > "$HOME/.xavani/profiles/{t["profile"]}/SOUL.md" <<\'SOUL_EOF\'\n'
...
def main():
    ...
    Path(args.out).write_text(setup, encoding="utf-8")  # nosec B105 - generated shell script, no credentials
    os.chmod(args.out, 0o755)
    print(f"Wrote {args.out}")

    if args.brief_out:
        Path(args.brief_out).write_text(brief, encoding="utf-8")  # nosec B105 - generated markdown brief, no credentials
