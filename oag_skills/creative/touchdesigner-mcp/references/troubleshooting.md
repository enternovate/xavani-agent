...
## 5. Xavani Configuration

### Config location

`$XAVANI_HOME/config.yaml` (defaults to `~/.xavani/config.yaml` when `XAVANI_HOME` is unset)
...
Restart the Xavani session for changes to take effect. The MCP connection is
