...
XAVANI_HOME_DIR="${XAVANI_HOME:-$HOME/.xavani}"
XAVANI_CFG="${XAVANI_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Xavani config has twozero_td MCP entry ──
if [[ ! -f "$XAVANI_CFG" ]]; then
    echo -e " ${FAIL} Xavani config not found at ${XAVANI_CFG}"
    manual_steps+=("Create ${XAVANI_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$XAVANI_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Xavani config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Xavani config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${XAVANI_CFG} manually"); }
    manual_steps+=("Restart Xavani session to pick up config change")
fi
