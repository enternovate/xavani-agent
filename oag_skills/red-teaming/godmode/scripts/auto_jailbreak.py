...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("XAVANI_HOME", os.path.expanduser("~/.xavani")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("XAVANI_HOME", Path.home() / ".xavani")) / "skills" / "red-teaming" / "godmode"
...
PREFILL_PATH = XAVANI_HOME / "prefill.json"
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "xavani" in model_lower or "nous" in model_lower:
        return "xavani"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Xavani config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.xavani/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Xavani for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Xavani for changes to take effect.")
