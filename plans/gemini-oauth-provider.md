...
## Client ID
- Need to register a "Desktop app" OAuth client on a Enternovate GCP project
...
## Token Lifecycle
- Store at `~/.xavani/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `xavani_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `xavani_cli/models.py` — Add Gemini model catalog
4. `xavani_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `xavani_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `xavani_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Enternovate GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via XAVANI_GEMINI_CLIENT_ID env var
...
- xavani-agent Copilot OAuth: `xavani_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
