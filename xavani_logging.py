...
"""Centralized logging setup for Xavani Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.xavani/logs/`` (profile-aware via ``get_xavani_home()``).
...
from xavani_constants import get_config_path, get_xavani_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_xavani_session_injector", False):
...
# Used by _ComponentFilter and exposed for ``xavani logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "xavani_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Xavani logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    xavani_home
        Override for the Xavani home directory.  Falls back to
        ``get_xavani_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_xavani_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._xavani_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the xavani group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from xavani_cli.config import is_managed
