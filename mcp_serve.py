...
"""
Xavani MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Xavani-specific extra)

Usage:
    xavani mcp serve
    xavani mcp serve --verbose
...
logger = logging.getLogger("xavani.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using XAVANI_HOME."""
    try:
        from xavani_constants import get_xavani_home
        return get_xavani_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("XAVANI_HOME", Path.home() / ".xavani")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from xavani_constants import get_xavani_home
        directory_file = get_xavani_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("XAVANI_HOME", Path.home() / ".xavani")
...
class EventBridge:
    ...
    This is the Xavani equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from xavani_constants import get_xavani_home
            db_file = get_xavani_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("XAVANI_HOME", Path.home() / ".xavani")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Xavani MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "xavani",
        instructions=(
            "Xavani Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Xavani MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
