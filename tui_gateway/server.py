...
load_xavani_dotenv(
    xavani_home=_xavani_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.xavani/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_xavani_home, "logs", "tui_gateway_crash.log")
...
_cfg_path = None
try:
    _slash_timeout = float(os.environ.get("XAVANI_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("XAVANI_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent XavaniCLI subprocess for slash commands."""
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `xavani` shows the prompt before constructing AIAgent; the TUI used
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["XAVANI_GATEWAY_SESSION"] = "1"
...
def resolve_skin() -> dict:
    try:
        from xavani_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("XAVANI_MODEL", "")
        or os.environ.get("XAVANI_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("XAVANI_TUI_PROVIDER", "").strip()
    ...
    try:
        from xavani_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("XAVANI_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("XAVANI_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("XAVANI_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] XAVANI_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from xavani_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown XAVANI_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in XAVANI_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid XAVANI_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid XAVANI_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from xavani_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from xavani_cli.config import get_compatible_custom_providers, load_config
    ...
    os.environ["XAVANI_MODEL"] = result.new_model
    os.environ["XAVANI_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # XAVANI_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["XAVANI_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["XAVANI_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from xavani_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from xavani_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from xavani_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from xavani_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("XAVANI_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("XAVANI_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    return AIAgent(
        model=model,
        max_iterations=_cfg_max_turns(cfg, 300),
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("XAVANI_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("XAVANI_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("XAVANI_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("XAVANI_IGNORE_RULES")),
        **_agent_cbs(sid),
    )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `XAVANI_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"xavani_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
...
#
# Layout:  $XAVANI_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_xavani_home() / "spawn-trees"
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from xavani_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from xavani_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: xavani_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from xavani_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _xavani_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from xavani_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_xavani_home), "display": display_xavani_home()})
    if key == "full":
...
def _(rid, params: dict) -> dict:
    try:
        from xavani_cli.main import _has_any_provider_configured
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.xavani/.env`` into the gateway process via
    ``xavani_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `xavani` is interactive — use `/xavani chat -q …` or run `xavani` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`xavani setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`xavani gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`xavani sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`xavani config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m xavani_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "xavani_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    if name in qcmds:
        qc = qcmds[name]
        if qc.get("type") == "exec":
            # Security: use shlex.split instead of shell=True to prevent
            # command injection through config-sourced commands
            import shlex
            ...
            try:
                ...
                # Fallback for complex shell commands that shlex can't parse
                r = subprocess.run(
                    cmd_str,
                    shell=True,  # nosec B602 — fallback for complex shell syntax after shlex fails; command is user-authored and guard-checked above
                    capture_output=True,
                    text=True,
                    timeout=30,
                )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from xavani_cli.config import is_managed, save_env_value
        from xavani_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `xavani model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.xavani/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from xavani_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from xavani_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("XAVANI_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("XAVANI_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["XAVANI_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from xavani_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["XAVANI_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from xavani_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from xavani_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from xavani_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from xavani_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("XAVANI_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("XAVANI_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_xavani_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from xavani_cli.skills_hub import do_install
        ...
        if action == "browse":
            from xavani_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from xavani_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        pass
    # Security: prefer shlex.split over shell=True for safer execution
    import shlex
    try:
        cmd_args = shlex.split(cmd)
        r = subprocess.run(cmd_args, capture_output=True, text=True, timeout=30, cwd=os.getcwd())
    except ValueError:
        r = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=30, cwd=os.getcwd())  # nosec B602 — intentional fallback after shlex.split fails; guard-checked
        return _ok(
            rid,
            {
