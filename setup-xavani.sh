...
# 5. Symlinks the 'xavani' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Xavani Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/xavani-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink xavani into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up xavani command..."

XAVANI_BIN="$SCRIPT_DIR/venv/bin/xavani"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$XAVANI_BIN" "$COMMAND_LINK_DIR/xavani"
echo -e "${GREEN}✓${NC} Symlinked xavani → $COMMAND_LINK_DISPLAY_DIR/xavani"
...
# ============================================================================
# Seed bundled skills into ~/.xavani/skills/
# ============================================================================

XAVANI_SKILLS_DIR="${XAVANI_HOME:-$HOME/.xavani}/skills"
mkdir -p "$XAVANI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.xavani/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$XAVANI_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  xavani status        # Check configuration"
if is_termux; then
    echo "  xavani gateway       # Run gateway in foreground"
else
    echo "  xavani gateway install # Install gateway service (messaging + cron)"
fi
echo "  xavani cron list     # View scheduled jobs"
echo "  xavani doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m xavani_cli.main setup
fi
