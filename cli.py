...
class XavaniCLI:
    ...
    def show_banner(self):
        ...
        # Warn if the configured model is a Nous Xavani LLM (not agentic)
        from xavani_cli.model_switch import is_nous_xavani_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_xavani_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Xavani 3 & 4 models are NOT agentic and are not "
                "designed for use with Xavani Agent.[/]"
            )
            self._console_print(
