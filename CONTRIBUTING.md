# Contributing to Xavani Agent

Thank you for contributing to Xavani Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Xavani to work everywhere.
...
Bundled skills (in `skills/`) ship with every Xavani install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `xavani skills browse` (labeled "official") and install it with `xavani skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Enternovate Discord](https://discord.gg/enternovate). Users can install it with `xavani skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.xavani/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(xavani_home, config)` for setup-wizard integration
...
- Integrate with `xavani memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/enternovate/xavani-agent.git
cd enternovate/xavani-agent
...
```bash
mkdir -p ~/.xavani/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.xavani/config.yaml
touch ~/.xavani/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.xavani/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/xavani" ~/.local/bin/xavani
...
├── cli.py                    # XavaniCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (xavani-cli, xavani-telegram, etc.)
├── xavani_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── xavani_cli/               # CLI command implementations
...
### User configuration (stored in `~/.xavani/`)
...
| `~/.xavani/sessions/` | JSON session logs |
| `~/.xavani/cron/` | Scheduled job data |
| `~/.xavani/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`xavani_state.py`) with full-text search and unique session titles. JSON logs go to `~/.xavani/sessions/`.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (Xavani Portal OAuth, OpenRouter API key, or custom endpoint).
...
(for example `_XAVANI_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.xavani`:
...
The user may skip setup and keep loading the skill. Xavani only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `xavani setup` or update `~/.xavani/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Xavani tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Xavani Agent" is the secondary collaborator. If the contributor's commit shows "Xavani Agent" as author because they used Xavani to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Xavani tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `xavani --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Xavani uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.xavani/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `xavani_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `xavani_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Xavani runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the xavani wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `xavani_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `xavani_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `xavani_cli/gateway_windows.py::
...
Xavani has terminal access. Security matters.
...
2. **Test manually**: Run `xavani` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/enternovate/xavani-agent/issues)
- Include: OS, Python version, Xavani version (`xavani version`), full error traceback
...
- **Discord**: [discord.gg/enternovate](https://discord.gg/enternovate) — for questions, showcasing projects, and sharing skills
