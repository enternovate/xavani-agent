# OpenAI-Compatible API Server for Xavani Agent
...
SSE streaming. By exposing this endpoint, xavani-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  xavani-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.xavani/.env`
2. Run `xavani gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with xavani-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns xavani-agent as a model) |
...
Authorization: Bearer xavani-api-key-here
...
But xavani-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "xavani-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured xavani-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of xavani-agent's core prompt
...
Once implemented, xavani-agent works as a drop-in backend for:
