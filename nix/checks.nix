...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      xavani-agent = self'.packages.default;
      xavaniVenv = xavani-agent.xavaniVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "xavani-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${xavaniVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "xavani-cross-eval" { } (
          if failures != [] then
        ...
        entry-points-sync = pkgs.runCommand "xavani-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in xavani xavani-agent xavani-acp; do
            test -x ${xavani-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "xavani-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          grep -q "gateway" "$help_output" || (echo "FAIL: gateway option missing"; exit 1)
          ${xavani-agent}/bin/xavani config --help > /dev/null 2>&1 || (echo "FAIL: config subcommand missing"; exit 1)
          echo "PASS: All commands accessible"
        ...
        bundled-skills = pkgs.runCommand "xavani-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${xavani-agent}/share/xavani-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${xavani-agent}/share/xavani-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "XAVANI_BUNDLED_SKILLS" ${xavani-agent}/bin/xavani || \
            (echo "FAIL: XAVANI_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: XAVANI_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "xavani-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${xavani-agent}/share/xavani-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${xavani-agent}/share/xavani-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "XAVANI_BUNDLED_PLUGINS" ${xavani-agent}/bin/xavani || \
            (echo "FAIL: XAVANI_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: XAVANI_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "xavani-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${xavani-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${xavani-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "XAVANI_TUI_DIR" ${xavani-agent}/bin/xavani || \
            (echo "FAIL: XAVANI_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: XAVANI_TUI_DIR set in wrapper"
        ...
        # Verify XAVANI_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        xavani-node = pkgs.runCommand "xavani-node-version" { } ''
          set -e
          echo "=== Checking XAVANI_NODE in wrapper ==="
          grep -q "XAVANI_NODE" ${xavani-agent}/bin/xavani || \
            (echo "FAIL: XAVANI_NODE not set in wrapper"; exit 1)
          echo "PASS: XAVANI_NODE present in wrapper"

          XAVANI_NODE=$(sed -n "s/^export XAVANI_NODE='\(.*\)'/\1/p" ${xavani-agent}/bin/xavani)
          test -x "$XAVANI_NODE" || (echo "FAIL: XAVANI_NODE=$XAVANI_NODE not executable"; exit 1)
          echo "PASS: XAVANI_NODE executable at $XAVANI_NODE"

          NODE_MAJOR=$("$XAVANI_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All XAVANI_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify XAVANI_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "xavani-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(XAVANI_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking XAVANI_MANAGED guards ==="
          check_blocked "config set" ${xavani-agent}/bin/xavani config set model foo
          check_blocked "config edit" ${xavani-agent}/bin/xavani config edit
        ...
        extra-python-packages = let
          ...
          xavaniWithExtra = xavani-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "xavani-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${xavaniWithExtra}/bin/xavani || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${xavaniWithExtra}/bin/xavani || \
          ...
          if grep -q "PYTHONPATH" ${xavani-agent}/bin/xavani; then
        ...
        extra-dependency-groups = let
          xavaniWithGroups = xavani-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "xavani-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${xavaniWithGroups}"
          echo "venv: ${xavaniWithGroups.xavaniVenv}"
        ...
        in pkgs.runCommand "xavani-config-roundtrip" {
          ...
          merge_and_load() {
            local xavani_home="$1"
            export XAVANI_HOME="$xavani_home"
            ${configMergeScript} ${nixSettings} "$xavani_home/config.yaml"
            ${xavaniVenv}/bin/python3 -c '
import json, sys
