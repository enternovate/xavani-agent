# nix/nixosModules.nix — NixOS module for xavani-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: xavani runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $XAVANI_HOME/.env
# and read by xavani at startup — no container recreation needed for env changes.
#
# Tool resolution: the xavani wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.xavani-agent;
    effectivePackage =
    ...
    xavani-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-xavani-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "xavani-config-attrs";
      description = "Xavani YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "xavani-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "xavani-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'XAVANI_DOC_EOF'\n${value}\nXAVANI_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "xavani-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/xavani";
    ...
    # xavani user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "xavani-container-entrypoint" ''
      set -eu

      XAVANI_UID="''${XAVANI_UID:?XAVANI_UID must be set}"
      XAVANI_GID="''${XAVANI_GID:?XAVANI_GID must be set}"

      # ── Group: ensure a group with GID=$XAVANI_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$XAVANI_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="xavani"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$XAVANI_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$XAVANI_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$XAVANI_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$XAVANI_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="xavani"
        TARGET_HOME="/home/xavani"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$XAVANI_UID" -g "$XAVANI_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$XAVANI_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$XAVANI_UID:$XAVANI_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure XAVANI_HOME is owned by the target user.
      ...
      if [ -n "''${XAVANI_HOME:-}" ] && [ -d "$XAVANI_HOME" ]; then
        find "$XAVANI_HOME" \! -user "$XAVANI_UID" -exec chown "$XAVANI_UID:$XAVANI_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/xavani-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/xavani-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/xavani ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/xavani
        chmod 0440 /etc/sudoers.d/xavani
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$XAVANI_UID" --regid="$XAVANI_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $XAVANI_HOME/.env.
    ...
    # Default: /var/lib/xavani/workspace → /data/workspace.
  ...
  in {
    options.services.xavani-agent = with lib; {
      enable = mkEnableOption "Xavani Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = xavani-agent;
        description = "The xavani-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/xavani";
        description = "State directory. Contains .xavani/ subdir (XAVANI_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Xavani config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $XAVANI_HOME/.env at activation time.
          Xavani reads this file on every startup via load_xavani_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $XAVANI_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $XAVANI_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `xavani gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the xavani user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the xavani plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Xavani discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          xavani_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as xavani (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in xavani-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the xavani CLI to environment.systemPackages and export
          XAVANI_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.xavani symlink to the service
            stateDir. These users are automatically added to the xavani group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.xavani-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the xavani CLI to system PATH and export XAVANI_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.xavani/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.XAVANI_HOME = "${cfg.stateDir}/.xavani";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.xavani-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the xavani user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.xavani-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed xavani
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure xavani is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.xavani        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xavani/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xavani/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xavani/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xavani/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.xavani/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."xavani-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.xavani
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.xavani ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.xavani ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.xavani -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.xavani/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.xavani/$_subdir"
            chmod 2770 "${cfg.stateDir}/.xavani/$_subdir"
            find "${cfg.stateDir}/.xavani/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.xavani/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.xavani/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xavani/config.yaml
            chmod 0640 ${cfg.stateDir}/.xavani/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.xavani/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xavani/.managed
          chmod 0644 ${cfg.stateDir}/.xavani/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.xavani/.container-mode <<'XAVANI_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    xavani_bin=${containerDataDir}/current-package/bin/xavani
    XAVANI_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xavani/.container-mode
            chmod 0644 ${cfg.stateDir}/.xavani/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.xavani/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.xavani";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.xavani" ]; then
                  rm -f "${symlinkPath}"
                  echo "xavani-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.xavani -> stateDir/.xavani for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.xavani";
                target = "${cfg.stateDir}/.xavani";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "xavani-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.xavani/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.xavani/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.xavani/auth.json
              fi
            ''}
          ''}
          ...
          # Xavani reads $XAVANI_HOME/.env at startup via load_xavani_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.xavani/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'XAVANI_NIX_ENV_EOF'
    ${envFileContent}
    XAVANI_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.xavani/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.xavani/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.xavani/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.xavani-agent = {
          description = "Xavani Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            XAVANI_HOME = "${cfg.stateDir}/.xavani";
            XAVANI_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $XAVANI_HOME/.env by the activation script. load_xavani_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/xavani"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the xavani group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.xavani-agent = {
          description = "Xavani Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              XAVANI_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              XAVANI_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env XAVANI_UID="$XAVANI_UID" \
                --env XAVANI_GID="$XAVANI_GID" \
                --env XAVANI_HOME=${containerDataDir}/.xavani \
                --env XAVANI_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/xavani gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
